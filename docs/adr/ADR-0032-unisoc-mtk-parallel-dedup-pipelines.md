...
### D10：DLE 终态语义两平台同构（**已裁定**，#463，2026-09-19）

**裁决**（方向经 owner 确认：**展锐日志链逻辑与 MTK 保持一致**）：DLE 状态机对两平台
**同义同判**——`ARCHIVED` 的含义统一为「该事件目录已由 run 级 extract 复制进
`jira/{plan_run_id}/` 产物束」，触发与判据全仓唯一（`run_extract_sync` →
`mark_events_archived`，见
[`2026-scan-upload-merge-contract.md`](../design/2026-scan-upload-merge-contract.md)
「DLE 归档」节），**不引入任何平台分支**。

- **#463 2026-09-14 评论的 A/B 取舍**：**否决 A**（「上送即归档」——会让 `ARCHIVED`
  在两平台含义分叉）。**B 的目标（状态机语义一致）已达成**，但达成路径不是 B 原设想
  的「给归档层开无 xls 分支」，而是**给展锐补齐等价输入链**（D4c：`UnisocScanRunner`
  → `scan_result.py` 15 列 → D3 同一 merge 工具 → `merge_result_xls` 产物），归档层
  **零改动**——优于原设想，也是 D1「并列同构、禁止交叉」的自然结果。
- **生产证据**：真实 run 400/399/397 发布路径三判据核对（#2253，2026-09-16）；
  UNISOC `REMOTE` 660 / `ARCHIVED` 25、推进 7–22s（#1998，2026-09-18 只读实测）。
- **实现层残余差异**（登记于契约文档，**不构成状态机差异**）：scan 输入来源（MTK
  厂商外挂脚本 vs 展锐 Agent 内建 GT 工具链，D3/D4c）、上送触发源（MTK 由 scan xls
  Path 列标记精选子集 vs 展锐入库即提升 `UPLOAD_PENDING`，#1957）、merge 输入表头
  两列命名差异由工具归一（§B3）。
- **同构的对称约束**：`REMOTE` 停留语义对两平台同样适用（#2150 裁决 B）——run 无
  `merge_result_xls` ⇒ `REMOTE` 是事实终态；展锐 run 在 GT 工具链未配置或无输入时
  同样如此，不算卡住。
- #463 2026-09-14 评论的附带要求（展锐专属告警）已由 **#2394** 满足（两条规则 +
  promtool 场景）。
...
- [x] 归档端到端（Z2582 真机）：#1998 P1 上送端到端验收达成（2026-09-18：`REMOTE` 660 / `ARCHIVED` 25，推进 7–22s）
- [ ] Z258 真机：Watcher 冒烟 / 实时性（inotifyd 评估，需真机）—— 跟踪 **#1998**「P2 实时性」
- [x] #463 关闭路径：归档语义裁决完成（§D10，v1.0，落地记录见 Agent Note `2026-09-19-adr0032-d10-archive-semantics-parity-463.md`）；#73 已 CLOSED（采集并入 D4/D8）
...
- **列名差异的长期风险**：工具当前对 `ExpType`/`DeviceCount` → `ExpType␠`/`DeviceId` 做了
  ...
  **收口进度（2026-09-16）**：本仓库侧已由 #2307 收口——两处按名依赖取不到列时各记
  WARNING（带**实际读到的表头**，命名见「merge 产物表头契约」），表头契约、容忍面、
  盲区与两处**不阻断**面登记在
  [`2026-scan-upload-merge-contract.md`](../design/2026-scan-upload-merge-contract.md)，
  另有缺列/改名两类漂移单测钉住降级行为。#2256 已就此收口关闭、不再作活载体；
  本项作为**长期风险**保留，触发时以契约文档登记的「输出表头比对」验收项执行。
  跟踪：**#2256**（已收口；本项无需活载体——触发条件未到时不产生待办）。
- **Z258 真机验收**（B3 之外）：Watcher 冒烟 + 归档端到端仍未执行，保持原状。
  跟踪：**#1998**（P1 上送端到端验收 / P2 归档语义）+ **#463**（归档语义裁决）。
  **收口进度（2026-09-19）**：归档端到端已由 #1998 P1 验收达成（2026-09-18 实测，
  `REMOTE` 660 / `ARCHIVED` 25）；归档语义裁决已入 §D10（v1.0，#463 关闭）。
  残余 = **Watcher 冒烟 / 实时性**（inotifyd 评估，需真机），跟踪收窄为
  **#1998**「P2 实时性」。
