...
### D1 — Dispatcher 缺失元数据时 Fail-fast

**位置**：`backend/services/plan_dispatcher_sync.py` + `backend/services/plan_dispatcher.py`（async 对偶）

**改动**：

1. `_fetch_script_metadata` 返回前**对齐 keys 集合**——若实际查到的 `(name, version)` 集合 ≠ 输入 PlanStep 的 keys 集合，立即 `raise PlanDispatchError` 并附带 `missing` 列表。
2. `_build_lifecycle_from_steps` 把 `script_defaults.get(..., {})` 改为下标 `script_defaults[key]`——keys 完整性已由步骤 1 保证，缺失即 `KeyError`，符合 fail-fast。
3. `prepare_plan_run` / `dispatch_plan_sync` / `complete_plan_run_dispatch` 三处入口在 `PlanDispatchError` 时回写 `plan_run.result_summary = {"dispatch_failed": True, "missing_scripts": [...]}` + `status='FAILED'` + audit log，前端 PlanRun 列表能看到。
4. 同一份逻辑在 `prepare_plan_run` 的早期阶段就执行（避免先建 PlanRun 行再失败）：把 `_fetch_script_metadata` 的 keys 校验提到 `validate_pipeline_def` 之前。
...
3. `DeviceDetailDrawer.tsx:111-125` `KvList`：「当前步骤」行扩展为「`step_key`（`script_name` v`script_version`）」并做 monospace；新增一行「脚本路径」展示从 `plan_snapshot.steps[i].nfs_path` 派生（如 D4 的 snapshot drawer 已合入则跳到该 drawer）。
4. `PipelineStepInspector.tsx`（编辑态）的「在脚本管理中编辑参数」链接做 deep-link 加 `version` query：`/scripts?name=X&version=v` —— 配合 D7 反查端点跳转。
...
- **plan_snapshot 体积**：D4 暴露 plan_snapshot 后，前端 query payload 变大（每个步骤约 500 字节）；PlanRunDetailPage 改用 lazy fetch，默认不带 plan_snapshot，Drawer 打开时才单独 GET。
