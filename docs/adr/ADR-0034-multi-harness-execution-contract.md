...
| Antigravity CLI（agy 1.1.26） | ❌ 实测（2026-09-07）：根/嵌套 `AGENTS.md`、`CLAUDE.md`（含 symlink）、`GEMINI.md` 均不自动加载，引文诊断确认上下文仅 Gemini 系统指令 + USER_SETTINGS——**无仓库规则自动发现**；供给=调用方前置（`tools/dev/agy_with_rules.sh`） | `agy -p` 非交互可用；`agy agents` 空、无全局配置目录 |
