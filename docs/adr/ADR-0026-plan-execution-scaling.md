...
- 状态：Accepted
- 优先级：P0
- 目标里程碑：M5（P0–P2 已收口；P3 → M6 / ADR-0027）
...
② 开启短 admission transaction：
     ...
     - 按 host_id、device_id 固定顺序（升序）锁定 Host / Device 行
     - 在这些锁下按 device_id 固定顺序（升序）对 plan_run_target_device 清单做最终资源复查：
...
详见 **[ADR-0027](./ADR-0027-control-plane-horizontal-scaling.md)**（Accepted；生产多实例仍 opt-in）。

1. **准入 pump 的 leader election**——✅ P3-1：`hold_scheduler_leadership`（Postgres advisory lock）已接线 `admission_pump` / `counter_reconcile`；单实例行为不变，多实例误部署至多一 leader。
2. **SocketIO Redis adapter / Centrifugo**——✅ P3-2：`STP_SOCKETIO_REDIS_ADAPTER` opt-in + `AsyncRedisManager`（见 ADR-0027）。
3. **控制面多实例**——✅ P3-3：全量 singleton APScheduler leadership + Agent sid registry + room RPC（去 sticky）；生产仍 opt-in。
...
| 项 | v1 默认 | 依据 | 灰度仍需确认 |
|----|---------|------|--------------|
| pump 扫描间隔 | `STP_ADMISSION_PUMP_INTERVAL_SECONDS=5` | ≤5s 近瞬时「点了就跑」；∈[1,30] | 高排队深度下 CPU |
| pump batch | `STP_ADMISSION_PUMP_BATCH=5` | 与 permit 同量级，避免单拍打满 | 多 host 物化耗时 |
| 竞争回队退避 | `STP_ADMISSION_RETRY_BACKOFF_SECONDS=30` | ≥ pump interval，防自旋 | DEVICE_BUSY 风暴 |
| aging 步长 / 上限 | `1800s` / boost `5` | 30min +1；满额后 prio0≡静态5 | 多租户公平性 |
| OperationScheduler permit | `STP_MAX_CONCURRENT_OPERATIONS=5` | 仿真：20 设备 / 30s step 下 mean wait ≪ coordinator timeout | patrol 唤醒风暴实机 |
| 续租分块 / 上限 | chunk `100` / max `200` | P0 现值；payload 与隔离成本折中 | 单 host 200+ job |
| Coordinator 心跳间隔 | `COORDINATOR_HEARTBEAT_INTERVAL=30` | Agent `_tick` 默认 | — |
| Coordinator 超时 | `COORDINATOR_HEARTBEAT_TIMEOUT_SECONDS=300` | ≥3×interval（不变量） | 可下调至 180 需灰度 |
...
| 目标里程碑 | **M5 = ADR-0026 P0–P2 完成**；P3 → **M6 / ADR-0027** | 本文 + ADR-0027 | — |
...
| 2026-07-20 | 三信号时钟纪律收口：extend-batch / coordinator-heartbeat 按子状态写锚点；已知 state 不再双写 `updated_at`。准入/多实例灰度 Runbook：`docs/operations/adr-0026-admission-and-scale-gray-rollout.md`；`/health` 暴露 `admission_queue_*`。 |
| 2026-07-20 | 待定清单 v1 回填（`adr0026_params` 不变量 + permit 仿真）；P3-1 移交 [ADR-0027](./ADR-0027-control-plane-horizontal-scaling.md) leader election。 |
| 2026-07-20 | P3-2：SocketIO `AsyncRedisManager` opt-in（`STP_SOCKETIO_REDIS_ADAPTER`）；见 ADR-0027。 |
