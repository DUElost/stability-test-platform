...
- `plan_run_target_device`：唯一 `(plan_run_id, device_id)`；辅助 `(device_id)`（审计：一台设备参与过哪些排队/运行中计划）、`(plan_run_host_id)`。
...
| **aggregator** | `on_job_terminal` 全量加载兄弟 job（`aggregator.py:46-49`）；FAILED/ABORTED 分开统计（`plan_run_aggregation.py:43-44`） | 改 O(1) 五计数器 + 单一 terminalization 入口（§6）；`terminal == total` 才触发 `apply_plan_run_aggregation` + `trigger_next_plan`（`:52-53`） |
...
| **SAQ** | `_queue` 与 worker 耦合于 `start_saq_worker`（`saq_worker.py:99`） | P0 待办：拆 producer/worker（`init_saq_producer` / `run_saq_worker`），为 pump 入队和后续水平扩展铺路 |
...
4. **SAQ producer/worker 拆分**——`init_saq_producer` / `run_saq_worker` 解耦（`saq_worker.py:99-118`），使 `STP_ENABLE_INPROCESS_SAQ=0` 不再瘫痪生产端。
5. **心跳减负**——Agent 心跳收敛到带 backpressure 的 `/api/v1/agent/heartbeat`（`agent_api.py:702`），闭环 backpressure；增量/降采样。
6. **真实指标**——补齐 queue-latency、续租成功率、聚合耗时、per-host 并发度等 Prometheus 指标（对齐 ADR-0011），为后续压测提供判据。
...
3. **Host OperationScheduler**——Agent 侧 per-host 单例 permit 信号量（默认 5，待压测确认），解耦长跑设备数与瞬时执行并发（缺口②、不变量②）；`admission_batch_size_snapshot` 仅审计。
4. **execution_state 矩阵**——子状态列 + 按态选时钟的 recycler 改造（§3）；三信号拆列落库（不变量③）。

### P2：控制面规模优化

1. **O(1) 聚合计数器 + terminalization 服务**（§6）——`plan_run` / `plan_run_host` 五计数器列 + 单一终态入口 + 对账 sweep。
2. **实时日志批量化 + 背压**——重启 `step_log` 通路前必须先做（记忆核实项），否则空转变风暴。
3. **索引与查询优化**——QUEUED 出队复合索引、`plan_run_target_device` 集合查询、设备矩阵聚合端点（对齐 ADR-0022 C5a₂）在 1000 device 下的执行计划复核。
...
- `PlanRunHost.status` 枚举值收敛、terminalization 服务签名与对账 sweep 周期——**待落地细化**。
