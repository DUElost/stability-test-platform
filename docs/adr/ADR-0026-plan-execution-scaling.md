...
详见 **[ADR-0027](./ADR-0027-control-plane-horizontal-scaling.md)**（Proposed）。
...
2. **SocketIO Redis adapter / Centrifugo**——✅ P3-2：`STP_SOCKETIO_REDIS_ADAPTER` opt-in + `AsyncRedisManager`；Agent RPC 仍需 sticky（见 ADR-0027）。
3. **控制面多实例**——⏳ P3-3，需先解除 ADR-0002/0018 单进程约束，覆盖全部 APScheduler job。
