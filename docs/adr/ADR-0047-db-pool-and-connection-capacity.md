...
- 状态：**Accepted** v1.3（2026-09-25 裁决 D3/D4，D1–D6 全部有结论；v1.2 同日 §4 校准回填，维持首轮值；v1.1 于 2026-09-23 owner 裁决 D1/D2/D5/D6）
- 优先级：P1（它已经造成过一次控制面不可用并被踢回登录，且当时配置在算术上不可能同时成立）
- 目标里程碑：M7
- 日期：2026-09-18（起草）；2026-09-23（裁决）；2026-09-25（v1.2 校准回填）
- 决策者：owner（2026-09-23）；D3/D4 由 owner 授权 Claude 裁决（2026-09-25）；起草：平台研发组
...
- 版本记录：v1.0（2026-09-18）首次提出，D1–D6 全部开放；v1.1（2026-09-23）owner 裁决 D1/D2/D5/D6，D3/D4 保留开放，落地 pgbouncer 之外的首轮参数与启动门禁；v1.2（2026-09-25）**§4 校准回填**（模拟 #3243 + 生产真机 run 556）：`20/20`、`reserve=8`、`2s` **维持**；§5 标注门禁/告警生效状态（门禁启动实测待下次重启）；defer 的终态聚合项改指 ADR-0052；v1.3（2026-09-25）**裁决 D3「不合并」、D4「不引入 pgbouncer」**，由「保留开放」转为带复评触发器的否决，不改任何参数与代码
...
### D3（两份池是否合并）— **v1.3 裁决：不合并**（2026-09-25）

sync/async 双池的合一是**代码结构决策**（84+ 处同步调用点与异步侧混用是既成事实），
与容量裁决互相独立；D1 的不变量已覆盖「两份池加起来」的总量，不依赖是否合并。

裁决依据（v1.3）：
- 合并的唯一收益是省掉一份池的预留量，而 §4 真机峰值 async **4** / sync **11**（各预算 40）说明
  当前瓶颈不在总量；D1 门禁已把「两份池之和 ≤ 可用连接」钉死，不合并也不会越界；
- 代价是把 84+ 处同步调用点改写为异步（或反之），这是代码结构迁移，不应以容量为名发起；
- 双池并存的终态出口不是「合并」而是**同步调用点自然归零**：若将来同步侧迁完，sync 池随之退役，
  不需要一个合并步骤。

复评触发器（任一）：ADR-0027 多实例推进后，`n_instances × (sync+async 上限) + reserve` 在按 §4
实测峰值**下调单池上限之后**仍超过 PG 可用连接；或同步调用点迁移已成独立立项、需要确定 sync 池退役路径。

### D4（是否引入连接池代理）— **v1.3 裁决：不引入 pgbouncer**（2026-09-25）
...
v1.3 补充：单实例下应用合计 80 < 可用 97、真机峰值远低于预算，引入新生产依赖没有要解决的问题。
复评条件（任一）：ADR-0027 多实例推进；或「三条池序列的语义改写方案」与实现同 PR 就绪。
复评时须先给出会话级 advisory lock 在所选 pooling 模式下的归属证明，否则不进入选型。
