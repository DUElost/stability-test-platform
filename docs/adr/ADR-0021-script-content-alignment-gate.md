...
- 派发未对齐属**调度层**故障，不能与设备级失败混算（原句依据的 run 级失败阈值 `failure_threshold` 已由 **ADR-0048**（2026-09-18；v1.1 2026-09-20 恢复三态产出与前端派生通过率展示）移除，该字段不参与任何判定）；
...
> ⚠️ **2026-06-12 勘误**：上述两行声明与代码实现不符。实际运行时路径为：
> `pipeline_engine._run_script_action()` → `ScriptRegistry.resolve(name, version)` → 从 Server API 缓存取 `entry.nfs_path` → subprocess。
> Agent **不经 plan_snapshot** 解析脚本路径。`plan_snapshot.steps[].nfs_path` 写入后仅在审计/回溯场景消费，运行时不参与路径解析。
> 此设计是有意为之：ScriptRegistry 提供进程级单例映射，保证同 `(name, version)` 在并发 Job 间始终解析到同一文件，避免 per-Job 不同路径导致的工程复杂度。

**实际运行时锁定由以下三重机制覆盖，而非 plan_snapshot 直接锁定**：

1. **派发门禁 verify**（D1）：PlanRun 启动前校验 Agent 磁盘 sha256 与 DB 一致
2. **热更新软禁**（D8）：运行中 Job 存在时禁止 host hot-update，消除"中途漂移"的主要来源
3. **Script 不可变约束**：已存在版本的 `default_params` 不允许修改（`scripts.py:214-218` 422 拒绝）

- ~~运行中即使 host 被热更新，已下发到 Agent 的 Job 仍按 plan_snapshot 路径运行（pipeline_engine 已绑定 subprocess）~~ → 运行中热更新被 D8 阻止，此场景不会发生
