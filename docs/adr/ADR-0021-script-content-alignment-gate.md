...
将 `POST /api/v1/plans/{id}/run` 由"同步派发"改为"异步入队 + 立即返回 PlanRun 行"：
...
| **C5** | 前端 `PlanRunDetailPage` 重构：派发门禁卡片（hosts × scripts 矩阵 + sync 进度） | Vitest |
| **C6** | 前端 host 管理热更新按钮 disabled + 二次确认对话框（列出待 abort Jobs） | Vitest |

后端 C1~C4 串行 commit 全部完成后再做 C5 / C6。
