...
- 状态：**Accepted**（v2.5：派生归属重设计。2026-08-19 决策者拍板；历经 v1 评审 → v2 决策转向 → v2.1 → v2.2 → 定性纠正 → v2.4 产品面纠正，见修订记录）
- 优先级：P1
- 目标里程碑：M7
- 日期：2026-08-18
- 决策者：平台研发组
...
| 2026-08-19 | v2.3.1（状态传播补齐） | 修正漏损两处：reviews §6 落地表仍写旧定性「既有鉴权洞」（G13 行已改、阶段表漏）；adr README 条目卡在 Proposed（状态翻转后过期）。教训：**同一事实有七个挂靠位**，改任何一处必须逐一核对——本行之后每次修订先对传播清单 |
| 2026-08-20 | v2.4（产品面纠正） | P1 六个 key（`HONOR-MLD` / `HONOR-ELA` / `ZTE-Z258` / `ODM-DAM` / `TRANSSION-X110` / `LEGACY`）是 M-b/M-c **回填种子**（FK 完整性 + NULL 归零），**不是**登记簿产品目录。`/projects` 默认只列 `source=USER`；型号→项目须 `match_models` 精确填写，禁止前缀推断（M-c）。D2/D3 判据不变，**不复活 D5**。 |
| 2026-08-31 | v2.5（派生归属重设计） | 派生归属定案：`device.project_id` = `rule(device.model)` 的**缓存副本**，project 归属回归登记簿 + 派生视图——map/apply 只写成员行、不写设备列（D10 M3）；M1–M4 已落地（详见 §v2.5） |

**传播清单（2026-08-19 定，改任何一处先对一遍全部七处）**：
ADR 头部状态行 / ADR 正文（修订记录即本文）/ reviews 对应节 / reviews §6 落地表 /
adr README 清单行 / adr README 里程碑看板行 / CLAUDE.md 决策表行。
历史漏损：v2.2→v2.3 漏 adr README（状态没翻），v2.3 内部漏 reviews §6（定性没同步）。
...
**D3 是「何时应创建项目」的人工判据，不是把 P1 种子灌进 `/projects`。**
`HONOR-MLD` 等六个 key 留在库里只为 FK / 回填完成标准；工作台默认只展示
`source=USER` 的登记簿行。型号归属通过 `match_models` 精确填写（禁止前缀推断）。
...
> **v2.1 注**：原文「迁移期与公共池的既有行为」中的**「公共池」概念已废止**（v2.1，
> 见 D2：不存在公共池域，未识别设备归 LEGACY），复议本决策时不适用。
...
> 滞后维护 + 脚本路由已承担设备差异）挂起，不建。触发复议：脚本路由的 fail-fast 频繁
> 命中「人为选错脚本」（映射正确但 Plan 配错 `script:<name>`），需要编辑器过滤辅助时。
...
**落地与修订（2026-08-31）**：facet 减列 / 格式 validator / 详情页徽标 / KPI
已随 M4 落地并部署。`customer` 字典表按「**数据源、列不动**」落地（新增
`customer` 表，seed 从 `test_project.customer` 去重回填；编辑框改原生
datalist 下拉建议，列保持自由文本不约束历史值；API `GET /projects/customers`）。
**存在性探测正式放弃**：控制面无 JIRA API 访问通道（提单是厂商脚本在 Agent
侧执行，控制面只解析 stdout 提取 issue key），无地址/凭据可探测；详情页格式
校验已构成「硬需求的最强输入校验」的现实现。
