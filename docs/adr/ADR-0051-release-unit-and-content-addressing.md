...
- 状态：**Accepted** v1.8（2026-09-26 落地状态刷新：D6 闭合、D7 三片合入、D8 一项；决策不变。历史修订见下表）
...
- 落地状态（2026-09-26）：**未完结**。Phase 0、2a、2b、3 已落地；Phase 1 本机 bundle 运行根已切换，**D6 已闭合**——生产 env 真身落站点 `stp-releases/env.backend`、rev 根树内 symlink，运行时不再触开发检出（2026-09-25 实切）；独立站点 install/回滚可重放属多站点续集（ADR-0041）。Phase 4：展锐三族、控制面摘要面、manifest 显式退役已落地；D7 三片已合入——绑定机制（v1.7，脚本包 `requires_tools`）、flashtool/aimonkey `kind=tool` 登记并**发布到站点**、四个消费族新版本（flash_firmware 1.3.18 / flash_preflight 1.0.6 / monkey_resource_push 1.1.1 / monkey_test 1.2.3）已登记并发布（站点真实数据端到端核验：拉包-预热-注入-执行位均通过）；**激活**（换 rev 后 scan、带新 Agent 载荷的 fleet 推送、canary 真机取证、计划重指）与第 4 片（撤 `STP_FLASH_TOOL_DIR` 注入与 host-resources 通道；删键下发依赖 #3356）未完；控制面 dedup 工具包化与 legacy 路径键/回退退出未完。Phase 5：族级 `kind`、过渡登记簿、`allow_deactivate` 移除、`check_new_script_family` 退役（D0 归类由 kind 承担）已落地；`check-deploy-source.sh` 结构替代（排在 D7 之后）与新站 `default_params` 数据验收（#3289）未完。fleet 包模式：DB 与 Prometheus 两出口均 `package=48, tree=mixed=unknown=0`（2026-09-26 09:30 全量 sweep）。证据见修订记录与 [生产验收记录](../notes/process/2026-09-25-adr0051-production-acceptance.md)。
