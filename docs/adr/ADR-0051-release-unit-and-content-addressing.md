...
- 双轨：按 script 行灰度（`package_sha256` 为空 → 旧路径；非空 → 包路径），控制面零改动（沿 ADR-0033 v1.1「契约翻译在 Agent 边缘」）。
- `agent-code` artifact **排除 `scripts/`**：热更新载荷从 128.7k 行降为 Agent 代码本身；脚本按需拉包。
...
**Phase 3 不必等 2b**：删目录只依赖「包已登记且字节等价」，与执行路径切换解耦。
