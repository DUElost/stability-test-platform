...
- 状态：**Accepted** v1.9（2026-09-26 D7 激活完成 + 第 4 片 a/b；决策不变。历史修订见下表）
...
- 落地状态（2026-09-26）：**未完结**。Phase 0、2a、2b、3 已落地；Phase 1 本机 bundle 运行根已切换，**D6 已闭合**——生产 env 真身落站点 `stp-releases/env.backend`、rev 根树内 symlink，运行时不再触开发检出（2026-09-25 实切）；独立站点 install/回滚可重放属多站点续集（ADR-0041）。Phase 4：展锐三族、控制面摘要面、manifest 显式退役已落地；D7 三片已合入——绑定机制（v1.7，脚本包 `requires_tools`）、flashtool/aimonkey `kind=tool` 登记并**发布到站点**、四个消费族新版本（flash_firmware 1.3.18 / flash_preflight 1.0.6 / monkey_resource_push 1.1.1 / monkey_test 1.2.3）已登记并发布（站点真实数据端到端核验：拉包-预热-注入-执行位均通过）；**已激活**（2026-09-26：scan 建行、fleet 48/48 新 Agent 载荷、canary 真机刷机 run 575 + Monkey run 576 均经 tools_cache 取工具、15 个在用计划重指、48/48 主机预热；18:44 全机队回归 `monkey_resource_push 1.1.1` 476 次成功、失败与历史基线同量级）；第 4 片：a（撤 `STP_FLASH_TOOL_DIR` 渲染，台账结项）+ b（四族 10 个旧版本 retired）已提，c（host-resources 通道）未完；控制面 dedup 工具包化与 legacy 路径键/回退退出未完。Phase 5：族级 `kind`、过渡登记簿、`allow_deactivate` 移除、`check_new_script_family` 退役（D0 归类由 kind 承担）已落地；`check-deploy-source.sh` 结构替代（排在 D7 之后）与新站 `default_params` 数据验收（#3289）未完。fleet 包模式：DB 与 Prometheus 两出口均 `package=48, tree=mixed=unknown=0`（2026-09-26 09:30 全量 sweep）。证据见修订记录与 [生产验收记录](../notes/process/2026-09-25-adr0051-production-acceptance.md)。
...
| 版本 | 日期 | 变更 |
|------|------|------|
| v1.9 | 2026-09-26 | **D7 激活 + 第 4 片 a/b（非决策变更）**：激活——canary 设备 867 刷机 run 575（`argv0=tools_cache/flashtool/1.2444.00.100/flash_tool`，3.6G 真刷）与 Monkey run 576（15/15 巡检）均经 `required_tool_injected` 取工具；owner 确认后 15 个在用计划重指（审计 `changed=['steps']`，链不变，plan 68 删 `flash_tool_dir`）；48/48 主机 tools_cache 预热；18:44 全机队回归 `monkey_resource_push 1.1.1` 476 次成功。第 4 片 a：撤 `STP_FLASH_TOOL_DIR` 的 hot-update 渲染与路径核验登记，台账 `flash-tool-dir-env-injection` 结项（该键仍是步骤级注入键，不属 §6「无读取点」语义，存量主机旧行惰性）；b：四消费族 10 个旧版本 `retired:true`（换 rev 后 scan 停用；只读推演与启用步骤交集为空，唯 plan 13 [废弃] 的禁用步骤引用 1.3.15/1.0.4）。c（host-resources 通道下线）另片。**同日补记（#3417 正文措辞修订，决策不变）**：① D2「存量 208 个版本目录」改为不带数字的表述并补计数口径（2026-09-22 基线：manifest 全集 36 族 / 211 条目；平台脚本子集 35 族 / 210 个 `v*` 目录；等价证明子集 208），报数须注明口径与时点（#3205）；② §5 Phase 2b 行把「`agent-code` 排除 `scripts/`」移出 2b 范围、注明归 Phase 3，与 v1.1 勘误一致。 |
| v1.8 | 2026-09-26 | **落地状态刷新（非决策变更）**：纠正 v1.6 起遗留的失实状态——「落地状态」行与 Phase 1 表格行仍称「生产 `.env.backend` 仍指开发检出」，D6 已于 2026-09-25 实切（v1.6 当时只加了修订行、漏改这两处）；D7 三片合入（#3365 机制 / #3369 工具登记与发布守卫 / #3378 消费族新版本）且工具包与四个新脚本包已发布到站点、经站点真实数据端到端核验；`check_script_packages --publish` 改为原子落位 + 只增不改（#3379）；D8 `check_new_script_family` 退役（#3371，ADR-0033 v1.14）；Phase 2b 表格补 ✅；fleet 包模式 Prometheus 出口首个真值 `package=48`。 |
...
| **4** 部分 ✅ | 展锐三族、控制面摘要面与 manifest retired 策略已落地；D7 绑定机制、flashtool/aimonkey 登记发布、四消费族新版本已合入并**激活**（canary 真机 + 15 计划重指 + 全机队预热），第 4 片 a/b（撤 `STP_FLASH_TOOL_DIR` 渲染 + 旧版本 retired）已提；第 4 片 c（host-resources 通道）、控制面 dedup 工具包化及 legacy 路径键/回退出口未完 | Phase 1（清单形态）+ ADR-0042 D3 修订 | #737 清单门禁 + `.env*.example` 奇偶；digest 契约测试 |
