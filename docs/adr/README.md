...
| [ADR-0042](./ADR-0042-settings-convergence-and-bare-read-boundary.md) | 配置读取收敛——分域 pydantic-settings 与裸读取边界（#737 deferred） | **Proposed** | P1 | M7 | v0.1 初版（2026-09-14，由 #737 收口 deferred 触发）：D1 引入 pydantic-settings 分域 Settings；D2 迁移判据（≥3 旋钮 / 默认值-类型转换重复 ≥2 处 / 需跨字段校验），不满足保持裸读；D3 env 名与别名不变（validation_alias，运维零感知）；D4 惰性 get_settings() + Agent reload_settings()；D5 排除注入型/协议键与 scripts 目录；D6 env_inventory 门禁解析 Settings 字段（防可见性回退）；D7 分阶段（P1 试点 1 控制面域 + 1 agent 域） |
