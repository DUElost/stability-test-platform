...
| [ADR-0053](./ADR-0053-center-storage-event-dedup.md) | 中心存储事件去重——内容对象、事件引用与 baseline 复用（#3230 / #3233） | **Proposed** | P1 | M7 | v0.2：2026-09-25 代码对证与裁决建议。修正全量上送前提；建议文件 CAS + manifest + DLE 引用，硬链接仅作优化；baseline 复用依赖强身份与 scan 输入完整；派生物隔离，发布/GC/可观测为首版条件。待 owner 接受，尚未实施 |
...
> `Proposed` 为准。**Proposed（未决）：ADR-0052**（2026-09-24 起草：终态事实与父 Run 聚合解耦；转 Accepted 的门槛见其 §5；ADR-0047 已于 2026-09-23 转 Accepted）、**ADR-0053**（v0.2：2026-09-25 完成内容对象与引用分离的裁决建议，待 owner 接受）。
...
| M7 | 进行中 | **Superseded**（2026-09-22 由 ADR-0051 取代）：ADR-0039、ADR-0046。<br>**Accepted**：ADR-0029（M1–M4 已落地）、0030、0031、0032、0033（v1.13；§5.4 条件 4 多站点已触发；部分落地；后续切片并入 ADR-0051 Phase 4）、0034、0036、0037、0038（v0.3 草案待裁）、0040、0042、0043（实施已落地）、0044、0045（词表已落地 `adf9c24a`）、**ADR-0047**（v1.1：容量不变量进启动门禁 + 503 过载语义）、0048、0049、0050、**0051**（v1.0；发布单元与内容寻址，Phase 0 已落地，Phase 1–5 待排期）。<br>**Proposed**：**0052**（v0.1：终态事实与父 Run 聚合解耦；待部署窗真机复跑后裁决，见其 §5）、**0053**（v0.2：建议文件 CAS + 事件清单 + 观察引用；待 owner 接受方向，实施实证与架构接受分离）。 |
