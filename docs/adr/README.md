...
| [ADR-0033](./ADR-0033-tool-kit-ecosystem-integration.md) | 外部工具统一接入契约规范与包管理解耦模型（#745） | Accepted | P1 | M7 | v1.5：Phase 2 **选项 A**——B5 薄 `DedupMergeEngine` 包现态 `start_log_scan -merge_files_list`（GT 仍只做 B2；纠正「GT=merge 引擎」措辞；见 notes `2026-09-19-adr0033-phase2-option-a`）；v1.4 阻塞登记；v1.3 D1 flash（#2546）；v1.2 收窄；**落地状态：部分落地**（B5 样板；包存储/Phase 3 未做）；#745/#2546 |
...
| M7 | 进行中 | ADR-0029（项目分类域·登记簿；v2.5 派生归属 M1–M4 **已落地**）；ADR-0030（**Accepted** v1.9：P0 ✅ / P1 ✅ / D6 ✅ / **P2 核心 ✅** #429）；ADR-0031（**Accepted**：阶段二全栈 ✅，2026-08-28）；ADR-0032（**Accepted** v1.1：展锐 Watcher+归档；平台路由收口 R1–R4 + DLE 终态语义两平台同构（§D10，#463）+ **D8 增补 inotifyd 实时唤醒层**（opt-in 默认关，#1998）；v0.8：**B3 spike 已执行**，#463/#73）；ADR-0033（**Accepted** v1.5：Phase 2 选项 A·B5 薄适配器部分落地；v1.4 阻塞登记；v1.3 D1 flash；v1.2 收窄；D0/D3 即刻生效、包存储条件落地；#745）；ADR-0034（**Accepted** v1.12：多 Harness 执行契约，#855/#857）；ADR-0043（**Accepted** v1.1：中止宽限的请求主体同构，#2050/#1928/#1880；**实施已落地** 2026-09-15 [PR #2165](https://github.com/DUElost/stability-test-platform/pull/2165)，[#2154](https://github.com/DUElost/stability-test-platform/issues/2154)；v1.1 D3 判据裁决）；ADR-0037（**Accepted** v0.5：Agent 主机提权边界——wrapper 单入口与窄面 sudoers 已全队落地，legacy 分支/哨兵与宽文件退役完成（#2133/#2134/#2180）；R02 评审一稿 #2206） |
