...
| [ADR-0030](./ADR-0030-multi-case-suite-management.md) | 多用例平台化管理（test_suite / test_case + 外部管理面） | Proposed | P1 | M7 | D1 用例集/用例为配置层实体（粒度=testpoint，不进调度模型）；D2 文件↔库双向（import/export/validate，runtask.xml 变生成物）；D3 项目分化（project_id 可空=通用套件 / 必填=项目套件 + apk_binding，相机 MTBF）；D4 外部管理面（REST 8000 + OpenAPI 真源 + CLI，不新增端口，全量审计）；D5 快照留痕（清单 sha256）；D6 P0→P1→P2 分阶段，暂不实施。背景分析见 [reviews](../reviews/MTBF_MULTI_CASE_RESEARCH_2026-08-19.md) |
...
| M7 | 待定 | ADR-0029（项目分类域·登记簿；D1/D4/D5/D7/D8/D9 挂起，落地 P1–P3 最小形态）；ADR-0030（多用例平台化管理，Proposed，暂不实施）——0030 **依赖** 0029 的 `test_project` / `project_id`，但**不复活** 0029 挂起决策（test_suite 是 0029 非目标 ExecutionProfile 实体族的例外子集，见 0030「与 ADR-0029 的关系」） |
