...
| [ADR-0034](./ADR-0034-multi-harness-execution-contract.md) | 多 Harness 并行执行契约与执行登记（#855 / #857） | Proposed | P1 | M7 | v0.5：选择权原则（Harness 由开发者决定）/ 三维状态模型 lifecycle×liveness×integration（ADR 实现选择，非冻结条款）/ Registry=visibility-only 非调度器 / `--path-format=absolute` 唯一发现方式 / effective scope=declared∪derived(diff) 并集恒成立+drift 提示 / overlap 真值表（开放 PR 恒在风险窗口）/ P0 建 `execution-contract.md` 唯一权威源 / drift gate 非 merge queue / G2 真身+薄壳（symlink 优先）。两轮八源多 Harness 评审综合见 [reviews](../reviews/REVIEW_ADR0034_MULTI_HARNESS_2026-09-06_synthesis.md) |
...
| M7 | 进行中 | ADR-0029（项目分类域·登记簿；v2.5 派生归属 M1–M4 **已落地**）；ADR-0030（**Accepted** v1.9：P0 ✅ / P1 ✅ / D6 ✅ / **P2 核心 ✅** #429）；ADR-0031（**Accepted**：阶段二全栈 ✅，2026-08-28）；ADR-0032（**Accepted** v0.6：展锐 Watcher+归档，#463/#73）；ADR-0033（**Accepted** v1.1：外部工具接入契约与包管理解耦，#745）；ADR-0034（**Proposed** v0.5：多 Harness 执行契约，#855/#857） |
