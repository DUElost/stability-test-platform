...
| [ADR-0035](./ADR-0035-agent-secret-host-boundary.md) | Agent 主机级凭据边界——共享 AGENT_SECRET 风险接受与升级触发（#906） | Accepted | P1 | M7 | v1.0：接受全局共享 AGENT_SECRET 为受信域内主机级认证原语；成文泄露后冒充面（fencing/SID CAS/审计）+ 升级为主机级凭据的触发条件与设计骨架（Host 凭据列+校验收敛+per-host 分发+enrollment token+ops token 分界）（2026-09-08） |
...
| M7 | 进行中 | ADR-0029（项目分类域·登记簿；v2.5 派生归属 M1–M4 **已落地**）；ADR-0030（**Accepted** v1.9：P0 ✅ / P1 ✅ / D6 ✅ / **P2 核心 ✅** #429）；ADR-0031（**Accepted**：阶段二全栈 ✅，2026-08-28）；ADR-0032（**Accepted** v0.7：展锐 Watcher+归档，#463/#73）；ADR-0033（**Accepted** v1.1：外部工具接入契约与包管理解耦，#745）；ADR-0034（**Accepted** v1.9：多 Harness 执行契约，#855/#857）；ADR-0035（**Accepted** v1.0：Agent 主机级凭据边界，#906） |
