...
| [ADR-0030](./ADR-0030-multi-case-suite-management.md) | 多用例平台化管理（test_suite / test_case + 外部管理面） | Accepted | P0+P1 | M7 | v1.6：P0 验收✅ + **P1 全部✅**（#404：P1a 实体/管理面、P1b 绑定门禁+env 双层退役、P1c CLI `tools/dev/mtbf-cases.py` + 文档收口）。**未做**：真机冒烟（D6 总验收信号）、P2 前端与 `test_case_result`。背景：[reviews](../reviews/MTBF_MULTI_CASE_RESEARCH_2026-08-19.md) |
...
| M7 | 进行中 | ADR-0029（项目分类域·登记簿；D1/D4/D5/D7/D8/D9 挂起，落地 P1–P3 最小形态）；ADR-0030（**Accepted**：P0 ✅ / P1 ✅ #404 / 真机冒烟与 P2 待做；0030 依赖 0029 `test_project`，不复活 0029 挂起决策） |
