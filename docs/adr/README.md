...
| [ADR-0035](./ADR-0035-agent-host-identity.md) | Agent 主机身份与凭据体系（R02-R01/#906） | **Accepted** v1.0 | P1 | — | 草案：全局 AGENT_SECRET + 客户端自报 host_id 的冒充面分析；四方案对比（每主机凭据/mTLS/注册质询/接受边界）；推荐「A 每主机凭据为目标 + C 注册质询为引入路径」；决策请求见 §6 |
