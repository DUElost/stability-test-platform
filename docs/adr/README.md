...
| [ADR-0033](./ADR-0033-tool-kit-ecosystem-integration.md) | 外部工具统一接入契约规范与包管理解耦模型（#745） | Accepted | P1 | M7 | v1.9：Phase A3 follow-ups DLE zip + extract_bundle + PlanRun Jira 历史（notes `2026-09-21-adr0033-phase-a3-followups`）；v1.8：PlanRunArtifact 下载+#3015；v1.7：Phase A D0/Contract/Jira ACL；v1.6：§5.4 包存储评估未触发；v1.5 B5；**部分落地**（B5 + Phase A + A3；包存储未触发；Phase 3 未做）；#745/#3013 |
