...
- **状态**：Accepted v1.0（2026-09-08 用户裁决采纳推荐方案：A 每主机凭据为目标形态 + C 注册质询为引入路径；实施单另行拆分，#906 留作实施跟踪）
- **优先级**：P1
- **日期**：2026-09-08
- **决策者**：平台研发组 / 架构组
...
2026-09-08 用户裁决：**采纳推荐——A（每主机独立凭据）为目标形态，
C（注册质询换发）为引入路径**；实施时点独立排期（不与 #46 HTTPS 强绑定，
B mTLS 保留为 #46 落地后的演进方向）。本 ADR 转 Accepted；实施单
（host 凭据哈希列、注册审批 UI、握手/API 校验改造、agentctl 引导流、轮换
runbook）另行拆分并回链本 ADR。
