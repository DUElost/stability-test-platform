...
- **版本记录**：
  **v1.0（2026-09-08 草案 → Accepted，用户裁决采纳「A 目标 + C 引入路径」，来源 PR #1163/#1170）**；
  **v1.1（2026-09-09 用户裁决）：并入原 PR #1147 的「当前状态风险接受 + 升级触发 + 主机级方案骨架」，正文按决策层级四段化，消除「当前是否立即实施」的歧义**；
  **v1.2（2026-09-10 用户裁决）：§6.1 把四条升级触发条件钉到检测来源与检出方——原四条全部依赖外部信号、无来源映射，等价于无人监视的触发条件**
- **优先级**：P1
