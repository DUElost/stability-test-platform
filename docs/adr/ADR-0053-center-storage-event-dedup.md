...
- **L0 抽样 dry-run（2026-09-25，逐文件全量 SHA-256）**：抽了 5 个已知有重复的 run（345 / 349 / 379 / 430 / 435，
  合计 268 GB），得到 1557 个内容相同的文件组、6442 个待链接文件，**可回收 214.1 GB（80%）**，耗时 37.5 分钟
  （最低 I/O 优先级）。样本是刻意挑的重复密集 run，80% 不能外推到全量，但它按内容而非目录名证实了重复的规模。
  全量执行的数字待回填。
