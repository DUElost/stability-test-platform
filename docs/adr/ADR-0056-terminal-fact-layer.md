...
- 状态：**Accepted** v1.0（2026-09-26 owner 授权 Claude 裁决：F1–F5 全部采起草取向，见 §8；实施未开始）
- 优先级：P1
- 目标里程碑：M7
- 日期：2026-09-26
- 决策者：owner（2026-09-26 授权 Claude 裁决）；起草：平台研发组
- 归属域：semantic-ownership terminal-fact-layer
...
- 版本记录：v1.0（2026-09-26）**裁决**：F1–F5 全部采起草取向，转 Accepted，见 §8；v0.1（2026-09-26）首次起草，Proposed
...
## 8. 裁决记录（2026-09-26，owner 授权 Claude 裁决）

| # | 裁决 | 依据 |
|---|---|---|
| F1 | **分表**：事实表对运行行只存引用，不建级联 FK | 在运行表上加「永不删除」标记会把运行层也拖成永久保留，库增长失控（#3327）；分表后两层的保留策略可以独立演进 |
| F2 | **平台侧保存厂商签名原文 + 归一化键**，归一化规则按厂商适配器实现（ADR-0033 D4 的适配器层） | 只存原文无法跨厂商、跨工具版本对齐；归一化放在适配器里，厂商格式变化只影响适配器 |
| F3 | **MTBF 分母取租约持有时长** | 租约是「设备在跑测试」最直接的持久事实，已有 `acquired_at` / 释放时间；心跳在线时长包含空闲，偏大；脚本上报时长依赖各族脚本改造，覆盖不全。其它口径作为后续可选切片 |
| F4 | **存量尽力回填**，并在事实行上标注来源为「回填」 | 生产保留期实际停用，历史运行行都在；不回填就要从上线日起重新积累。回填标注来源，避免与实时写入的事实混淆 |
| F5 | **缺事实行即跳过删除该 run 并告警** | 事实层的价值在于「不随 run 消失」；只告警不阻断会让清理任务在事实写入失败时照样删掉原始行，违背 D1 |

实施按 §5 分期推进，第 1 期（事实表 + 写路径）即可在 #3326 领单。
