...
| 页面 | 路由 | 导航位置 | 功能 |
|------|------|----------|------|
| 任务实例 | `/task-runs` | 侧边栏 - 任务区块 | 分页展示任务运行记录，支持状态筛选 |
| 问题追踪 | `/issue-tracker` | 侧边栏 - 任务区块 | 显示 JIRA Draft，支持刷新与查看详情 |
| 环境资源 | `/resources` | 侧边栏 - 资源区块 | WiFi 配置、存储工具管理等 |
| 修改密码 | `/account/password` | 侧边栏 - 账户区块 | 用户密码修改表单 |
