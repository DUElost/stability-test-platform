...
保持 Proposed，不代替 owner 声明已接受新的 CAS 方向，也不把文档评审当成实施完成。

推荐文件级内容对象 + 版本化事件清单 + 独立 DLE 观察引用；
...
本需求以 #3230 显式 declare，独立 worktree，仅修改 ADR、索引与本 Note。
...
- 文档相对链接全部可解析，D1–D6 标题与 Proposed v0.2 索引一致；`git diff --check` 通过。
...
owner 接受方向后，按 ADR Phase A→B→C→D 实施并同步既有权威文档；
不能让 Proposed 在实现中被当成隐式 Accepted。
