...
**未落地（有意，附原因）**：issue 期望的**界面口径**（在 DLE 视图上区分「未归档：该 run
无 merge 产物」）需要 `frontend/src/components/plan-run/`——该目录当前被另一在窗 Execution
（`fix-2359-2361-2363-2364 前端批`）占用，按并行纪律不碰。建议由该批或后续单承接，
判据与文案可直接引用本文档一节。
...
- **界面口径**：待 `components/plan-run` 释放后补一行状态说明（判据已定，改动很小）；
  在此之前，运维按文档的「两段式排查口径」操作。
