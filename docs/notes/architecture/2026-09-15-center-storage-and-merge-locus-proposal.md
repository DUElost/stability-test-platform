# 中心存储结构重排与 merge 执行位置：方案提案（D 步已评审裁决，待落地）
...
| **§6.9：D 步 W2 取舍**（2026-09-16 拍板） | **接受评审一稿建议：W2 + 每 host 分片，载荷收窄为仅 R1 scan 产物账；D 照推** | 依据 = [D 步方案评审一稿](../../reviews/REVIEW_CENTER_STORAGE_DSTEP_2026-09-16_5aba0.md)（PR #2427，已合入）+ **Owner 拍板评论**（#2188，marker `dstep-w2-owner-verdict-0916`：接受核心结论、知悉 D 升格为契约动作的成本上修、四未决答案照单为 D 设计定稿基线、**R-1…R-5 逐项带入验收判据**）。要点：①清单是写侧**登记账**，读者按权威对待，glob 降级为一次性恢复工具；②「旧 Agent 不写」检测搭既有通道、禁新增私有 env 键（ADR-0033 §5.4），过渡降级带**终态出口**（全队切换后删 glob 路径）；③**R-1 前提已生产只读复核成立**（2026-09-17：盘上 1,144 事件目录、DLE 行反例 **0**、行有盘无 2,310 = 先文件后行正常瞬态）——R2 改查 DLE，清单第一版不承载事件位置索引；④清单只记 family 内相对键，与 §2.0 外链解耦。**落地义务**：R-2（`_meta/` 进 `purge_run_storage_dirs` 桶清单，`cron_scheduler.py:289` 现写死三族）与 R-4（结构判据契约测试）绑定 D 落地 PR；R-3/R-5 绑 A 步。本行**不触发 ADR-0025 修订**（仍待落地与效果对比） |

**尚未做**（等上表落地）：ADR-0025 修订；§5 各项阈值；E-2 / E-4 / E-5 的采集（脚本已显式列为 not covered）。**D 步已评审并裁决推进**（§2.3 + 裁决记录 §6.9 行，2026-09-16）——**待落地**。
...
### 2.3 D 步方案：读者 → 写者 → 并发模型（**裁决输入**，2026-09-16）

裁决记录把 D 定为第一段（「先落 `_meta/{run}.json`，**不动目录树**」），理由是「可独立先行」。
但**「改动面小」不等于「收益已经存在」**——§2.1 的 D 行自己把代价写成「不直接省存储；多一份需要
维护的派生文件」。故本节按 **读者 → 写者 → 并发模型** 回答，顺序不可换：读者的语义决定清单里该
有什么，也决定它是「可丢的缓存」还是「必须与写入同步的账本」。

#### 2.3.1 谁读：控制面上**确实有两处「扫目录推事实」**，但都不是 D 行设想的那个

D 行把现状描述成「完备性判定靠『扫目录 + 四维收窄』」。按代码核，这**两半分属不同位置**：
「四维收窄」是纯 DB 查询（`dedup_scan.py:167` `scan_completeness`），与扫目录无关；真正在扫中心
盘的是另外两处，且都不在 §1.1 的清单里：

| # | 位置 | 今天怎么答 | 扫了什么 |
|---|---|---|---|
| **R1** | `_register_scan_artifacts_from_nfs`（`dedup_scan.py:70`），由 `run_scan_sync`（`:106`）按 `dedup/{run}/[mtk\|unisoc]`（`:131`）逐目录调用 | **glob** `*_org.xls` / `*_org_*.xls`，用**文件名约定** `{host_id}_Result_*_org.xls`（`:75`）经 `_HOST_PREFIX_RE`（`:67`）提取 host，逐条查 DB 去重后写 `plan_run_artifact` | 中心盘 `dedup/{run}/` 的**文件** |
| **R2** | `_resolve_center_event_path`（`dedup_scan.py:961`） | 本 run 的 `devices/{run}/{event_dir}/` 不在时，**glob `devices/*/{event_dir}`**（`:976`）跨**所有历史 run** 找同名事件目录；找不到保留原映射 | 中心盘 `devices/*/` 的**目录** |

**R1/R2 才是 D 的买家**，且暴露的是同一个根因：**生产者不登记**。scan 产物由 Agent 经
`UploadManager` 直接写共享盘（R1 自己的注释即写「由 UploadManager.fill 放置」），控制面只能靠
glob + 文件名约定**事后发现**；事件目录由 `event_uploader` 写入（`backend/agent/event_uploader.py:413`），
R2 只能在「名字对得上」的前提下跨 run 猜位置——它的注释写着「搜不到保留原映射（尽力而为）」。

而**被点名的那几个消费者全都不遍历目录**，因此**不构成 D 的收益**：

| 消费者 | 权威来源 | 为什么清单省不掉 |
|---|---|---|
| scan 完备性屏障 `scan_completeness`（`dedup_scan.py:167`） | DB 查询（`plan_run_artifact`，按 (host, platform) 四维收窄） | 它回答「本轮 `expected` 内**该**齐吗」——是**查询**不是**遍历**；清单表达「有什么」，给不出「该有什么」 |
| extract 缺口 `list_remote_paths_for_extract` + 完成标记（`backend/services/dedup_extract.py:19,151`） | DB 行状态 + 磁盘存在性 | 同上 |
| unassigned 归属搬移 `adopt_unassigned_event_dirs`（`backend/services/device_log_event.py:242`） | **DB 行**（`remote_path LIKE '%/devices/unassigned/%'`）再按行定位 rename | 以行为索引是**有意**的（「用行 id 定位事件目录是确定的」） |
| retention `purge_run_storage_dirs`（`backend/scheduler/cron_scheduler.py:244`） | **DB 行是「哪些目录属于此 run」的唯一索引**（该函数 docstring `:247`） | **负收益候选**：清单若被当成第二索引，清理就多一个可能陈旧的真值源 |

**推论：D 步的载荷与 D 行写的不一样。** 第一版清单应收 **scan 产物账**（host / 平台分区 / URI，
即 R1 现在靠文件名约定猜的那几项）与**事件目录位置索引**（`event_id → 所在 run`，即 R2 现在跨 run
glob 找的那个答案），**而不是**「事件清单 + 缺口」——后者在本步没有具名读者。

#### 2.3.2 谁写：由 R1/R2 的根因反推，只有写侧登记能兑现收益

| 选项 | 形态 | 能否消掉 R1/R2 | 代价 |
|---|---|---|---|
| **W1 控制面单写**（从 DB 派生） | 控制面在 `run_scan_sync` / merge 发布后生成 | **不能**：控制面得先知道「有新产物」才能派生，而 R1 正是它获取该知识的手段 → 循环。除非改成上送后回调登记（**新契约**，与 W2 同代价还多一跳） | 无并发问题，但收益 ≈ 0 |
| **W2 写侧登记** | Agent 在写产物/事件目录的同一动作里登记清单 | **能**：把「事后发现」变成「事前声明」，R1 的 glob + 文件名约定与 R2 的跨 run 猜测一起消失 | 清单成为 **Agent↔控制面契约**：需版本字段、失败语义、旧 Agent 的降级路径 |
| **W3 混合**（写侧登记 + 控制面兜底） | — | 能，但**保留兜底 = 保留 R1 的代码路径** | 两套真值并存，最贵 |

**W2 是唯一能兑现收益的选项，也是唯一把 D 从「文档动作」变成「契约动作」的选项。** 评审应明确
接受或否决它：若否决 W2，D 的收益归零，那就应当**只做 §2.1 的 A，或干脆不推 D**。

#### 2.3.3 并发模型：单一共享清单会是这棵树里的**第一个跨主机共享可写文件**

事实：`devices/{run}/` 今天由 **N 台 Agent 并发写**（`event_uploader.py:413`，每台写自己的
`{event_id}/`），而它安全的原因是 `event_id` 隔离（#1073）让「**没有任何两个写者碰同一路径**」
——这是结构性 lock-free，不是靠锁。`dedup/{run}/` 同理：`{host_id}_...` 的文件名本身即分片键。

于是结论几乎是给定的：

- **一个 `_meta/{run}.json` = 引入全树第一个跨主机共享可写文件**，而现存唯一互斥是同机 `flock`
  （#1072），§3.1 已论证它在 CIFS/NFS 上跨实例无效；
- **分片写（`_meta/{run}/{host_id}.json`，一写者一文件）+ 读者聚合**不需要任何锁——它只是把现在
  「文件名即分片」的**隐式**约定**显式化**；聚合读 N 个小文件的代价，远低于 R1 现在的「遍历 +
  正则 + 逐条查库去重」；
- 反向若选 W1，并发问题不存在，但那正是「收益 ≈ 0」的那条路。

**生命周期硬约束**：清单与 run 同生命周期（retention 一并清），且**不得**成为「哪些目录属于此
run」的第二索引——`purge_run_storage_dirs` 的单一真值（DB 行）是不变量，清单只能是**派生视图**，
丢失后必须能由 DB 重建。

#### 2.3.4 建议与未决（供评审回答）

**建议**：D 步按 **W2 + 每 host 分片清单**推进，且**第一版只承载 R1/R2 所需的信息**（scan 产物账 +
事件目录位置），不承载「事件清单 + 缺口」——只有这版清单有具名读者，收益才可判定。

**判据（替代 §5 的字节口径）**：D 不省存储，不能按 E-1 的字节数验收。应改为**结构性判据**：
`run_scan_sync` 与 `_resolve_center_event_path` 不再遍历中心盘，且 R1 的**文件名约定依赖**一并消失。

**未决**：

> **（2026-09-16 Owner 已拍板**（marker `dstep-w2-owner-verdict-0916`）：四项均由[评审一稿](../../reviews/REVIEW_CENTER_STORAGE_DSTEP_2026-09-16_5aba0.md)
> 回答并照单为 D 设计定稿基线——登记账（glob 降级为一次性恢复工具）/ schema_version + 既有通道检测 +
> 带终态出口的过渡降级 / 每 host 分片 + **载荷收窄为仅 R1 产物账**（R2 改查 DLE，前提已生产只读
> 复核成立：2026-09-17 盘上 1,144 目录、反例 0）/ 清单只记 family 内相对键。原文保留作历史记录。**）**

1. **权威还是缓存**？若是缓存，读者仍须保留「清单缺失/陈旧 → 退回 glob」的兜底，收益即打折；
   若权威，则「清单没写」必须等价于「产物不存在」，需要写侧的失败语义——**写产物成功但登记失败
   = 静默丢交付物**，这正是本仓反复吃过的形态。
2. **版本与降级**：W2 让清单成为 Agent 侧契约——旧 Agent 不写清单时，控制面如何区分「还没写」与
   「这一版不写」？（与 ADR-0033 的工具/契约分发同族；不得新增工具私有 env 键。）
3. **粒度与容量**：scan 产物量级小（数十/run），但 `devices/` 侧 E-1 基线是 10,725 文件 / 21 run
   ——清单记到「每事件一条」还是「每 host 一桶」，需按 R2 的查询形态定。
4. **与 §2.0 外链约束的关系**：清单若成为 JIRA 侧引用的依据，须一并满足「路径不得改名」。

**边界**：本节是**裁决输入**，不是裁决；**不触发 ADR-0025 修订**（该修订仍绑 I-12/I-13 方案落地
与效果对照）。本节不改任何代码，也不改 R1/R2 的现行行为。
...
9. **D 步（§2.3）**：是否接受 **W2（写侧登记）+ 每 host 分片清单**？§2.3.4 的四项未决（权威/缓存、版本与降级、粒度、外链）如何回答？——**否决 W2 等于否决 D 步的收益**（§2.3.2），此时应明确「只做 A」或「不推 D」。
...
- **§2.3 的 `file:line` 为 2026-09-16 会话亲读**（基线 `babf27ac`，逐处 grep 复核后引用）：
  两处中心盘遍历 = `backend/services/dedup_scan.py:70`（`dedup/{run}/` glob + 文件名约定）与
  `:961`（`devices/*/` 跨 run glob）；四处 DB 权威 = `dedup_scan.py:167`（`scan_completeness`）、
  `dedup_extract.py:19,151`（完成标记）、`device_log_event.py:242`（adopt 以行为索引）、
  `cron_scheduler.py:244,247`（purge 的单一索引声明）。
...
- **D 步（§2.3）的读者前提失效时**：若 R1/R2 在落地前已被其它改动消掉（例如 scan 产物改为登记制），
  清单即无具名读者——此时应只做 A 或**不推 D**。注意这与首条「收益小 → 只做 D、不做 A/B/C」是
  **反向**取舍，两条判据要一起看：前者说"D 够用"，本条说"D 没人读"，都不支持"照原样推 D+A"。
