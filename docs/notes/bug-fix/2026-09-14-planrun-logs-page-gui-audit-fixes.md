...
- 未验证（如实记录）：IAB 运行时输入管线中途损坏 + 不发 download 事件，CSV 文件本体
  ...
  - **回填（2026-09-15，#2028）**：上句括号里的「逻辑由单测覆盖」在提交当时**不成立**——
    `1e339766` 没有给 `handleExportCsv` 留任何测试，这正是 #2028 的立项理由（Note 与测试树
    不一致即视为过度声明）。现已补 6 例（分块终止、CSV 转义 + BOM 字节、20 000 上限可见化、
    在途窗口位移去重、非终态快照声明、blob URL 延迟撤销），并修掉其中三个原本静默的行为。
    本条其余记录（对比度、GUI 端到端）不改动——它们是当时的实测事实。

## Revisit

- ~~`--primary`（light）白字 3.68:1 不达 AA~~ **已裁决并落地（2026-09-15）**：升真
  blue-600，详见 `2026-09-15-primary-brand-color-contrast-aa.md`。
- facets 在搜索过滤下仍显示全集计数（与 stage/severity 口径一致，刻意保留）；若用户
  反馈「搜索后徽标数不变」困惑，再议是否加「命中数」徽标。
- ~~生产部署后需在真实数据上复测搜索过滤与 CSV 落盘~~ **已闭环（2026-09-14 晚部署）**：
  搜索（标题 5392 条 / 设备号 12 条）、空态清除、导出分块请求均线上实测通过。
- 待办仅剩：CSV 文件本体落盘的浏览器侧复核（IAB 运行时不发 download 事件，
  常规浏览器一次导出即可确认）。
