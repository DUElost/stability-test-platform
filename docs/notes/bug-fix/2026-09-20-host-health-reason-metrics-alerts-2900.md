...
- **生产只读实测**（psycopg3，`application_name='diag-2900-*'`，只 SELECT）：见 Decision 5 的
  4 条数据。凭据面：连接串只从 `.env.backend` 读入内存，未写入任何脚本、文件、日志、
  仓库或评论；一次 psycopg 异常把 DSN 回显在**本机终端**（首行缺 `=` 的解析错误），
  该文本未离开本机、未进入仓库与 PR，脚本随后改为 `re.sub(r"^postgresql\+\w+://", ...)`
  且 stdout 一律 `grep -v` 过一遍连接串。
