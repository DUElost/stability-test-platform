...
- **后端**：`routes/action_templates.py`、`models/action_template.py`、
  ...
  **漏网两处随验证补删**：`tests/conftest.py` 与 `scripts/init_dev_db.py`
  的 `import backend.models.action_template`（模型元数据注册用途；收集期
  ImportError 暴露，非 issue 原清单）；
...
- 残留检查：`grep -rn "action_template\|ActionTemplate\|actionTemplates"`
  全仓仅剩 alembic 历史迁移与 ADR 的「已删除」标注；
