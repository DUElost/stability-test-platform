...
- 若将来设备列表改为按业务字段（如 status / project_key）排序，`id` 应继续保留
  为最后一级 tie-breaker。
- **2026-09-22 已重议（`#3123`）**：本文「整个结果集全序确定」只对**同一时刻**成立。
  主键 `last_seen` 每次设备心跳都被重写，全序因此在两次轮询之间整体重排——生产实测
  862 台、10s 窗口内前 50 行 0% 重合、位置中位移动 250 行。默认排序已改为
  `(host_id, id)`；本文「`id` 作最末级 tie-breaker」这条结论保留。后续决策见
  [`2026-09-22-device-list-stable-order-3123.md`](2026-09-22-device-list-stable-order-3123.md)。
