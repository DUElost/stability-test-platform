...
2026-08-15 网段迁移后，34 台 host 的 IP 全部迁至 172.21.x.x，但其中 20 台的
`host.id`（前端显示的主机标识）仍保留旧 8.x/9.x 网段号（如 `.80` 的 id 仍是
`172-21-x-x`），与其当前 `host.ip`（`172.21.x.x`）不一致——按编号找机
与按 IP 认机出现二义。

用一次性 Alembic 数据迁移 `k8l9m0n1o2p3` 将所有 `host.id` 统一为
`172-21-<新网段>-{ip 末段}`（与已迁移的 13 台同约定）。映射由 `host.ip` 末段在库里
现算，不硬编码，杜绝漂移。
...
- scratch `postgres:16` 一次性容器（非生产）：`alembic upgrade` 至 `h2i3j4k5l6m7`，
  ...
  `172-21-x-x→172-21-x-x`、`172-21-x-x→172-21-x-x`，6 FK + 2 快照列
  全部正确改写、`host` 无旧号残留、中性 host（`172-21-x-x`）不动。
