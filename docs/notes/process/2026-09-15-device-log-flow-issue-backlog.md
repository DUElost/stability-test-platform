...
| **文档漂移 issue** | 事实已定，表述过期 | I-10（**查重命中 #2016，不新立**）、I-11（归 #1050，不新立） |
...
**issue 载体落地（2026-09-15，提交前逐条查重后创建）**：

| 条目 | issue | 说明 |
|---|---|---|
| I-5 B3 分平台 merge spike | **#2183** | 标 `capacity:inventory-bound`（需真实 toolkit + SPRD `_org.xls`） |
| I-6 DLE 终态视图缺平台维度与失败态 | **#2184** | 纯展示，不改契约 |
| I-8 归档状态分散三卡 + 缺最终轮入口 | **#2185** | |
...
即：**7 条新 issue（#2183–#2189）+ 2 条查重命中不新立 + 8 项已由 PR #2174 承载**。
本表因此同时是"哪些没立单"的记录——避免后来者以为漏建。
