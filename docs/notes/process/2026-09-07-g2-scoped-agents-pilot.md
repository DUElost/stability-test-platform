...
- **矩阵扩展（09-07 续）**：Cursor **IDE** 3.17.19 人工探针 Q1=是 Q2=是 Q3=2——与 CLI 同引擎对齐（含双份加载）；**Zcode 3.11.2** Q1=否 Q2=是——子目录**只装载 workspace 的 AGENTS.md、根不注入**（#857 互补形态；可发现性由 scoped 真身根指针覆盖，P2 动作表 #919 补「评审类会话同样 declare」指引）；**CodeBuddy 2.143.1** Q1=是 Q2=是 Q3=一次——零配置通过。七形态矩阵见 ADR 附录 A。
