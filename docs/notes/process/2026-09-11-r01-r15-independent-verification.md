...
**§9 组合治理核验（同 PR 追加）**：确认存在临时止血与"同一根因点修链"三类——
① `#894`→`#1171`→`#1276` 迁移种子常量三连，`#1276` 的 Revisit 自述"第 3 次即应做门禁"而该门禁**至今未建**
（`tests/test_script_seed_governance.py` 名近实异，内容是 `#942` 的引用感知，不含 sha 比对）；
② `#1123`/`#1085`/`#1205` SAQ 超时-线程残留三连；③ `#986`→`#1172` 终态提交契约（此项做对了）。
类级复发量化：静默吞 27 / 假成功 13 / 竞态 17 / 保留清理 36 / 测试隔离 4，六个系统性根治单 `#734`–`#739` 全 OPEN。
机制缺口可测：`ruff.toml` 未启用 `E722`/`BLE`，AST 口径宽泛捕获 **999** 处、body 仅 `pass`（真静默吞咽）
**306** 处 / 84 文件，现成规则可报 186 条。给出 §IV-B06 七项"并单而非开单"建议与阈值规则
（同类第 2 次即升级机制级，第 3 次未升级判治理失效）。
