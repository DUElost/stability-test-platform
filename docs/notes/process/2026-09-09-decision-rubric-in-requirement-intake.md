...
- 若三问连续若干需求都填「无 / 无 / 一次性」而成套话，则降级为注释行或
  移入 `docs/notes/README.md`（提示语留痕即够）——出现「填了没人看」的样本
  即重议；
- 若出现「需求阶段按三问否掉、事后证明该做」的事故，则把第三问从
  「复用预期」改为显式的「不做会怎样（可验证的损失）」，并考虑在
  `.cursor/rules` 增一条需求评审提示；
- 团队规模 ≥3 人常驻时，与「刻意不追」中「强制 code owner 审批」同批重议
  （两者前提相同：单人主路径）。
