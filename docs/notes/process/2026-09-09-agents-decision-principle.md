...
- **观测方式**：① 凡 diff 触及 `AGENTS.md` 时，人工复核该节是否被改写/删除；
  ...
  确认锚文本仍在。
- **触发后二选一（含代价）**：
  - **A. 加 S11 锚串**：改写即 BLOCK；代价是与「原则本可演进」冲突，需改
    `tools/dev/check_governance_surface.py` 的 `ROOT_HEADING_ALLOWLIST` 配套逻辑；
  - **B. 升格进「硬不变量」**：位置即权限，自动获 S11 锚保护；代价是须满足
    该节入场标准"对任何 Requirement 都成立"——**仅第二行（止血标注）够格**，
    第一行（要不要做/本质判断）不够。
