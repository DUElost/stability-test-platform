...
> 关联：`#2113`（本单）、`#2022`（保留清理取锁顺序对齐 I1/I2）、`#2104`（窗口与等待的观测面）。
> 过程说明：立单时 GitHub API 连续返回 `Post … EOF`（与 `git fetch` 的 TLS 握手中断一致，
> 环境侧临时故障），故先落本地提交保住改动；网络恢复后已按 `#2113` 补单，并 rebase 到含
> `#2106` 的 main（避免本 Note 与 `#2104` 的文档改动在合并时冲突）。
