...
- 成本：派生化让告警面守卫、锚点在场守卫、仪表板面各扫一次全仓源码（`--durations`
  实测 1.5s / 1.8s / 2.1s，两文件合计 **5.6s**）。**不做跨用例缓存**——缓存等于在同一
  pytest 会话里共享可变 AST；#2287 扩面若成本再翻倍，改只读缓存时必须同时写清失效边界
  （新增/删除源文件要让缓存失效），否则缓存本身会变成下一类假绿；
