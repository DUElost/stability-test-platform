...
- **实施计划**：[2026-06-22-sprint4-scan-upload-merge.md](../superpowers/plans/2026-06-22-sprint4-scan-upload-merge.md)
...
| AC-S4-02 | ScanRunner glob 取最新 xls | 多个 `_org.xls` 存在 | `max(candidates, key=st_mtime)` 取最新 | `test_scan_runner.py::test_run_local_scan_picks_latest_by_mtime` | PR #35 |
...
| AC-S4-13 | SAQ merge_task 保留 | 终态触发 | 调用 `run_scan_sync` 扫 NFS → merge | `saq_tasks.py` | PR #35 |
...
| AC-S4-26 | 前端 DedupReportCard | PlanRun 详情页 | scan/merge/extract 按钮 + 产物列表 | `DedupReportCard.tsx` | PR #35 |
...
- [ ] AC-S4-01 ~ AC-S4-10 Agent 单元测试 570 PASS
