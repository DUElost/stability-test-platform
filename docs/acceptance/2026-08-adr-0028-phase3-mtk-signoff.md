...
| inotifyd 独占写 DLE | Reconciler 在岗时 inotifyd 路径被抑制；「关 Reconciler 只走 inotifyd」自动化覆盖见 #310（`test_device_watcher_dle.py`：pull 成功 → DLE create + EventUploader enqueue；pull 失败 → pull_failed 记录；reconciler 激活 → 抑制注册）。**真机 E2E 已完成（2026-09-19，#310）**：单机翻转 `STP_WATCHER_AEE_RECONCILE_ENABLED=false` + reload_config，骑周期回归链两轮采证——创建半程（inotifyd 触发 → pull → DLE LOCAL + signal 关联）在合成与真实条目重放（17MB ANR）上均验，含 puller 严格校验重拉与 inotifyd 源 31 次断连退避自愈实录。**上送半程曾断链、修复后已验**：合成不可解析内容停在 LOCAL 属过滤模型设计行为（保持）；真实条目重放在当时未达标记，根因 puller 落地 `<epoch_ms>_` 前缀与 `event_dir_basename_from_path` 年首时间戳正则不兼容——#2822 已修复合入（`27b060a0`），2026-09-21 控制面重启生效后经 run-445 重扫复测：卡滞 DLE 全链 LOCAL→UPLOAD_PENDING→REMOTE→ARCHIVED 4 分钟闭合（extract 产物实盘在），#310 验收标准 2 由「明确限制」升级为「已验」（证据见 #2822 评论）。执行程序 [`../operations/2026-09-19-inotifyd-only-e2e-procedure.md`](../operations/2026-09-19-inotifyd-only-e2e-procedure.md)；证据与 Agent Note [`../notes/testing/2026-09-19-inotifyd-only-e2e-310.md`](../notes/testing/2026-09-19-inotifyd-only-e2e-310.md) | |
