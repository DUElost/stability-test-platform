...
todos:
  ...
  - id: wave2-audit
    ...
    status: pending
  - id: wave3a-services
    content: "Wave 3a: 后端服务层迁移 — recycler + report_service 已完成；post_completion 待 Alembic 列新增"
    status: partial
  ...
  - id: wave3b-api
    ...
    status: pending
  - id: wave3c-frontend
    content: "Wave 3c: 前端逐页迁移（重点适配新的 Workflow-level 概念）"
    status: pending
  - id: wave3d-cleanup
    content: "Wave 3d: 清理 schemas.py，仅保留 User/NotificationChannel/AlertRule/TaskSchedule"
    status: pending
  - id: wave3e-tests
    content: "Wave 3e: 测试迁移（conftest/pbt/concurrent + 前端测试）"
    status: pending
  - id: wave4-migration
    content: "Wave 4: 重写 Phase 1 为 3 步渐进式迁移脚本（create -> data -> drop）"
    status: pending
  - id: wave5-ci
    content: "Wave 5: CI 一致性检查（alembic check + DDL 回归 lint + downgrade 测试）"
    status: pending
isProject: false
---
...
## Wave 4 & 5: 数据迁移与 CI 守护

重写 Phase 1 为 3 步增量迁移（CREATE -> MIGRATE -> DROP），并引入 CI 一致性检查。
