...
cd "$STP_DEPLOY_ROOT"
./tools/dev/check-deploy-source.sh   # 部署源守卫：HEAD 必须在 main、tracked 工作区干净、热更新载荷根 backend/agent/ 无未跟踪文件、schema 未超前代码 head
...
HOST_ID=<唯一 ID，推荐 IPv4 点转横杠，如 198-51-100-6>
...
注意：
- `HOST_ID` 是逻辑主机主键；新建主机推荐由节点 IPv4 派生（`198.51.100.6` → `198-51-100-6`），须全局唯一且固定。
