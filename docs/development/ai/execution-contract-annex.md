...
- **状态**：规范附录，**与正文同版本演进**（当前 v1.13；v1.13 为正文 §3.4 的「空 issue 集必须显性」单条语义增补，附录侧无细则变化）。正文为 [`execution-contract.md`](execution-contract.md)（语义唯一权威源）；本附录是其**规范组成部分**，承载实现级细则与历史留档。
...
## A.5 空 issue 集边界与 #2706 撞车实录（#2729）

**边界**：§3.4 的 issue 集 = 显式 `--issue N` ∪ requirement/branch 名的启发式提取
（`(?<![A-Za-z0-9])(?:issue|fix)(?:[-/_\s]|#)+(\d{1,6})(?![0-9])`）。名字里没有**带标记的数字**时，
该记录的 issue 集为空 ⇒ 它对 §3.4 查重**不可见**——工具不会、也无法从名字推断出没写出来的号。

**实录（2026-09-18，一次真实撞车）**：同一 issue #2706 被两个 harness 各自实现（PR #2708 先合入，
#2709 被 owner 按重复关闭）。两条在窗记录：codebuddy 的 requirement 名为
`fix-ms04 MS-04 的 S3 证据补候选集（#2404 同类：迁移路径发 install.s3.migrate）`、`issues=[]`——
`ms04` 非数字、`#2404` 是母题号，提取结果为**空集**；9 分钟后 claude 的名字含 `fix-2706`，
declare 时与前者无交集、静默通过。

**关键事实（v1.13 的依据）**：该信息**此前已在提示**——issue 集为空时 `[OK]` 行尾会带
`（hint：…在窗查重无输入，建议 --issue N）`（#978 起，`9c5c4446`），而撞车**发生在 hint 在场时**。
故 v1.13 只做两件事：把提示升级为独立 `[WARN]` + 风险措辞；撤掉行尾 hint（同一事实只留一处）。
**不改** §3.4 的判定规则、不动 §3.5 决策类的强制 `--issue`、不引入硬拒绝（纯重构/探索是合法场景）。
