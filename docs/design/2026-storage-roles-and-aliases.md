...
- **状态**：Living（2026-09-25：同步 ADR-0053 的对象/引用/工作区边界；部署快照未复测）
...
### 1.1 中心存储内部角色（ADR-0053 已裁决，待实施）

[ADR-0053 v0.2](../adr/ADR-0053-center-storage-event-dedup.md) 于 2026-09-25 Accepted：
中心存储内部区分不可变文件 blob、版本化事件 manifest 和可再生的 scan/extract/Jira 工作区；
PG 保存 DLE 观察、内容引用与使用保活，Redis 不保存这些事实。内容对象独立于首次上传 run，
工作区可写且不共享证据 inode。共享根仍是同一存储角色，CAS 不要求引入新存储服务。

Phase A–D 尚未实施；对象根的具体布局随实施定义。本文 `devices/{run}/` 等路径是当前实现坐标，
不能据此要求目标保留每 run 的完整物理副本，也不能直接用 run 目录删除代替目标引用 GC。
...
**Agent 侧挂载的两个语义（#2321，工作站内已权衡，改动需重新裁决）：**

1. **挂载源语法**：NFS 源必须是 `<host>:<export>`，IPv6 字面量写作 **`[addr]:/path`**。
   控制面从站点入口派生服务端主机（`agent_installer.nfs_server_from_api_url`）时**保留方括号**
   ——`urlsplit().hostname` 会剥掉它，拼出 `2001:db8::1:/srv/...` 是冒号歧义，挂载必然失败，
   且现象与「网络真的不通」不可区分。
2. **hard mount（默认，未加 `soft/timeo/retrans`）**：控制面分享断线时，触碰该路径的 Agent
   线程进入**不可中断睡眠**——`STP_AEE_NFS_ROOT` 是 AEE reconciler / 事件上传 / dedup 的落点，
   而 `nofail,_netdev` 只解决开机顺序、不解决运行期断线；恢复只能等对端回来或强杀进程。
   **维持 hard 的理由**：站点内网同机房、可用性由 NFS 侧保证；改 soft 的代价是断线以 `EIO`
   暴露，而 Agent 侧对 `EIO` 的处理路径未经验证（改成 soft 之前必须先有那条处理链的验证）。
   复议触发：出现「控制面/NFS 侧非计划断线导致 Agent 线程挂死」的现场实例。
...
| 日期 | 变更 |
|------|------|
| 2026-09-25 | 同步 ADR-0053 v0.2 Accepted：内容对象、观察引用、派生工作区职责分离；实施尚未开始 |
| 2026-08-09 | 初版：九角色 + CIFS=中心存储 + 健康页≠CIFS + 8.202 过渡 / 15.4·9.4 目标 + #205 |
| 2026-09-16 | §4.4 补 Agent 侧挂载的两个语义（#2321）：**IPv6 源保留方括号**（`urlsplit().hostname` 会剥掉 → 冒号歧义，挂载必失败且与「网络不通」不可区分）；**hard mount 语义记录在案**（维持 hard 的理由 + 复议触发），此前仓库无任何权衡记录 |
