...
| 3 | **控制面** | FastAPI / Dashboard / SAQ，生产现 `172.21.15.253`（2026-08-13 起 IP 由 8.202 变更，其余不变） |
...
| **8.202** | 旧控制面 IP（2026-08-13 起不再使用） | 不再指任何角色 |
...
| 推荐名 | 合法别称 | 不要当成 |
|--------|----------|----------|
| 控制面 | 控制平面、backend、FastAPI、**15.253**（8.202 为旧 IP） | CIFS（仅过渡期同机） |
...
| 8.202 | CIFS 或控制面（含糊） | 旧控制面 IP；2026-08-13 起为 15.253 |
...
| 2026-08-13 | 控制面 IP 由 172.21.8.202 改为 172.21.15.253（仅 IP 变更，其余不变）；过渡 UNC 与健康页展示 IP 相应同步 |
