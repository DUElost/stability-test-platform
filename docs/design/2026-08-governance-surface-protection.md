...
| S14 | 代码注释里的 ADR 版本引用 ∈ 该 ADR **已发布版本集合**（`- 版本记录：` 块 token ∪ `## 修订记录` 章节 token ∪ 头部规范位版本）；头部不带规范位版本者不约束（同 S12）；只绑 ADR 号后**首个** `vX.Y`；`backend/agent/scripts/`（内容冻结，改即触发 immutability 门禁）整枝跳过 | BLOCK | 2026-09-15 随 #2154 收口引入（ADR-0043 升 v1.1 时注释漏改一处，只能人肉 grep）；2026-09-16 判据放宽（#2249）：旧判据「== 头部规范位版本」在头部 bump 后把**刻意引用历史版本**的合法注释判红（ADR-0032 头部 v0.9、代码引 v0.8 即现实例），并使「给头部补版本」的努力制造大面积红灯（#2250 的 57 处）→ 覆盖缺口永久化。集合取两形态是因实际都在用：12 篇项目符块、7 篇 `## 修订记录` 表格（列布局三种） |
...
| 2026-09-16 | S12 状态 cell 粗体归一 + 增「索引行状态 cell 须在场且可解析」（#2035）：索引里 8 行写作 `**Accepted**`，与裸词不等 → 整行静默退出状态比对（旧实现实跑同一条坏样例：`--check` 仍 OK）。S14 判据由「== 头部规范位版本」放宽为「∈ 已发布版本集合」（#2249）：集合 = 项目符块 token ∪ `## 修订记录` 章节 token ∪ 头部版本——两种版本历史形态都在用（12 篇项目符 / 7 篇表格），只读项目符会让 ADR-0032 v0.8 这个现实例依旧判红；同批补登 S14 规则行（#2154 落地时未回填本文）。零存量红灯（70 处引用新旧判据下同为 0 红） |
