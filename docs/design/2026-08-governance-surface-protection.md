...
| S12 | ADR 索引一致性：头部状态行 ↔ adr/README 主表/DOC-MAP/M7 看板（status 词级 + 规范位版本），头部行 ↔ 版本记录块**最新版本**（按版本号取最大，不按书写位置取末项——各形态书写顺序不一致）；**头部状态行须在场且可解析**（#1524：键位粗体 `- **状态**：` 是执行契约文档形态、表格 `Status` 行都取行失败 → 该 ADR 静默退出全部索引校验，ADR-0035/ADR-0022 实测无覆盖）；**索引行状态 cell 也须在场且可解析**（#2035：`**Accepted**` 粗体形态与裸词不等 → 该行静默退出状态比对，2026-09-16 实核 8 行——开单时点 5 行，其后新增的 ADR-0041..0044 沿用该风格；归一剥标记，判据不变） | BLOCK | 2026-09-07 随 #867 收口引入：ADR 版本 bump 漏同步索引已五次复发（#861 修两处、DOC-MAP/adr/README 再漏），且出现文内形态（头部行停 v1.3 而版本记录块已 v1.6）；版本约束仅限头部行携带规范位版本（**Status（vX.Y）** / **Status**（vX.Y：）），注解散文 token 不算 |
...
| S14 | 代码注释里的 ADR 版本引用 ∈ 该 ADR **已发布版本集合**（版本记录块 token ∪ `## 修订记录` 章节 token ∪ 头部规范位版本）；头部不带规范位版本者不约束（同 S12）；只绑 ADR 号后**首个** `vX.Y`；`backend/agent/scripts/`（内容冻结，改即触发 immutability 门禁）整枝跳过——**永久豁免**，排除面现有 6 处（`mtbf_setup` v1.0.0–v1.4.1 各一处 `ADR-0029 v2.2`），随版本退役自然淘汰（#2250 登记） | BLOCK | 2026-09-15 随 #2154 收口引入（ADR-0043 升 v1.1 时注释漏改一处，只能人肉 grep）；2026-09-16 判据放宽（#2249）：旧判据「== 头部规范位版本」在头部 bump 后把**刻意引用历史版本**的合法注释判红（ADR-0032 头部 v0.9、代码引 v0.8 即现实例），并使「给头部补版本」的努力制造大面积红灯（#2250）→ 覆盖缺口永久化。集合取三种书写形态：单行罗列 / 缩进续行（ADR-0035）/ 缩进子项（ADR-0038、0042），外加 `## 修订记录` 表格 7 篇（列布局三种）——只读其中一种会漏掉整篇 ADR 的已发布版本 |
...
| 2026-09-16 | S12 状态 cell 粗体归一 + 增「索引行状态 cell 须在场且可解析」（#2035）：索引里 8 行写作 `**Accepted**`，与裸词不等 → 整行静默退出状态比对（旧实现实跑同一条坏样例：`--check` 仍 OK）。S14 判据由「== 头部规范位版本」放宽为「∈ 已发布版本集合」（#2249）：集合 = 版本记录块 token ∪ `## 修订记录` 章节 token ∪ 头部版本——版本历史有**三种书写形态**（单行罗列 / 缩进续行 ADR-0035 / 缩进子项 ADR-0038、0042）外加表格 7 篇，只读一种会漏掉整篇 ADR 的已发布版本（缩进子项被旧块扫描误当「块结束」，ADR-0038/0042 整块零 token）。S12 的 record_tip 同步改取**版本号最大者**：书写顺序形态间不一致，按位置取末项对 ADR-0035（缩进续行、最新在后）会得到 v1.0 与头部 v1.2 假红。同批补登 S14 规则行（#2154 落地时未回填本文）。零存量红灯（70 处引用新旧判据下同为 0 红） |
