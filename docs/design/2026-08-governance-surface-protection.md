...
| S14 | 代码注释里的 ADR 版本引用 ∈ 该 ADR **已发布版本集合**（版本记录块 token ∪ `## 修订记录` 章节 token ∪ 头部规范位版本）；头部不带规范位版本者不约束（同 S12）；只绑 ADR 号后**首个** `vX.Y`；`backend/agent/scripts/`（内容冻结，改即触发 immutability 门禁）整枝跳过 | BLOCK | 2026-09-15 随 #2154 收口引入（ADR-0043 升 v1.1 时注释漏改一处，只能人肉 grep）；2026-09-16 判据放宽（#2249）：旧判据「== 头部规范位版本」在头部 bump 后把**刻意引用历史版本**的合法注释判红（ADR-0032 头部 v0.9、代码引 v0.8 即现实例），并使「给头部补版本」的努力制造大面积红灯（#2250 的 57 处）→ 覆盖缺口永久化。集合取三种书写形态：单行罗列 / 缩进续行（ADR-0035）/ 缩进子项（ADR-0038、0042），外加 `## 修订记录` 表格 7 篇（列布局三种）——只读其中一种会漏掉整篇 ADR 的已发布版本 |
