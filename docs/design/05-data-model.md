...
| 字段 | 说明 |
|------|------|
| `status` | RUNNING / SUCCESS / PARTIAL_SUCCESS / FAILED / QUEUED / PRECHECK（PARTIAL_SUCCESS=完成但有设备失败，黄；ADR-0048 v1.1 恢复产出，无阈值线、永不因设备失败判红）（QUEUED/PRECHECK 为准入队列状态，`STP_PLAN_ADMISSION_QUEUE_ENABLED` 默认开） |
