...
**网络**：控制面生产 `172.21.15.253`（2026-08-13 起由 `172.21.8.202` 变更，仅 IP 不同）。**中心存储（CIFS / NFS，同一台分享）** 过渡期与控制面同机（`//172.21.15.253/jxtinno/sonic_tinno`）；目标独立盘 `172.21.15.4` 或 `172.21.9.4`。角色/别称见 [`2026-storage-roles-and-aliases.md`](./2026-storage-roles-and-aliases.md)。
