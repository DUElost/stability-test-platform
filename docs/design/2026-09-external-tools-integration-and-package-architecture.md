...
>
> **排期口径（2026-09-10，对齐 ADR-0033 v1.2 §5）**：上方 Gantt 的日期为**参考序**，非承诺排期——Phase 2/3 零启动、原时间点（09-07 / 09-12 / 09-17 / 09-24 / 09-29）已作废，实际排期以 issue 为准。**D0（源码不入仓）与 D3 的版本权威裁定不等包存储就绪，已即刻生效**；D2 契约（本文 §2）收窄为**新工具族准入要求**；本文 §3 的 manifest / tar.gz / `tools_cache/` 分发机制为**条件落地**（触发条件见 ADR-0033 §5.4）。
