...
>
> **排期口径（2026-09-10，对齐 ADR-0033 v1.2 §5；2026-09-22 现态勘误）**：上方 Gantt 的日期为**参考序**，非承诺排期——原时间点（09-07 / 09-12 / 09-17 / 09-24 / 09-29）已作废，实际排期以 issue 为准。**Phase 2 控制面选项 A（B5 `DedupMergeEngine` 包现态 `start_log_scan -merge_files_list`）已落地**（见本文 §4 / ADR-0033 v1.5）；**Phase 3 仍未启动**；**包存储 §5.4 已触发（条件 4·多站点，ADR-0033 v1.11），实现跟踪 [#3075](https://github.com/DUElost/stability-test-platform/issues/3075)，本设计修订不实现代码**。**D0（源码不入仓）与 D3 的版本权威裁定不等包存储就绪，已即刻生效**；D2 契约（本文 §2）收窄为**新工具族准入要求**；本文 §3 的 manifest / tar.gz / `tools_cache/` 分发机制为**已触发、待实现**的终态。
