...
| `release.bundle` / `release.manifest` | `bundle` 在 I3 按**本地目录树**消费（须含 `release-manifest.json`、`backend/`、`backend/agent/`、`backend/schemas/`、`frontend/dist-prod/`、`deploy/`、`tools/`，离线时含 `wheelhouse/`）；`manifest`（I2 新增）为本地绝对清单路径，`validate` 允许为空、`plan`/`install` 必填。清单为 ≤1 MiB 的 JSON，拒绝未知字段/重复键；要求 `product.version`（须等于 `expected_release`）、`source.revision`、至少 `agent-code` 的 `sha256:<64hex>` 摘要（`host-resources` 自 ADR-0040 D8 R3 退役：新清单不再声明，旧清单仍可声明、照收不核验）、`database.schema_target`、`compatibility`（协议范围 + 逐发行版版本/架构支持矩阵）与 `provenance.attestation`（`signature` 或 `controlled_channel` 声明） | 控制面、前端 `dist-prod`、Agent、脚本/schema 及迁移的固定版本；I3 的 S0 用 ADR-0040 既有实现重算 agent-code / control-plane 摘要并与清单比对（不另造算法）；签名验签与来源渠道核验仍留给发布端与后续阶段 |
...
以下命令是这三个脚本背后的**权威接口**；在仓库根目录、已有 Python ≥ 3.10 环境中运行（下限唯一声明处是 `tools.site_config.preflight.MIN_PYTHON`；现场不足时 preflight 会以 `tool_python_version` 直接 FAIL）。依赖现有锁定环境内的 Pydantic v2 和 PyYAML：
...
| 支持面 | Debian 13 / Ubuntu 22.04（2026-09-15 实测纳入）/ 24.04、x86_64、systemd、Nginx、PostgreSQL、Redis；解释器下限 3.10（两个 venv 都由系统 `/usr/bin/python3` 建，故「平台在矩阵内」要求该平台自带解释器 ≥ `MIN_PYTHON`，#2268）；资源下限 2 核 / 4 GiB RAM / 根 ≥20 GiB。不支持 Kubernetes、开发 Compose 作生产入口、公网 `curl \| bash` |
...
| 中心存储（日志服务器） | 新建 Linux 分享或已有 NAS，NFS/CIFS、容量/权限与本地盘布局。演练观测**已过期**（#2845 起）：Agent 启动**不再**硬依赖 `STP_AEE_NFS_ROOT`——无根时 EventUploader / HddSpill 按「未配置」跳过（心跳正常但零上送，日志只留 `event_uploader_skipped cifs_root_empty`）。缺挂载因此**没有响亮失败兜底**，必须由安装链的挂载断言负责暴露：站点自建导出时 `install.s5.storage` 按 Agent 心跳的 `MOUNT_POINTS` fail-closed（`shared_storage_not_mounted`）；而外部分享站点（`storage.export_to_agents` 未开）在该链上**刻意不断言**（`blocked/export_not_enabled`）——这类站点的挂载缺口无人承接，需站点侧自行断言。本地目录替身只能用于隔离演练 | 不格式化、不创建未知分享、不假设所有挂载字符串都可原样覆盖；真实写入/读回单独授权 |
