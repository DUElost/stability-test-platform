...
1. **实体**：`test_suite` + `test_case`（粒度 = testpoint，`exec_descs` JSONB 含 1..N 执行描述）；
   `project_id` 可空 = 通用套件 / 必填 = 项目套件；套件级 `apk_binding`；快照列 `source_sha256`（导入时文件）+ `exported_sha256`（最近一次导出渲染产物，D5 比对键）。
...
4. **D2 绑定**：`plan_step.default_params` 显式声明 `suite_key` → 派发时 `inject_suite_params`（`inject_wifi_params` 同构）注入三字段 + 冻结 `plan_snapshot` / `run_context`；precheck 门禁四步 fail-fast 挂 admission 既有链。
...
| `exported_sha256` | str 可空 | **最近一次 export-to-tool-dir 渲染产物 sha**（D5 比对键；内容变更后置空，逼重导） |
...
| `PUT /api/v1/test-suites/{id}` | admin | 更新元数据；**改 `root_config`/`global_params`/`project_id`/`export_dir` → `exported_sha256` 置空**（内容变了须重导）；name 唯一校验 |
...
| `POST /api/v1/test-suites/{id}/cases` | admin | 单条创建（name/ordinal/times/enabled/exec_descs；schema 校验复用 `_validate_suite` 单条逻辑） |
| `PUT /api/v1/test-cases/{id}` | admin | 更新（name/ordinal/times/enabled/exec_descs 整覆盖）；改名后 `exported_sha256` 置空 |
| `DELETE /api/v1/test-cases/{id}` | admin | 硬删（P1 无历史结果引用；P2 `test_case_result` 引入后复议） |
| `POST /api/v1/test-suites/{id}/import` | admin | multipart：`file`=runtask.xml + `global`=UiAutomatorTestData.xml（可选）；**upsert 按 name**；更新 `source_sha256`/`root_config`/`global_params`/cases（ordinal 按文件顺序）；`exported_sha256` 置空；审计 |
...
**错误语义**：未知 suite 404；`exported_sha256` 已过期（库比磁盘新）在 `GET export` 响应头/`issues` 提示 `EXPORT_STALE`；跨项目访问套件？——套件 `project_id` 是**配置数据归属**（D3b 门禁在派发时），API 不按项目隔离读（登录即可看全部，同 ADR-0029 登记簿开放读）。
...
### 3.3 precheck 四步门禁（fail-fast）

挂 admission 既有链（`admission_pump.py` script_verify_failed 同层）：

1. suite 存在且 `is_active`（无 → `suite_verify_failed: missing`）；
2. 已导出：`exported_sha256` 非空 **且** `{NFS}/mtbf/{export_dir}/runtask.xml` 磁盘存在（无 → `suite_verify_failed: not_exported`）；
3. 磁盘文件 sha256 == `exported_sha256`（不等 → `suite_verify_failed: sha_mismatch`——「库改了没导出」在此拦截；脚本 setup 的 `suite_sha256` trace 与此闭环）；
4. **D3b**：`suite.project_id` 非空时与目标设备 `device.project_id` 匹配（不等 → `suite_verify_failed: project_mismatch`）；套件空 = 通用放行。

任一步失败 Plan 准入 `script_verify_failed` 同层失败，**禁止带病派发**（ADR-0030 D2）。
...
- 测试：migration + model + 端点（testcontainers）+ **渲染 golden**（导入生产 130 条快照 → 导出 → 与源文件逐字节除 CRLF/属性序外一致）+ 审计断言；
- **验收**：curl 走通「导入 130 条 → 改 1 条 → 导出渲染 → validate 0 error」，audit_logs 可见全操作。

### P1b：D2/D3b 绑定 + 门禁 + 脚本侧注入

- `inject_suite_params` + 冻结 run_context；admission 四步门禁 + 状态；
- `mtbf_setup`/`mtbf_check` 读注入参数（expected 替代 env；project 注入）；
- 测试：注入 golden（含已有值优先）、门禁矩阵（missing/not_exported/sha_mismatch/project_mismatch × admin 观察）、脚本侧解析 env 新键；
...
| 6 | 渲染字节保真 | 属性顺序不保证与源一致（ET 序）；golden 判据 = 解析语义等价 + `@@` 原样，非逐字节 |
