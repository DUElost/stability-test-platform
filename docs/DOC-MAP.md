...
| **执行契约** | [`development/ai/execution-contract.md`](./development/ai/execution-contract.md) | AI Execution Contract 唯一权威源（Registry 协议/三维状态与 transition table/scope 谓词/test_impact/启动判据/实现与契约先后纪律/竞争提案可见性与决策实体唯一性；Living v1.8（§3.5 决策实体唯一性 + 决策类必须 `--issue` 09-09 / §8 并发上限反转 09-08 / §1.2 role 缺省归一化+再开启条件 09-08 / §10 先后纪律 09-08 / v1.3 T9 `resume` / v1.2 issue 查重），ADR-0034 P0a 交付） |
