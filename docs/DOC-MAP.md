...
| **架构 ADR** | [`adr/ADR-0047-db-pool-and-connection-capacity.md`](./adr/ADR-0047-db-pool-and-connection-capacity.md) | 控制面 DB 连接池与 PostgreSQL `max_connections` 的容量取向（#703 第 ② 面 / #2959）：原默认两份池（sync+async 各 30+60）峰值 180 vs 可用 97。**Accepted v1.2**（v1.1 于 2026-09-23 owner 裁决 D1/D2/D5/D6；v1.2 于 2026-09-25 §4 校准回填——模拟 #3243 + 生产真机 run 556 两层证据维持 `20/20`、`reserve=8`、`2s`，门禁已实装、告警已生效）：D1 总量不变量（含 `n_instances` 与预留项）成为**启动期硬门禁**、预算不成立即拒启；D2 `pool_timeout=2s` 且槽耗尽/池超时统一对外 **503 + `Retry-After`**；D5 告警落事件侧立即触发；首轮值 sync/async 各 20+20=40、reserve=8（**已校准确认维持**）。D3 双池合并 / D4 pgbouncer 保留开放并写明复评条件） |
| **架构 ADR** | [`adr/ADR-0051-release-unit-and-content-addressing.md`](./adr/ADR-0051-release-unit-and-content-addressing.md) | 发布单元与内容寻址——不可变性从源码目录移到包：脚本 / 外部工具 / 控制面统一为内容寻址 artifact（复用 ADR-0040 digest），每族一棵源码树、版本目录退役，Agent 经 `tools_cache` 拉包（新建「DB 权威→包身份」一环、解三处 `nfs_path` 形态耦合），控制面部署源切 bundle 归档树（接管 ADR-0046，选 B）。**Accepted**（v1.2 2026-09-23 Phase 3 落地：目录删除 / 族树 / manifest+包注册 / 门禁退役，fleet 48/48 strict；此前已勘误 Phase 3 依赖并落地 Phase 2b；2026-09-22 初版 owner 全采推荐项；§2 回应 ADR-0039 D6；继承其 D2/D3/D4/D5/D7；接管 ADR-0046；修订 ADR-0033 D3 采 C1；§9 四组机械改动已落地；Phase 1–5 待排期）；v1.3 2026-09-24 族级 kind 字段（script/tool 唯一归类判据，收编 ghost 保护；v1.4 落地审查修复（bootstrap 零冲突 + strict 默认） |
| **架构 ADR** | [`adr/ADR-0052-terminal-fact-parent-aggregation-decoupling.md`](./adr/ADR-0052-terminal-fact-parent-aggregation-decoupling.md) | 终态事实与父 Run 聚合解耦——Job 事务不写父级热行（#2959 / #3244）：R523 的放大回路已由 P0（[ADR-0047](./adr/ADR-0047-db-pool-and-connection-capacity.md)：预算门禁 / 503 / 舱壁）止血，**剩余热点**是每个终态都锁并改写同一条 `plan_run`（#3243 校准：被接纳的 `/complete` p99≈1.1s、p50≈0.55s≈舱壁等待）。**Accepted v1.0**（2026-09-24 起草，2026-09-25 裁决：D1–D5 接受并开始实现、D6 延后）：D1 终态事务移出父级聚合行（含删逐 Job `acknowledged_job_ids` 写）；D2 durable pending 标记 + 提交后唤醒（Redis 仅传输）；D3 按 `plan_run_id` 合并聚合、单 Run 单聚合者、读事实重算、at-least-once；D4 副作用在父终态提交后且幂等；D6 post_completion 独立队列（可单独延后）。**替代 ADR-0026 §6 两处**（终态事务内自增计数 / 每 Job 持父行锁），保留单一 terminalization 入口与对账 sweep；原 6 条真机门槛改为**实施验收门槛**（② 基线取真机 2.467s）。部署衔接：[`2026-09-24-capacity-p0-rollout-runbook`](./operations/2026-09-24-capacity-p0-rollout-runbook.md) |
