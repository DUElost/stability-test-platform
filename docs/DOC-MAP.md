...
| **架构 ADR** | [`adr/ADR-0047-db-pool-and-connection-capacity.md`](./adr/ADR-0047-db-pool-and-connection-capacity.md) | 控制面 DB 连接池与 PostgreSQL `max_connections` 的容量取向（#703 第 ② 面 / #2959）：原默认两份池（sync+async 各 30+60）峰值 180 vs 可用 97。**Accepted v1.1**（2026-09-23 owner 裁决 D1/D2/D5/D6）：D1 总量不变量（含 `n_instances` 与预留项）成为**启动期硬门禁**、预算不成立即拒启；D2 `pool_timeout=2s` 且槽耗尽/池超时统一对外 **503 + `Retry-After`**；D5 告警落事件侧立即触发；首轮值 sync/async 各 20+20=40、reserve=8（须隔离压测校准）。D3 双池合并 / D4 pgbouncer 保留开放并写明复评条件） | 
