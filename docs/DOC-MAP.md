...
| **架构 ADR** | [`adr/ADR-0051-release-unit-and-content-addressing.md`](./adr/ADR-0051-release-unit-and-content-addressing.md) | 发布单元与内容寻址——不可变性从源码目录移到包：脚本 / 外部工具 / 控制面统一为内容寻址 artifact（复用 ADR-0040 digest），每族一棵源码树、版本目录退役，Agent 经 `tools_cache` 拉包（新建「DB 权威→包身份」一环、解三处 `nfs_path` 形态耦合），控制面部署源切 bundle 归档树（接管 ADR-0046，选 B）。**Proposed v0.1**（2026-09-22 起草，待 owner 裁决；§2 回应 ADR-0039 D6；继承其 D2/D3/D4/D5/D7；修订 ADR-0033 D3 采 C1） |
