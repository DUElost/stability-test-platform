...
| **架构 ADR** | [`adr/ADR-0042-settings-convergence-and-bare-read-boundary.md`](./adr/ADR-0042-settings-convergence-and-bare-read-boundary.md) | 配置读取收敛——分域 pydantic-settings 与裸读取边界（325 处裸读 / 70 处 import-time 固化的收敛判据；env 名不变 + 惰性访问 + 清单门禁联动；**Accepted v1.0**（2026-09-14 裁决引入）；#737 deferred） |
