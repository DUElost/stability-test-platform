...
| **架构 ADR** | [`adr/ADR-0033-tool-kit-ecosystem-integration.md`](./adr/ADR-0033-tool-kit-ecosystem-integration.md) | 外部工具统一接入契约规范与包管理解耦模型（三层宿主/Tool Contract/Manifest×DB catalog/防腐适配器/与 ADR-0032 权威分家；D0·D3 即刻生效、包存储条件落地；**落地状态：部分落地**——Phase 2 选项 A：B5 `DedupMergeEngine` 包现态 `-merge_files_list`（GT 留 B2）；Accepted v1.5；#745/#2546） |
...
| **设计** | [`design/2026-semantic-ownership.md`](./design/2026-semantic-ownership.md) | 跨域语义归属索引（Ownership Authority only；Accepted ADR 覆盖框架；B/C 核心能力深嵌 + A/D 编排外置；X1/X2/X3 + S15 表内判据；日志 X2 地图路由 → `2026-device-log-chain-contract`；**Draft**，#2546 / PR #2751） |
