...
| **架构 ADR** | [`adr/ADR-0035-agent-secret-host-boundary.md`](./adr/ADR-0035-agent-secret-host-boundary.md) | Agent 主机级凭据边界——共享 AGENT_SECRET 风险接受与升级触发（接受全局共享为受信域内认证原语；泄露后冒充面收窄（fencing/SID CAS/审计）+ 主机级凭据升级触发条件与设计骨架；ops token 与主机凭据分界；**Accepted v1.0**；#906） |
