...
| **Living 审查** | [`reviews/MTBF_MULTI_CASE_RESEARCH_2026-08-19.md`](./reviews/MTBF_MULTI_CASE_RESEARCH_2026-08-19.md) | MTBF 多用例平台化研究（runtask.xml 实测 + 平台缺口 G1–G5 + 候选形态 A/B/C + 设计草图；[ADR-0030](./adr/ADR-0030-multi-case-suite-management.md) 背景分析，Accepted v1.10） |
...
| **Living 审查** | [`reviews/SCRIPT_VERSION_BLOAT_ENDGAME_FEASIBILITY_2026-09-10.md`](./reviews/SCRIPT_VERSION_BLOAT_ENDGAME_FEASIBILITY_2026-09-10.md) | 脚本版本膨胀终态可行性（#735 长效机制）：复制冗余 78% 量化 + 四条路径裁决（P2 共享库不做、P1「不可变收窄至零引用退役」是唯一需新 ADR 项、P3 归 ADR-0033 D3 轨道）；**2026-09-13 后续**：§4 两项只读统计已完成、P1 已立 [ADR-0039](./adr/ADR-0039-script-version-immutability-narrowing.md)（零引用目录占版本目录行数 63.0%；零引用版本在 `plan_step` 中出现数为 0） |
...
| **Living 审查** | [`reviews/PLATFORM_AUDIT_2026-09-11.md`](./reviews/PLATFORM_AUDIT_2026-09-11.md) | 平台整体只读审查（四维度并行：架构 6/10·代码质量 7.5/10·测试与 CI 8/10·需求与文档；总分 6.8/10，初基线 `ca0665c4`，合入前复检基线 `dfbeb2ef`）：**2 Critical**（同步连接池未配容量参数 / RunConsole 进程内单例未登记为扩展阻塞点）+ **9 High**（心跳超时默认值多处重复定义、`services→api.routes` 反向依赖、God-module 路由、NFS 原始日志零 TTL、HddSpill 腾退不足、`merge_task` 全失败无终态、ADR-0031 ID 冲突、ADR 状态标注碎片化、PR 门禁不覆盖控制面测试、`action_templates` 死代码）；**风险台账 R-01~R-09 复核（09-11 快照）：完整修复 0 条 / 部分缓解 2 条 / 仍存在 7 条**（⚠️ 该计数为**报告基线时点**结论，**已过期**——R-01 的 NFS 轨已于 2026-09-12 由 PR #1596 闭环、R-02 的同步池已由 #1516 闭环，故现应为 **2 / 1 / 6**（仍存在：R-03、R-05~R-09）；详见 issue #1515 的「只读审计」评论。正本 `2026-09-03-platform-risk-register.md` 只记客观事实、不含修复状态，无需同步）；**合入前逐条复检：11 条仍成立 / C-02·H-01·H-04 部分或形态变化 / H-09 已由上游 `56234cb3` 修复**；前序基线见 `PLATFORM_HEALTH_REVIEW_2026-09-03.md`（本轮为其后 815 提交复检） |
...
| **架构 ADR** | [`adr/ADR-0033-tool-kit-ecosystem-integration.md`](./adr/ADR-0033-tool-kit-ecosystem-integration.md) | 外部工具统一接入契约规范与包管理解耦模型（三层宿主/Tool Contract/Manifest×DB catalog/防腐适配器/与 ADR-0032 权威分家；D0·D3 即刻生效、包存储条件落地；**落地状态：部分落地**——Phase 2 B5；Phase A D0/Contract/Jira ACL（#3005）；Phase A3 下载+#3015 + follow-ups #3028/#3013；§5.4 包存储评估 **已触发**（条件 4·多站点；Phase B 第一切片机械面已落地（v1.12，fleet 未切换）；[`2026-09-22-adr0033-package-store-multisite-trigger`](./notes/architecture/2026-09-22-adr0033-package-store-multisite-trigger.md)；实现跟踪 [#3075](https://github.com/DUElost/stability-test-platform/issues/3075)）；§5.6 **D0 可拦对象口径**=带外部资产的族（#3014 案 3A）；Accepted v1.15（v1.13：D3 措辞改采 C1 双列，由 ADR-0051 D3 裁决；D0 机械门禁改由族级 kind 登记承担，v1.14；D3 按对象拆分回写——kind=script 入 catalog、kind=tool 不建 script 行，v1.15 / #3203）；#745/#2546/#3075） |
...
| **架构 ADR** | [`adr/ADR-0037-agent-host-privilege-boundary.md`](./adr/ADR-0037-agent-host-privilege-boundary.md) | Agent 主机提权边界（单入口 wrapper `/usr/local/sbin/stp-agent-priv` + 子命令白名单/路径属主校验 + 不动 Ansible 密码 become；**Accepted v0.6**（v0.2：§1.2 勘误（存量宽 sudoers 与 flash 修复分支仍在用）+ D5 flash 链运行时提权收敛（`ensure-udev-rule`/`usb-authorized` 窄子命令 + provisioning 归位）+ §5 退役判据收严；v0.3：迁移期结束——legacy fallback 与哨兵退役、热更新 fail-closed，#2180；v0.4：**转 Accepted**——R02 评审一稿已交付（#2206/PR #2209），S1/O1/O2 已落地、S2/S3/O4 待排期；R14-F04 #1250；实现与全 fleet 迁移已落地，#2133/#2134；v0.5：§2 D2 勘误——`selftest` 是自指判据（只证 wrapper 自洽），脚本头补 `capabilities` 能力前置判据，#2319；v0.6：D7 内核日志只读子命令 `read-kernel-log`（#2957：两个整数参数 + 固定 argv + 环境清空/30s 超时/8 MiB 上限，截断即非零退出；首扫改最近 1 小时窗））） |
| **架构 ADR** | [`adr/ADR-0038-host-retirement-semantics.md`](./adr/ADR-0038-host-retirement-semantics.md) | 主机退役语义（`retired_at` 生命周期真源 + reason 必填/审计 fail-closed / DELETE 与 retire 分家（Cordon 前置）/ 退役即终态 / 心跳如实记录 + 新列去重 / 14 面收口 + 派发归位 fatal + 在飞 Run 显式收敛 / 身份契约（id 判定、ip 可变）/ retire ≠ credential revoke；**Accepted v0.2（2026-09-13 定稿：9 稿评审 synthesis `reviews/REVIEW_ADR0038_2026-09-13_519e346_synthesis.md` + D-1～D-6 裁决 + 模型归属 errata）**；#796/#937 触发）；**v0.3（2026-09-25 裁决生效，§7.7）：§7 D9 设备面意图（空置/人工清空）+ UsbBlind/AdbOfflineConcentration 规则侧豁免 + D9.8 意图陈旧告警，现网实例 #3065、实施单 #3159（未实施）** |
| **架构 ADR** | [`adr/ADR-0042-settings-convergence-and-bare-read-boundary.md`](./adr/ADR-0042-settings-convergence-and-bare-read-boundary.md) | 配置读取收敛——分域 pydantic-settings 与裸读取边界（325 处裸读 / 70 处 import-time 固化的收敛判据；env 名不变 + 惰性访问 + 清单门禁联动；**Accepted**（2026-09-14 P1 试点完成：依赖+D6+调度域 21 旋钮+agent 租约域 5 旋钮；v1.2（2026-09-18）回填 P2 已落地四片、P2 未收口，#2661；v1.3（2026-09-23）D3 加 ADR-0033 §5.4 legacy 例外键终态出口豁免（ADR-0051 D7 裁决、其 Phase 4 前置））；#737） |
| **架构 ADR** | [`adr/ADR-0046-control-plane-checkout-roles.md`](./adr/ADR-0046-control-plane-checkout-roles.md) | 控制面检出的角色分离——开发工作区 vs 部署源：脚本 catalog / 主机 Agent 热更新 / 派发期补推三条路径读同一棵检出树，而该树同时是多会话开发工作区；`check-deploy-source.sh` 是「操作者自觉」级守卫（systemd 用 `ExecStartPre=-` 失败也继续）。**Superseded**（2026-09-22 由 ADR-0051 D6 接管：D1 是 / D2 退役显式 / D3 同根 / D4 部署动作推进 / D5 选 B / D6 审计落 digest；正文保留为判据与事故记录；#1987 / #2386） |
| **架构 ADR** | [`adr/ADR-0047-db-pool-and-connection-capacity.md`](./adr/ADR-0047-db-pool-and-connection-capacity.md) | 控制面 DB 连接池与 PostgreSQL `max_connections` 的容量取向（#703 第 ② 面 / #2959）：原默认两份池（sync+async 各 30+60）峰值 180 vs 可用 97。**Accepted v1.3**（v1.3 于 2026-09-25 裁决 D3/D4；v1.1 于 2026-09-23 owner 裁决 D1/D2/D5/D6；v1.2 于 2026-09-25 §4 校准回填——模拟 #3243 + 生产真机 run 556 两层证据维持 `20/20`、`reserve=8`、`2s`，门禁已实装、告警已生效）：D1 总量不变量（含 `n_instances` 与预留项）成为**启动期硬门禁**、预算不成立即拒启；D2 `pool_timeout=2s` 且槽耗尽/池超时统一对外 **503 + `Retry-After`**；D5 告警落事件侧立即触发；首轮值 sync/async 各 20+20=40、reserve=8（**已校准确认维持**）。D3 双池不合并 / D4 不引入 pgbouncer（v1.3 裁决，均带复评触发器）） |
| **架构 ADR** | [`adr/ADR-0051-release-unit-and-content-addressing.md`](./adr/ADR-0051-release-unit-and-content-addressing.md) | 发布单元与内容寻址——不可变性从源码目录移到包。**Accepted v1.8，落地未完结**：Phase 0/2a/2b/3 已落地；现站 48/48 包模式、在位缺口 0；D6 闭合（生产 env 真身落站点 `stp-releases/env.backend`，运行时不触开发检出）；Phase 1/4/5 余部与过渡退出仍待收口。详见正文状态与 [2026-09-25 生产验收](./notes/process/2026-09-25-adr0051-production-acceptance.md)。 |
| **架构 ADR** | [`adr/ADR-0052-terminal-fact-parent-aggregation-decoupling.md`](./adr/ADR-0052-terminal-fact-parent-aggregation-decoupling.md) | 终态事实与父 Run 聚合解耦——Job 事务不写父级热行（#2959 / #3244）：R523 的放大回路已由 P0（[ADR-0047](./adr/ADR-0047-db-pool-and-connection-capacity.md)：预算门禁 / 503 / 舱壁）止血，**剩余热点**是每个终态都锁并改写同一条 `plan_run`（#3243 校准：被接纳的 `/complete` p99≈1.1s、p50≈0.55s≈舱壁等待）。**Accepted v1.1**（2026-09-24 起草，2026-09-25 裁决：D1–D5 接受并开始实现；v1.1 D6 不采纳（带复议触发器）、§7 开放问题定值）：D1 终态事务移出父级聚合行（含删逐 Job `acknowledged_job_ids` 写）；D2 durable pending 标记 + 提交后唤醒（Redis 仅传输）；D3 按 `plan_run_id` 合并聚合、单 Run 单聚合者、读事实重算、at-least-once；D4 副作用在父终态提交后且幂等；D6 post_completion 独立队列（v1.1 不采纳）。**替代 ADR-0026 §6 两处**（终态事务内自增计数 / 每 Job 持父行锁），保留单一 terminalization 入口与对账 sweep；原 6 条真机门槛改为**实施验收门槛**（② 基线取真机 2.467s）。部署衔接：[`2026-09-24-capacity-p0-rollout-runbook`](./operations/2026-09-24-capacity-p0-rollout-runbook.md) |
...
| **设计** | [`design/2026-semantic-ownership.md`](./design/2026-semantic-ownership.md) | 跨域语义归属索引（Ownership Authority only；Accepted ADR 覆盖框架；B/C 核心能力深嵌 + A/D 编排外置；X1/X2/X3 + S15 表内判据；日志 X2 地图路由 → `2026-device-log-chain-contract`；**Living**，#2546 Closed） |
| **设计** | [`design/2026-device-log-chain-contract.md`](./design/2026-device-log-chain-contract.md) | 设备日志链路 Living Contract（全链地图 + 逻辑坐标 + Central namespace 映射；**非 ADR**；路由入口而非第五 owner；细节展开见 `2026-log-chain-global-semantics.md`；**Draft**，#2546） |
