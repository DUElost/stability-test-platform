...
| **架构 ADR** | [`adr/ADR-0052-terminal-fact-parent-aggregation-decoupling.md`](./adr/ADR-0052-terminal-fact-parent-aggregation-decoupling.md) | 终态事实与父 Run 聚合解耦——Job 事务不写父级热行（#2959 / #3244）：R523 的放大回路已由 P0（[ADR-0047](./adr/ADR-0047-db-pool-and-connection-capacity.md)：预算门禁 / 503 / 舱壁）止血，**剩余热点**是每个终态都锁并改写同一条 `plan_run`（#3243 校准：被接纳的 `/complete` p99≈1.1s、p50≈0.55s≈舱壁等待）。**Proposed v0.1**（2026-09-24 起草；owner 口径：先 ADR 后实现）：D1 终态事务移出父级聚合行（含删逐 Job `acknowledged_job_ids` 写）；D2 durable pending 标记 + 提交后唤醒（Redis 仅传输）；D3 按 `plan_run_id` 合并聚合、单 Run 单聚合者、读事实重算、at-least-once；D4 副作用在父终态提交后且幂等；D6 post_completion 独立队列（可单独延后）。**替代 ADR-0026 §6 两处**（终态事务内自增计数 / 每 Job 持父行锁），保留单一 terminalization 入口与对账 sweep；**转 Accepted 须真机复跑 6 条达标**。部署衔接：[`2026-09-24-capacity-p0-rollout-runbook`](./operations/2026-09-24-capacity-p0-rollout-runbook.md) |
