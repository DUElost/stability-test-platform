...
| **架构 ADR** | [`adr/ADR-0033-tool-kit-ecosystem-integration.md`](./adr/ADR-0033-tool-kit-ecosystem-integration.md) | 外部工具统一接入契约规范与包管理解耦模型（三层宿主/Tool Contract/Manifest×DB catalog/防腐适配器/与 ADR-0032 权威分家；D0·D3 即刻生效、包存储条件落地；**落地状态：部分落地**——Phase 2 B5；Phase A D0/Contract/Jira ACL（#3005）；Phase A3 PlanRunArtifact 下载（#3015/#3013）；§5.4 包存储评估 **未触发**（[`2026-09-20-adr0033-package-store-trigger-assessment`](./notes/architecture/2026-09-20-adr0033-package-store-trigger-assessment.md)）；§5.6 **D0 可拦对象口径**=带外部资产的族（#3014 案 3A）；Accepted v1.9；#745/#2546） |
