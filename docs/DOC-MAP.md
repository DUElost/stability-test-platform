...
| **架构 ADR** | [`adr/ADR-0037-agent-host-privilege-boundary.md`](./adr/ADR-0037-agent-host-privilege-boundary.md) | Agent 主机提权边界（单入口 wrapper `/usr/local/sbin/stp-agent-priv` + 子命令白名单/路径属主校验 + 迁移期 legacy fallback 与 `priv_mode` 哨兵 + 不动 Ansible 密码 become；**Proposed v0.1**（R14-F04 #1250，待 R02 联审；实现与全 fleet 迁移已落地）） |
