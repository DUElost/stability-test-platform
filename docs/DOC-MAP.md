...
| **架构 ADR** | [`adr/ADR-0035-agent-host-identity.md`](./adr/ADR-0035-agent-host-identity.md) | Agent 主机身份与凭据体系（全局 AGENT_SECRET+自报 host_id 的冒充面/四方案对比/推荐 A 每主机凭据+C 注册质询引入；**Proposed 待裁决**；R02-R01/#906） |
...
| **执行契约** | [`development/ai/execution-contract.md`](./development/ai/execution-contract.md) | AI Execution Contract 唯一权威源（Registry 协议/三维状态与 transition table/scope 谓词/test_impact/启动判据/实现与契约先后纪律；Living v1.7（§8 并发上限反转 09-08 / §1.2 role 缺省归一化+再开启条件 09-08 / §10 先后纪律 09-08 / v1.3 T9 `resume` / v1.2 issue 查重），ADR-0034 P0a 交付） || **设计** | [`design/2026-09-external-tools-integration-and-package-architecture.md`](./design/2026-09-external-tools-integration-and-package-architecture.md) | 外部工具统一接入架构与包管理实施计划（ADR-0033 配套：协议定义与 §2.5 双轨衔接/§3.3 注册流与门禁分工/NFS 布局/Agent 缓存/去重+专项适配器/三阶段排期） |
