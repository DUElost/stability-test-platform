...
**Goal:** 在 `203.0.113.36` 上交付基于 Ansible 的 Linux agent host 首次部署、热更新、服务管理和状态检查闭环，并保留后续原生迁移到 role/tasks 的扩展点。

**Architecture:** 以 `tools/ansible/` 作为唯一运维入口，新增 `group_vars`、`host_vars` 和四个 playbook。首次安装阶段先桥接现有 `install_agent.sh`，避免一次性重写高风险逻辑；热更新、服务管理和状态检查直接用 Ansible tasks 落地，减少对人工 SSH 的依赖。共享变量、排除列表和 handlers 放入 `tools/ansible/roles/agent_deploy/`，为 Phase B 的原生迁移保留稳定边界。
...
**Files:**
- Create: `tools/ansible/group_vars/linux_hosts.yml`
- Create: `tools/ansible/host_vars/203.0.113.36.yml`
- Create: `tools/ansible/roles/agent_deploy/defaults/main.yml`
- Create: `tools/ansible/roles/agent_deploy/handlers/main.yml`
- Test: `wsl bash -lc 'cd /mnt/f/stability-test-platform/tools/ansible && ANSIBLE_CONFIG=./ansible.cfg ansible-inventory -i inventory.ini --host 203.0.113.36'`
...
```bash
wsl bash -lc 'cd /mnt/f/stability-test-platform/tools/ansible && ANSIBLE_CONFIG=./ansible.cfg ansible-inventory -i inventory.ini --host 203.0.113.36'
```
...
```yaml
agent_api_url: "http://203.0.113.15:8000"
...
```bash
git add tools/ansible/group_vars/linux_hosts.yml tools/ansible/host_vars/203.0.113.36.yml tools/ansible/roles/agent_deploy/defaults/main.yml tools/ansible/roles/agent_deploy/handlers/main.yml
...
**Files:**
- Create: `tools/ansible/playbooks/service_agent.yml`
- Test: `wsl bash -lc 'cd /mnt/f/stability-test-platform/tools/ansible && ANSIBLE_CONFIG=./ansible.cfg ansible-playbook playbooks/service_agent.yml --syntax-check'`
...
```bash
wsl bash -lc 'cd /mnt/f/stability-test-platform/tools/ansible && ANSIBLE_CONFIG=./ansible.cfg ansible-playbook playbooks/service_agent.yml --syntax-check'
```
...
```bash
wsl bash -lc "cd /mnt/f/stability-test-platform/tools/ansible && ANSIBLE_CONFIG=./ansible.cfg ansible-playbook playbooks/service_agent.yml --limit 203.0.113.36 -e agent_service_action=status"
```
...
```bash
git add tools/ansible/playbooks/service_agent.yml
...
**Files:**
- Create: `tools/ansible/playbooks/check_agent.yml`
- Test: `wsl bash -lc 'cd /mnt/f/stability-test-platform/tools/ansible && ANSIBLE_CONFIG=./ansible.cfg ansible-playbook playbooks/check_agent.yml --syntax-check'`
...
```bash
wsl bash -lc 'cd /mnt/f/stability-test-platform/tools/ansible && ANSIBLE_CONFIG=./ansible.cfg ansible-playbook playbooks/check_agent.yml --syntax-check'
```
...
```bash
git add tools/ansible/playbooks/check_agent.yml
...
**Files:**
- Create: `tools/ansible/playbooks/install_agent.yml`
- Test: `wsl bash -lc 'cd /mnt/f/stability-test-platform/tools/ansible && ANSIBLE_CONFIG=./ansible.cfg ansible-playbook playbooks/install_agent.yml --syntax-check'`
...
```bash
wsl bash -lc 'cd /mnt/f/stability-test-platform/tools/ansible && ANSIBLE_CONFIG=./ansible.cfg ansible-playbook playbooks/install_agent.yml --syntax-check'
```
...
```bash
git add tools/ansible/playbooks/install_agent.yml
...
**Files:**
- Create: `tools/ansible/playbooks/update_agent.yml`
- Test: `wsl bash -lc 'cd /mnt/f/stability-test-platform/tools/ansible && ANSIBLE_CONFIG=./ansible.cfg ansible-playbook playbooks/update_agent.yml --syntax-check'`
...
```bash
wsl bash -lc 'cd /mnt/f/stability-test-platform/tools/ansible && ANSIBLE_CONFIG=./ansible.cfg ansible-playbook playbooks/update_agent.yml --syntax-check'
```
...
```bash
wsl bash -lc "cd /mnt/f/stability-test-platform/tools/ansible && ANSIBLE_CONFIG=./ansible.cfg ansible-playbook playbooks/check_agent.yml --limit 203.0.113.36"
wsl bash -lc "cd /mnt/f/stability-test-platform/tools/ansible && ANSIBLE_CONFIG=./ansible.cfg ansible-playbook playbooks/service_agent.yml --limit 203.0.113.36 -e agent_service_action=restart"
```
...
```bash
git add tools/ansible/playbooks/update_agent.yml
...
- [ ] **Step 2: 新增 `tools/ansible/README.md`，收敛 Ansible 运维命令**
...
```bash
wsl bash -lc 'cd /mnt/f/stability-test-platform/tools/ansible && ANSIBLE_CONFIG=./ansible.cfg ansible-playbook playbooks/install_agent.yml --syntax-check && ANSIBLE_CONFIG=./ansible.cfg ansible-playbook playbooks/update_agent.yml --syntax-check && ANSIBLE_CONFIG=./ansible.cfg ansible-playbook playbooks/service_agent.yml --syntax-check && ANSIBLE_CONFIG=./ansible.cfg ansible-playbook playbooks/check_agent.yml --syntax-check'
```
...
```bash
git add tools/ansible/README.md backend/agent/DEPLOY.md
...
本计划只覆盖 Phase A 单机闭环和 Phase B 的结构扩展点。完成 `203.0.113.36` 验收后，应基于真实运行结果再写一份新计划，将 `install_agent.sh` 的桥接安装迁移为原生 `role/tasks/template/systemd` 实现，并补充批量滚动执行策略。
