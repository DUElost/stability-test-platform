...
| AGENTS「scan/upload 跨进程契约」保持常驻（5.4KB） | 段落自述「始终加载」为有意决策；跨进程排障需契约在场，盲降级留 dedup 盲区 | 攒 4 周使用样本：排障场景高频则评估懒加载化（子文档+指针）〔2026-08-27 瘦身评估 C1〕 |
| AGENTS「Production access」保持常驻（2.1KB） | 安全护栏——凭据位置永远在场防越权摸索 | 无（除非凭据源本身迁移）〔C2〕 |
| AGENTS「Key conventions」保持常驻（4.7KB） | 近期事故线有意沉淀的活契约 | CI/gate 机制大改时随之更新而非删除〔C3〕 |
| CLAUDE「脚本目录契约」保持常驻（1.6KB） | ef8808e 高频复发区，警告密度换事故率 | 同上〔C4〕 |
...
| 2026-08-27 | 常驻上下文瘦身 A 级三项执行落地（plan-c/cursor-rules import 降级 + 决策记录单行化，−13KB/−23%）；「刻意不追」续页登记 C1–C4 四项显式保留；B1 DOC-MAP 迁移立项待独立 PR。评估全文见 `reviews/RESIDENT_CONTEXT_AUDIT_2026-08-27.md` |
