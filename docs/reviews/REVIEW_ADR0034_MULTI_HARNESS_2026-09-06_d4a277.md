...
| 观察 | 提出处 | 去向 |
|---|---|---|
| lifecycle 缺 `FINISHED→CODING` 回退 | §9.1 | ✅ **已修，#983 / #946 CLOSED（2026-09-07 16:16）**——契约 v1.3 §3.3 增 **T9 `resume`**（FINISHED→CODING，T8 的 lifecycle 侧对称）；守卫：`MERGED` 拒绝、`ABANDONED` 不可 resume。E2E 实测：`declare→finish→resume→CODING→abandon` 全通，ABANDONED 后 resume 正确 REFUSED |
...
| 僵尸候选判据在并集语义下几乎不触发 | §13.2 | ✅ **已修，#962 CLOSED（2026-09-07 15:23）**——死代码实证：declare `--scope` required、update 只替换不清空 → effective 恒非空 → 判据不可达 |
...
**回链更新（2026-09-07 晚，T9 落地后）**：第 1、4 行均已完结（#983 / #962 合入并关单），
四条观察中仅第 3 行（`test_impact` 召回率）维持⬜——它是条件触发型观察，
「长期空转再立单」，现在立单为时过早。

### 20.5 closing keyword 失效的真实原因（**归因修正**）

本轮曾把 #926/#929/#931/#963 合入后 issue 未自动关闭归因于「**API 抖动**」。
**该归因错误，此处更正。**

**为什么不是 API 抖动**：closing keyword 的解析与关单**完全在服务端**完成，
与本机 API 可达性无关——本机断网不影响它，本机畅通也不会让它更可靠。

**真实原因（四单全同型）**：issue 号后紧跟全角句号、无词边界——

```
#926 │ Closes #855。按第…
#929 │ Closes #928。declaration-…
#931 │ Closes #930。Gi…
#963 │ Closes #962。#9…
```

四单关键词均在 body 首行、均不在代码围栏内（已逐行核验围栏状态），
唯一共性是 `#[0-9]+` 后无词边界 → GitHub 不解析 issue 引用。

**受控验证**：#983 改用 `Closes #946. `（ASCII 句号 + 空格）后，
#946 于 **16:16:48Z 自动关闭**。前后对照 **4/4 失败 → 1/1 成功**，诊断成立。

**正确姿势**：issue 号后补空格或 ASCII 标点（`#946. `）；失效先查**写法**
（词边界 / 代码围栏 / base 是否默认分支），**不要先查网络**。

> 这条与本文件 §20.2 记录的错误是**同一类**——拿表层信号直接推原因、未验证机制。
> §20.2 是核查者（本会话）犯的，本节是流程侧犯的；两类都值得留档。
