...
**路由表位置（v2.2 定稿，防二义）**：路由表住**工具目录**（`test-config.properties` 等，
与专项工具同目录），**不在 `STP_SCRIPT_ROOT` 下**——跟随先例（族映射与 APK 本来就在
工具目录，拆进脚本目录会切断与工具链的联动），因此**不受 ADR-0020 版本不可变约束**：
改一行映射即生效，无版本、无 sha、无审计。此边界的留痕补偿见配套约定 #2（step_trace
记路由表 sha）：平台能回答「映射何时变了、run #212 用了哪个 APK」，回答不了「谁改的」——
共享挂载的写者由文件系统审计/操作规范负责，平台侧以 sha 变化告警兜底（同快照两次 run
结果不同时可归因）。

**版本膨胀基线仍然成立**（§2.4：monkey_setup 13 版本、38 个活跃版本），但那是
「迭代被迫复制」；逐族 APK 差异不再走版本复制路径——新族接入是**改路由表**（工具目录内
一行配置，比复制整份脚本好一个数量级），不是逐项目整份脚本。

**路由方案的配套约定**（脚本目录契约，不涉平台协议）：

1. **未匹配 fail-fast**：入口脚本对不认识指纹/能力的设备立即报明确错误（如
   `no executor for model X in <专项> v<ver>`），不进假跑
2. **路由决策可观测（含映射留痕）**：step_trace 记录实际使用的执行器 / APK，**并记录
   路由表文件（`test-config.properties` 等）的 sha256**——同一快照两次 run 结果不同时，
   对照 sha 可归因「映射被改」；sha 变化同时触发告警（detect 不出写者，但变化必现）
...
| B2 | 入口脚本未匹配设备执行器时 **fail-fast**：明确错误码 + step_trace 记录实际路由到的执行器 / APK **及路由表文件 sha256**（同快照两次 run 结果不同可归因「映射被改」） | 并行（脚本路由） |
