...
> **最后更新**：2026-09-16（§5/§5.1：R01–R11 动态验证升「已完成」）
>
...
2. **并发与恢复**：重入、幂等、竞态、重复 / 乱序、超时、取消、崩溃恢复与资源释放。
   **并发这一项必须额外产出「共享行加锁表」**（见第 7 条）：只按本区路径自查只能证明
   每条路径内部自洽，看不到跨路径的环。
...
7. **共享行加锁全序**：以**共享行 / 资源**为单位（本平台的最小集：
   `job_instance`、`device_leases`、`plan_run`、`plan_run_host`、`host`）穷尽列出
   全部加锁点及其获取顺序，断言全局存在一致的全序；出现相反顺序即判 P1。
   **枚举单位必须是共享行，不是业务路径**：路径分区天然看不到环——同一批行被两条
   并发路径以相反顺序加锁时，每条路径在自己的区里都是自洽的。共享行数量有限，
   属可穷尽面，不需要穷举并发交错。既有先例见
   `_cas_renew_leases` 的调用契约（`agent_api.py`，「先 Job 再 Lease」）。
8. **方法能力边界**：静态只读审查**必须**登记一份「运行期方可成立的断言」清单
   （锁序环、活锁、连接池耗尽、超时收敛、真实并发交错、真机 / NFS 行为等），标注
   本方法**不可覆盖**，并写明应由哪类运行证据填补（指标 / 专项并发测试 / 生产只读
   诊断）。该项不得留空、也不得以「未发现问题」结案——**「没发现」在只读静态方法下
   是预期结果，不是结论**；把它写成结论会让下一轮元审查以新编号重复命中同一空白。
...
- **覆盖缺口与修复有效性审计稿命名**（多 Harness 并行，避免撞名；2026-09-11 统一）：
  - 会话稿：`REVIEW_COVERAGE_AND_FIX_EFFECTIVENESS_<YYYY-MM-DD>_<会话后六位>.md`。
    `<会话后六位>` = harness 会话编号末 6 位：UUID 型会话去连字符后取末 6 位
    （例：`a64b6cba-0b5f-4b71-afdc-4728f55e3831` → `5e3831`）；非 UUID 型会话取编号字符串末 6 位；
  - 同一会话产出多份同题稿时以二级角色后缀区分：`<会话后六位>-<角色>.md`
    （如 `-verification` / `-confirmation` / `-convergence`）；
  - 新会话**必须**使用新文件名，不覆盖旧稿；多源汇聚可互相引用，不以 `_synthesis`
    单文件覆盖他源；
  - 禁止使用无会话后缀的笼统名（如 `PROJECT_REVIEW_R01_R15_SYNTHESIS_*.md`），
    以免多 Execution 同路径互踩。
...
| 确定缺陷 · 锁序相反 | 给出相反顺序的两处 `path:line` 与它们共同触碰的共享行。**不需要**先复现死锁才可判定——「相对顺序相反」是静态事实；但须注明「是否成环取决于并发交错」，不要把潜在环写成已发生故障。归入本类型，不要就近并入「性能 / 队首阻塞」 |
...
| 首轮正式审查基线 / 开始日期 | 2026-09-07 → 2026-09-11（**逐区基线不同**，见 §5.1；不构成单一快照） |
| R01–R15 | 逐区**静态只读审查已交付**，15 个台账均已收口（R02/#910 于 2026-09-16 收口）；**R01–R11（#891/#910/#945/#961/#979/#996/#1015/#1031/#1055/#1086/#1125）已于 09-16 补齐动态验证升「已完成」**，其余 4 区口径为「待验证」。逐区明细见 §5.1 |
| 跨区收口 | 链 A（执行链）已完成：[`REVIEW_CROSS_REGION_CHAIN_A_2026-09-12.md`](./REVIEW_CROSS_REGION_CHAIN_A_2026-09-12.md)（基线 `d820e1a9`，PR #1600）；链 B（日志链）已完成：[`REVIEW_CROSS_REGION_CHAIN_B_2026-09-13.md`](./REVIEW_CROSS_REGION_CHAIN_B_2026-09-13.md)（基线 `df4458ec`）——均为静态对证 + 测试盘点，不代表缺陷修复 / 动态验证 / 生产验收 |
| 本轮总记录与区域报告 | 尚未建立，实际开展后再登记链接 |
| R01–R15 台账载体 | 2026-09 轮次在 GitHub issue：#891、#910、#945、#961、#979、#996、#1015、#1031、#1055、#1086、#1125、#1201、#1230、#1266、#1302。**仓库内没有区域报告文件**，覆盖记录留在台账正文与评论中——事后无法在本仓库内核对「某区是否为某维度登记过覆盖或不适用理由」，需查 GitHub |

### 5.1 逐区状态（2026-09-14 实查核对；R01–R11 行为 2026-09-16 动态验证更新）

依据：`gh issue view <N>` 实查 15 个台账的**状态 / 关闭时间 / 正文自述基线与口径**；
「静态交付」= 台账存在且记录了二次源码核验结果，「状态」按 §4.1 四态口径填写。

| 区域 | 台账 | 状态 | 审查日期 | 审查基线 | 备注 |
|---|---|---|---|---|---|
| R01 总体架构与硬契约 | [#891](https://github.com/DUElost/stability-test-platform/issues/891) | **已完成** | 09-07 | `c9230c08` | 核验 HEAD `c3afca79`；结论 10/10 属实；台账已 CLOSED；**09-16 动态验证补齐**：基线 `b5ff1625` 隔离 testcontainers / 门禁 / Agent 分跑 F01–F08+F10 归属套件 **69 passed**（F09 为文档项无运行断言），登记见 #891 动态验证评论与 Agent Note |
| R02 认证、授权与安全边界 | [#910](https://github.com/DUElost/stability-test-platform/issues/910) | **已完成** | 09-07 | `963bdd18` | 7 缺陷 + 4 风险；台账 09-16 收口 CLOSED（`6b21e3b2` 回源码 7/7）；**09-16 动态验证补齐**：基线 `a0e6a4c8` 隔离 testcontainers 跑 F01–F07 + #908 归属套件 **121 passed**，登记见 #910 动态验证评论与收口 Agent Note；#906/#91 留 OPEN 由自身 issue 继续追踪 |
| R03 数据模型与数据库迁移 | [#945](https://github.com/DUElost/stability-test-platform/issues/945) | **已完成** | 09-07 | `e850f48a` | 8 缺陷 + 1 文档 + 3 风险；台账已 CLOSED；**09-16 动态验证补齐**：基线 `7456277d` 隔离 testcontainers 跑 F01–F08+F10–F12 归属套件 **75 passed**（F09=#889 文档项无运行断言），登记见 #945 动态验证评论与 Agent Note |
| R04 项目、主机、设备与资源 | [#961](https://github.com/DUElost/stability-test-platform/issues/961) | **已完成** | 09-07 | `deef9830` | 13 新建 + 6 映射既有；台账已 CLOSED；**09-16 动态验证补齐**：基线 `c84c6824` 隔离 PG + Agent + Vitest 跑归属套件 **66 passed**（F16 文档项无运行断言；F12/F13/F19 交叉引用 R03 已验），登记见 #961 动态验证评论与 Agent Note |
| R05 Plan、Suite 与参数编排 | [#979](https://github.com/DUElost/stability-test-platform/issues/979) | **已完成** | 09-07 | `f4715e76` | 9 缺陷 + 5 风险；台账已 CLOSED；**09-16 动态验证补齐**：基线 `96f00ca7` 隔离 PG + Vitest 跑归属套件 **104 passed**（F09 文案项、F11 已接受延期无运行断言），登记见 #979 动态验证评论与 Agent Note |
| R06 调度、派发与执行状态闭环 | [#996](https://github.com/DUElost/stability-test-platform/issues/996) | **已完成** | 09-07~08 | `f4715e76` | 8 缺陷 + 1 风险 + 1 文档；台账已 CLOSED；**09-16 动态验证补齐**：基线 `9ccb90dd` 隔离 PG 跑 F01–F09 归属套件 **55 passed**（F10 文档项无运行断言），登记见 #996 动态验证评论与 Agent Note |
| R07 Agent 执行引擎与运行可靠性 | [#1015](https://github.com/DUElost/stability-test-platform/issues/1015) | **已完成** | 09-08 | `a9f1ceaa` | 15 项属实；台账已 CLOSED；**09-16 动态验证补齐**：基线 `f4bf8b61` Agent 归属套件 **103 passed**（F01–F15；无真机），登记见 #1015 动态验证评论与 Agent Note |
| R08 脚本库、版本与外部工具接入 | [#1031](https://github.com/DUElost/stability-test-platform/issues/1031) | **已完成** | 09-08 | `345499eb` | 11 缺陷 + 2 风险；台账已 CLOSED；**09-16 动态验证补齐**：基线 `767ef9ea` 隔离 PG + Agent/脚本归属套件 **171 passed**（F11 文档/类型项无运行断言），登记见 #1031 动态验证评论与 Agent Note |
| R09 设备日志采集与异常事件 | [#1055](https://github.com/DUElost/stability-test-platform/issues/1055) | **已完成** | 09-08 | `345499eb` | 9 缺陷 + 3 风险；台账已 CLOSED；**09-16 动态验证补齐**：基线 `2f4421cf` 隔离 PG + Agent 归属套件 **315 passed**（F09 文档项无运行断言；#1054 交接预跑计入本区证据），登记见 #1055 动态验证评论与 Agent Note |
| R10 扫描、上传、存储与结果后处理 | [#1086](https://github.com/DUElost/stability-test-platform/issues/1086) | **已完成** | 09-08 | `9b80ee6f` | 13 缺陷 + 4 风险；台账已 CLOSED；**09-16 动态验证补齐**：基线 `703773b7` 隔离 PG + Agent 归属套件 **295 passed**（F13 文档项无运行断言），登记见 #1086 动态验证评论与 Agent Note |
| R11 实时通信与异步任务基础设施 | [#1125](https://github.com/DUElost/stability-test-platform/issues/1125) | **已完成** | 09-08 | `a0637a41` | 12 缺陷 + 5 风险；台账已 CLOSED；**09-16 动态验证补齐**：基线 `babf27ac` 隔离 PG + Agent + Vitest 归属套件 **297 passed**，登记见 #1125 动态验证评论与 Agent Note |
| R12 前端架构与交互体验 | [#1201](https://github.com/DUElost/stability-test-platform/issues/1201) | 待验证 | 09-10 | `0aab4f6e` | 未跑 Vitest/目视/性能 |
| R13 平台 AI 助手 | [#1230](https://github.com/DUElost/stability-test-platform/issues/1230) | 待验证 | 09-10 | `d11a0d62` | 未跑 pytest/Vitest/浏览器 |
| R14 部署、运维与可观测性 | [#1266](https://github.com/DUElost/stability-test-platform/issues/1266) | 待验证 | 09-10 | `d11a0d62` | 收尾主干 `a009eeee`；未执行部署/恢复/pytest |
| R15 测试、CI 与工程治理 | [#1302](https://github.com/DUElost/stability-test-platform/issues/1302) | 待验证 | 09-11 | `0d523265` | 未跑 pytest/Vitest/check:quick |

**口径说明（为什么统一记「待验证」而不是「已完成」）**：

- §4.1 明确「已完成」只代表约定范围的**审查交付**完成，不得被读成无缺陷、修复完成或
  生产验收通过。15 个台账虽已交付，但多数区**尚未做过动态验证**（各台账自述未跑
  pytest / 隔离 PG / 真机 / 浏览器 / 部署 / 故障注入），影响主要结论的关键证据仍不足，
  故按 §4.1「影响主要结论的关键证据不足时应保持待验证」记「待验证」。
- 「静态交付完成」不是独立的四态取值，只作为备注保留。把两者并成一个「已完成」会让
  下一轮元审查高估覆盖质量——2026-09-11 的覆盖与修复有效性报告已就这一点做过更正。
- **从「待验证」转为「已完成」的条件**：该区域的关键结论取得对应运行证据（隔离环境
  动态复现 / 规模与故障注入 / 真机与存储 / 部署验收），并登记在同一台账或本轮总记录。
- 09-16 已按本节转态条件升「已完成」的区域：**R01–R11**（R01 69 / R02 121 / R03 75 /
  R04 66 / R05 104 / R06 55 / R07 103 / R08 171 / R09 315 / R10 295 / R11 实时通信
  归属套件 **297 passed**，基线 `babf27ac`）。「已完成」仍只代表约定范围审查交付，
  不代表生产验收通过（§4.1）。其余 4 区保持「待验证」。

**尚未建立的载体**（本文件只登记，不在本轮新建）：

- 本轮总记录 `docs/reviews/PROJECT_REVIEW_<YYYY-MM-DD>_<short-sha>.md` **未建立**：本轮
  实际以 15 个台账 issue 为载体逐区推进，因此没有单文件总记录。这是**纪律缺口**——
  下一轮开始时应先按 §4.1 建总记录（含完整基线、开始日期、约定范围、工作区差异是否
  纳入）再逐区审，而不是沿用「只在 issue 里记」的形态。
- 仓库内没有区域报告文件，见上表「台账载体」行的限制说明。
