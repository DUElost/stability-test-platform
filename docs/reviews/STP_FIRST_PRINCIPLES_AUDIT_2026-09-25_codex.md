...
## 12. #107 七天自动续跑验收裁决（2026-09-26）

owner 明确「允许部署和人工验证，只要求链路自动连续」。据此，#107 的「无人工干预」
指 CHAIN/SCHEDULE 链不需要人工续跑或救活；部署与独立的 MANUAL 验证 run 不作为
自动链证据，也不因其存在而使窗口失效。部署期间受影响的 run 必须如实记录，并核对
后续轮次是否由链自行触发；若需手工续跑，窗口应从恢复后的自动链重新起算。

[#107 的 09-25 七日裁决记录](https://github.com/DUElost/stability-test-platform/issues/107)
覆盖 09-18 至 09-24 七个完整日：CHAIN/SCHEDULE 每日都有自动轮次，失败波次之后
无需人工触发下一轮；09-23 force 热更新使 run 523 中止，下一轮 525 自动续跑。
窗口内 27 条 MANUAL run 是独立验证，不填补自动链证据。09-26 日检又记录部署窗口
内 4 条 CHAIN run 被人工中止，之后 563 等轮次自动续跑，且无超 6 小时卡死或
TooManyConnectionsError。按本节定义，#107 的七日验收成立，issue 已关闭；该结论仅适用于
现有机队的自动续跑能力，不等同于 #105/#106 的 150/3750 容量验收。
