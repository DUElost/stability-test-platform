...
**为什么这反而让 #3093/#3094 更要紧（含对本节原稿的修正）**：原稿写「目录退役后
包通路成为唯一通路、没有兜底」——**这句过头了，正式更正**。读完
`origin/main` 后的准确机制是三层：

1. **兜底仍在，但语义降级**：`STP_SCRIPT_PACKAGES` 未设/非法一律按 `off`
   （`backend/agent/script_packages.py:49`–`:57`；示例配置同为 off，
   `backend/agent/.env.example:188`–`:189`），`on` 模式包不可用时 `_fallback`
   打一条 WARNING 后回退 `nfs_path`。而 Phase 3 之后 `nfs_path` 指向的是
   **每族一棵可变源码树**（`backend/services/script_catalog.py:3`），不再是某版本的
   冻结副本 ⇒ **回退不再等于「跑冻结字节」，而缺省档位就是回退档**。
2. **谁在兜底档，fleet 无账**：判据本来就在 Agent 回包里
   （`backend/agent/script_verifier.py:103` 计算、`:126` 随 ack 回传
   `package_active`），但在位账本没有这一列
   （`backend/models/script_presence.py:38`–`:58`）、sweep 分类也不使用它
   （`backend/services/script_presence.py:272`–`:314`），只有派发侧治愈判据消费一次
   （`backend/services/admission_pump.py:490`–`:492`）。
   ⇒ 不可变性事实上由「每台都在 `strict`」承担，而这句话**当前无法机器核验**
   （已立 **#3222** 补这条账）。
3. **准入侧强度没跟着上调**：`tool-contract` 门禁仍只验 fixture
   （`scripts/run_gates.py:126`–`:128` 不传 `--entrypoint`）、
   `STP_VERIFY_TOOL_CONTRACT=0` 仍可静默 SKIP（#3094）、新族归类靠 PR 自声明（#3093）。

结论方向不变、成因更准确：**风险不是「没有兜底」，而是「兜底不再保证字节不可变 +
谁在兜底档没人知道 + 准入绿灯仍偏软」**。新增专项越多，这三条一起变贵。
...
真实 `--check-env` + 一次端到端产物校验留痕。重评的三条新前提（缺省档 = 回退档、
回退字节不再冻结、fleet 档位无账）已分别写进 #3093 / #3094 的评论留痕，
档位可见性本身单立 **#3222**；不改 ADR-0051 的裁决、不批量下发开关。
...
- Pending：隔离库执行计划与基数压测、真实 25 台/host ADB/USB 长跑、
  ...
  「fleet 48/48 `strict`」的逐台复核（判据已有但未落账 ⇒ 先做 #3222）、
  生产只读容量复核；
  没有一项被本次静态审计宣称“通过”。
...
## 5. 后续立案（2026-09-23 收尾后追加，不改写上文快照）

审计的「待验证」项按「一单一判据」落成三张可执行单；本节只登记出口，不重述证据。
**上文各节结论与行号仍只对 `main@ae232a30` 负责**，唯一例外是 F06.1 中已显式标注的
措辞修正（该修正依赖 `origin/main` 的 Phase 3 后状态，不可反过来套用到旧基线）。

| 单子 | 覆盖的审计结论 |
|---|---|
| **#3217** `[agent][observability][P2]` crash artifact 满队列即丢、丢弃数只在 Agent 本机可见 | F08：三段式（extras 出口 → 显式消费产指标 → 事件侧告警 + promtool 场景），验收要求「四条数对得上」 |
| **#3219** `[agent][testing][P2]` 单 host 25 台真机整轮心跳时间预算未证 | F02：先补分段计时（现无任何整轮耗时序列），再按 1/5/10/25 档 × 场景矩阵实测，判据对齐 `HOST_HEARTBEAT_TIMEOUT_SECONDS` / `DEVICE_OFFLINE_TIMEOUT` |
| **#3222** `[agent][observability][P2]` 包模式无 fleet 视图（`package_active` 不落账不聚合） | F06.1 修正后的第 2 层：让「fleet 全 `strict`」成为可机器核验的不变量，再谈 Phase 4/5 门禁 |

**刻意未另立单的三项**（避免重复立案，理由留此）：

- **F01 观察腿**：#2959 已被指认为 ADR-0047 的事实来源；其缺口①（零告警）随 PR #3026
  闭合，其评论提出的压测腿读数缺口**现已在 main 修好**——
  `tools/dev/abort_scale_probe.py:319`–`:334` 已改为逐 `kind` 汇总、`:451`/`:463`
  产出 `checkout_failures_delta_by_kind`、`:480` 以合计判 FAIL。
  ⇒ 剩余三项证据（分布基线 / 按入口归因 / 非应用连接）与 abort 因果仍留在 #2959，
  只在该单补状态留痕，不复制成新单。
- **#3093 / #3094**：本审计的新前提属**重评分输入**而非新事实，已在两单各补一条评论
  留痕（含「缺省档即回退档」「回退不再冻结字节」「档位无账 ⇒ #3222」），
  是否抬 P3→P2/P1 由 owner 裁。
- **F03 / F05 的减法**（稳态 INFO 降噪、设备列表后端筛选或投影）：判据依赖 #3219
  与多查看者实测分布，属「先测后调」；无数据即立案等于拍脑袋调参，故暂不立。
