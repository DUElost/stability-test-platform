...
- **CodeRabbit 复评（参考意见）**：CodeRabbit 因配额限制实际使用不稳定，定位为参考而非硬门禁。`.coderabbit.yaml` 已关 `auto_incremental_review`，push 修复后不会自动复评；需要它对当前 head 给出新结论时，在 PR 评论 `@coderabbitai review` 显式触发。旧 commit 上的 CHANGES_REQUESTED 不构成当前 head 的阻断决策；其不可用（rate limit / skipped）时不阻塞 auto-merge。**例外（#267 复评竞态）**：本 PR 历史上有未撤销的 CR CHANGES_REQUESTED 时，新 head 的 gate 不接受 skipped/paused 放行，须等 CR 对当前 head 给出终态或明确 rate limited；逃生阀与语义见 `enable-auto-merge.yml` merge-gate 注释。
- **CI 分层（2026-08-07）**：PR 只跑轻量 job（lint / pr-typecheck / pr-compileall / pr-agent-tests）；全量 backend-test（PG + pytest）、frontend-check（vitest + build）、docker-build 仅在 push main、workflow_dispatch 或 post-merge 兜底运行。auto-merge 的 merge commit 不触发 on: push / closed / workflow_run（GITHUB_TOKEN 级联限制），由 `main-ci-backstop.yml` 每 15 分钟检查 main 尖端是否已有全量 CI、没有则显式 dispatch；`enable-auto-merge.yml` 的 closed 事件 job 仅覆盖手动合入。PR 合入前不跑 PG/vitest/docker，风险由合入后全量兜底。**合入路径注意力预算（核心原则，2026-08-14）**：合并路径上的阻塞检查保持 ~2 分钟内（required checks：lint / CodeQL / pr-typecheck / pr-compileall / pr-agent-tests；`code-rabbit-gate` 同为 required check 但语义 best-effort）；任何引入等待或分心的检查一律走异步（夜间全量兜底），不得为「合并前验证」加长合入等待（Merge Queue、vitest/backend-test 前置、hourly backstop 等均因此否决）；人工评审属同一注意力块内的同步动作，不在此限。详见 `docs/notes/process/2026-08-14-merge-path-attention-budget.md`。
