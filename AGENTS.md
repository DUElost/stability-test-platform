...
- **PR 合入**：仓库已开启 Auto-merge；`.github/workflows/enable-auto-merge.yml` 会自动给非 draft PR 挂上 auto-merge（merge commit），合入一律等 required checks + 1 个 approving review 后自动完成，不要手动点 Merge。
