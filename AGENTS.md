...
- **CI 分层（2026-08-07）**：PR 只跑轻量 job（lint / pr-typecheck / pr-compileall / pr-agent-tests）；全量 backend-test（PG + pytest）、frontend-check（vitest + build）、docker-build 仅在 push main、workflow_dispatch 或 post-merge 兜底运行。auto-merge 的 merge commit 不触发 on: push / closed（GITHUB_TOKEN 级联限制），由 `post-merge-full-ci.yml` 监听 CodeQL `workflow_run` 完成后 dispatch 全量 CI；`enable-auto-merge.yml` 的 closed 事件 job 仅覆盖手动合入。PR 合入前不跑 PG/vitest/docker，风险由合入后全量兜底；需要“合并前全量校验”时应引入 Merge Queue。
