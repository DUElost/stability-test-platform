...
def test_console_hint_and_warning_p4_branch(monkeypatch):
    """#1737 P4：注册表启用 → 声明跨实例 status/cancel/replay（replay 需共享 log_root）。"""
    from backend.services import run_console as rc
    ...
    assert "日志 replay" in hint
    assert "STP_RUN_CONSOLE_LOG_ROOT" in hint
    assert "#1114" in hint

    warning = rc.multi_instance_console_warning()
    assert warning is not None
    assert "console_cancel_forwarding=true" in warning
    assert "console_replay_cross_instance=shared_log_root_only" in warning
