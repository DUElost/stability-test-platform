...
# ── #2188 D 步 单3：读侧清单注册 + per-host 降级（R-4 契约测试）──────────────


def _meta_setup(tmp_path, monkeypatch, plan_run_id, host_id, artifacts):
    """造 `_meta/{run}/{host}.json` 分片 + 对应的 dedup/ 文件，返回 nfs_root。"""
    import json as _json

    nfs_root = tmp_path / "nfs"
    shard_dir = nfs_root / "_meta" / str(plan_run_id)
    shard_dir.mkdir(parents=True)
    (shard_dir / f"{host_id}.json").write_text(_json.dumps({
        "schema_version": 1,
        "host_id": host_id,
...
def test_meta_shard_registration_uses_shard_fields_not_filename(
    db_session, sample_plan_run, tmp_path, monkeypatch,
):
    """R-4 ①/②：分片齐全时按分片字段注册，**不解析文件名**。

    分片里的 host_id 与文件名前缀**故意不一致**——若读侧仍解析文件名
    （`_HOST_PREFIX_RE`），注册的 host_id 会是文件名里的那个，本用例即失败。
    """
    plan_run_id, real_host = sample_plan_run.id, "host-from-shard"
    file_key = "mtk/wrongprefix_Result_x_org.xls"
    nfs_root = _meta_setup(tmp_path, monkeypatch, plan_run_id, real_host, [{
        "file_key": file_key,
        "platform": "mtk",
        "size_bytes": 7,
        "registerable": True,
    }])
    target = nfs_root / "dedup" / str(plan_run_id) / file_key
    target.parent.mkdir(parents=True)
    target.write_bytes(b"payload")

    monkeypatch.setattr(
        "backend.core.database.SessionLocal", lambda: db_session, raising=True,
    )
    n = ds.run_scan_sync(plan_run_id, expected_hosts=[real_host])

    assert int(n) == 1
    from backend.models.plan_run_artifact import PlanRunArtifact
    row = db_session.query(PlanRunArtifact).filter_by(plan_run_id=plan_run_id).one()
    assert row.host_id == real_host, "host_id 必须来自分片字段，而非文件名前缀"
    assert row.storage_uri == str(target), "storage_uri 须与 legacy str(路径) 同形"


def test_meta_shard_path_does_not_glob_dedup_dir(
    db_session, sample_plan_run, tmp_path, monkeypatch,
):
    """R-4 ①：分片齐全 run 上 `run_scan_sync` **零中心盘 glob 遍历**。

    把 `dedup/` 目录做成「存在但不可列举」——若代码走 glob 会抛/得 0，
    而清单路径不触碰它，故仍能正常注册。
    """
    plan_run_id, host_id = sample_plan_run.id, "h1"
    file_key = "Result_a_org.xls"
    nfs_root = _meta_setup(tmp_path, monkeypatch, plan_run_id, host_id, [{
        "file_key": file_key, "platform": "", "size_bytes": 1, "registerable": True,
    }])
    target = nfs_root / "dedup" / str(plan_run_id) / file_key
    target.parent.mkdir(parents=True)
    target.write_bytes(b"x")

    # 记录对 dedup 目录的 glob 调用（应为 0）
    calls = []
    real_glob = ds.Path.glob

    def _spy_glob(self, pattern):
        if "dedup" in str(self):
            calls.append(str(self))
        return real_glob(self, pattern)

    monkeypatch.setattr(ds.Path, "glob", _spy_glob, raising=False)
    monkeypatch.setattr(
        "backend.core.database.SessionLocal", lambda: db_session, raising=True,
    )
    ds.run_scan_sync(plan_run_id, expected_hosts=[host_id])

    assert calls == [], f"清单路径不得 glob 中心盘 dedup 目录，实际: {calls}"


def test_missing_shard_falls_back_to_per_host_legacy_glob(
    db_session, sample_plan_run, tmp_path, monkeypatch,
):
    """R-4 降级：**缺分片**的 host 走 legacy glob，且**只注册该 host** 的文件。"""
    plan_run_id = sample_plan_run.id
    nfs_root = tmp_path / "nfs"
    dedup_dir = nfs_root / "dedup" / str(plan_run_id)
    dedup_dir.mkdir(parents=True)
    # 两个 host 的文件都在盘上，但只有 legacy_host 缺分片
    (dedup_dir / "legacy_host_Result_a_org.xls").write_bytes(b"a")
    (dedup_dir / "other_host_Result_b_org.xls").write_bytes(b"b")
    monkeypatch.setenv("STP_AEE_NFS_ROOT", str(nfs_root))
    # 为 other_host 造分片（空清单 → 不注册），使其**不**走 glob
    import json as _json
    shard_dir = nfs_root / "_meta" / str(plan_run_id)
    shard_dir.mkdir(parents=True)
    (shard_dir / "other_host.json").write_text(_json.dumps({
        "schema_version": 1, "host_id": "other_host",
        "plan_run_id": plan_run_id, "updated_at": "2026-09-17T00:00:00+00:00",
        "artifacts": [],
    }), encoding="utf-8")
    ...
    n = ds.run_scan_sync(plan_run_id, expected_hosts=["legacy_host", "other_host"])

    assert int(n) == 1, "只有缺分片的 host 应经 legacy glob 注册"
    from backend.models.plan_run_artifact import PlanRunArtifact
    rows = db_session.query(PlanRunArtifact).filter_by(plan_run_id=plan_run_id).all()
    assert [r.host_id for r in rows] == ["legacy_host"], "不得注册其它 host 的文件"


def test_no_manifest_and_no_expected_hosts_keeps_global_glob(
    db_session, sample_plan_run, tmp_path, monkeypatch,
):
    """向后兼容：无分片且**未给** `expected_hosts` → 保持原全目录行为。

    过渡期不得因清单未落地而窄化既有覆盖面（既有调用点传 None）。
    """
    plan_run_id = sample_plan_run.id
    nfs_root = tmp_path / "nfs"
    dedup_dir = nfs_root / "dedup" / str(plan_run_id)
    dedup_dir.mkdir(parents=True)
    (dedup_dir / "h1_Result_a_org.xls").write_bytes(b"a")
    (dedup_dir / "h2_Result_b_org.xls").write_bytes(b"b")
    monkeypatch.setenv("STP_AEE_NFS_ROOT", str(nfs_root))
    monkeypatch.setattr(
        "backend.core.database.SessionLocal", lambda: db_session, raising=True,
    )

    n = ds.run_scan_sync(plan_run_id)      # 不传 expected_hosts

    assert int(n) == 2, "缺省应保持原全目录 glob 行为"


def test_non_registerable_entries_are_skipped(
    db_session, sample_plan_run, tmp_path, monkeypatch,
):
    """`registerable=False` 的条目不得注册（谓词由写侧冻结）。"""
    plan_run_id, host_id = sample_plan_run.id, "h1"
    nfs_root = _meta_setup(tmp_path, monkeypatch, plan_run_id, host_id, [
        {"file_key": "keep_org.xls", "platform": "", "size_bytes": 1, "registerable": True},
        {"file_key": "skip_org.xls", "platform": "", "size_bytes": 1, "registerable": False},
    ])
    ddir = nfs_root / "dedup" / str(plan_run_id)
    ddir.mkdir(parents=True, exist_ok=True)
    for name in ("keep_org.xls", "skip_org.xls"):
        (ddir / name).write_bytes(b"x")

    monkeypatch.setattr(
        "backend.core.database.SessionLocal", lambda: db_session, raising=True,
    )
    n = ds.run_scan_sync(plan_run_id, expected_hosts=[host_id])

    assert int(n) == 1
    from backend.models.plan_run_artifact import PlanRunArtifact
    row = db_session.query(PlanRunArtifact).filter_by(plan_run_id=plan_run_id).one()
    assert row.storage_uri.endswith("keep_org.xls")


def test_broken_shard_is_skipped_not_fatal(db_session, sample_plan_run, tmp_path, monkeypatch):
    """坏分片（非 JSON）应跳过而非让整轮注册失败。"""
    plan_run_id, host_id = sample_plan_run.id, "h1"
    nfs_root = _meta_setup(tmp_path, monkeypatch, plan_run_id, host_id, [
        {"file_key": "ok_org.xls", "platform": "", "size_bytes": 1, "registerable": True},
    ])
    (nfs_root / "_meta" / str(plan_run_id) / "broken.json").write_text(
        "{not json", encoding="utf-8")
    (nfs_root / "dedup" / str(plan_run_id)).mkdir(parents=True, exist_ok=True)
    (nfs_root / "dedup" / str(plan_run_id) / "ok_org.xls").write_bytes(b"x")
