...
def test_agent_api_verify_rejects_placeholder_server_secret(monkeypatch):
    ...
    with pytest.raises(HTTPException) as excinfo:
        verify_agent_secret("change-me-in-production")
...
@pytest.mark.asyncio
async def test_lifespan_requires_secure_auth_cookies_in_production(monkeypatch, auth_env):
    monkeypatch.setenv("TESTING", "0")
    monkeypatch.setenv("ENV", "production")
    monkeypatch.setenv("AGENT_SECRET", "test-agent-secret")
    auth_env.set("AUTH_COOKIE_SECURE", "0")
    auth_env.set("AUTH_COOKIE_SAMESITE", "lax")
...
def test_internal_allows_http_cookies_without_secure(monkeypatch, auth_env):
    ...
    auth_env.set("AUTH_COOKIE_SECURE", "0")
    auth_env.set("AUTH_COOKIE_SAMESITE", "lax")
    auth_env.set("STP_CSRF_ENABLED", "1")

    validate_production_auth_cookie_settings()  # 不抛


def test_internal_still_rejects_csrf_disabled(monkeypatch, auth_env):
    ...
    auth_env.set("AUTH_COOKIE_SECURE", "0")
    auth_env.set("AUTH_COOKIE_SAMESITE", "lax")
    auth_env.set("STP_CSRF_ENABLED", "0")

    with pytest.raises(RuntimeError, match="STP_CSRF_ENABLED"):
        validate_production_auth_cookie_settings()


def test_internal_still_rejects_invalid_samesite(monkeypatch, auth_env):
    ...
    monkeypatch.setenv("ENV", "internal")
    auth_env.set("AUTH_COOKIE_SECURE", "0")
    auth_env.set("AUTH_COOKIE_SAMESITE", "invalid")
    auth_env.set("STP_CSRF_ENABLED", "1")
...
@pytest.mark.asyncio
async def test_lifespan_rejects_samesite_none_without_csrf_protection(monkeypatch, auth_env):
    monkeypatch.setenv("TESTING", "0")
    monkeypatch.setenv("ENV", "production")
    monkeypatch.setenv("AGENT_SECRET", "test-agent-secret")
    auth_env.set("AUTH_COOKIE_SECURE", "1")
    auth_env.set("AUTH_COOKIE_SAMESITE", "none")
...
@pytest.mark.asyncio
async def test_lifespan_rejects_csrf_disabled_in_production(monkeypatch, auth_env):
    monkeypatch.setenv("TESTING", "0")
    monkeypatch.setenv("ENV", "production")
    monkeypatch.setenv("AGENT_SECRET", "test-agent-secret")
    auth_env.set("AUTH_COOKIE_SECURE", "1")
...
@pytest.mark.asyncio
async def test_lifespan_rejects_invalid_samesite_in_production(monkeypatch, auth_env):
