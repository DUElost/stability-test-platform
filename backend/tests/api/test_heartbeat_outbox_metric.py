...
def test_heartbeat_persists_dead_letter_total(db_session):
    """#302 / #742: outbox 死信总量落 host.extra，控制面清单 API 可见。"""
    host = Host(
        id="dl-host-1",
        hostname="dl-agent",
        status="ONLINE",
    ...
    payload = HeartbeatIn(
        ...
        status="ONLINE",
        extra={
            "log_signal_dead_letter_total": 4,
            "terminal_outbox_dead_letter_total": 2,
        },
    )
    _process_heartbeat_with_db(payload, db_session)
    db_session.commit()
    ...
    assert host.extra["terminal_outbox_dead_letter_total"] == 2
