...
# ---------------------------------------------------------------------------
# run_dir 绑定（#810 / R08）：setup 记录本次观测到的运行目录，check/finish
# 只认它——避免「取最新目录」在残留目录下混入上一轮数据或误判启动成功。
# 三件套在同一 Agent 主机、同一设备的同一 Job 生命周期内串行执行，故用
# 主机本地状态文件（按设备 serial 键控）交接；项目不一致/缺失时回退。
# ---------------------------------------------------------------------------

def run_dir_marker_path() -> Path:
    import tempfile

    return Path(tempfile.gettempdir()) / f"mtbf_run_dir_{device_serial()}.json"


def save_run_dir(project: str, run_dir: str) -> None:
    if not run_dir:
        return
    try:
        run_dir_marker_path().write_text(
            json.dumps({"project": project, "run_dir": run_dir})
        )
    except OSError:
        pass


def load_run_dir(project: str) -> str:
    """返回 setup 绑定的 run_dir；无记录或项目不符时返回空串。"""
    try:
        payload = json.loads(run_dir_marker_path().read_text())
    except (OSError, ValueError):
        return ""
    if not isinstance(payload, dict):
        return ""
    if str(payload.get("project")) != str(project):
        return ""
    return str(payload.get("run_dir") or "")


def clear_run_dir() -> None:
    try:
        run_dir_marker_path().unlink()
    except OSError:
        pass
