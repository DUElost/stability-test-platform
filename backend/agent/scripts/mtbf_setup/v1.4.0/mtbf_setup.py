...
（设备端 mtime 校验，残留目录不再让新任务假阳性），并把 run_dir 写为设备级
...
def _run(cfg: dict) -> dict:
    ...
    run_dir = _wait_run_dir(timeout_s=60, newer_than=started_at)
    if not run_dir:
...
def _wait_run_dir(timeout_s: int = 60, newer_than: float = 0.0) -> str:
    """轮询 /sdcard/results/realresult/ 直到出现**本步骤起点之后**的运行目录。

    #810：只看「存在目录」会让上一轮残留目录瞬时满足 60s 门（新任务没起来也
    报启动成功）。要求候选目录设备端 mtime 不早于 ``newer_than``（留 2s 容差，
    规避设备时钟抖动），否则继续等待。
    """
    ...
    while time.time() < deadline:
        ...
        for name in sorted(names, reverse=True):
            ...
            try:
                ...
                continue
            if newer_than <= 0 or mtime >= (newer_than - 2.0):
                return name
        time.sleep(5)
    return ""
