...
``stat -c %Y``（仅 2s 容差），设备时钟未对时（偏差 >2s）时新目录被判「早于
步骤起点」拒绝 → 60s 超时假失败。快照自比是设备时钟对设备时钟，无跨钟。

v1.4.0（#810，R08）：run_dir 绑定——只接受本步骤起点之后新建的结果目录
（残留目录不再让新任务假阳性），并把 run_dir 写为设备级
绑定供 check/finish 消费（不再各自取「最新目录」）。
...
def _run(cfg: dict) -> dict:
    ...
    # #1715：快照在**触发启动前**取——此后出现/变化（mtime 增大）的目录才是
    # 本轮产物。设备 mtime 与快照同源，不与宿主时钟交叉。
    run_dirs_before = _snapshot_run_dirs()
    ...
    # #810：只接受本步骤起点之后新建的目录——残留的上一轮 run_dir 不得让
    # 新任务「未真正起来也报启动成功」，随后把它绑定给 check/finish。
    # #1715：判据 = 启动前快照之外（新建）或快照内 mtime 增大（同名重建），
    # 设备时钟自比，宿主时钟偏差不再影响判定。
    run_dir = _wait_run_dir(timeout_s=60, before=run_dirs_before)
    if not run_dir:
        raise RuntimeError("RunTaskService 未在 60s 内创建结果目录，启动失败")
    save_run_dir(project, run_dir)
...
def _snapshot_run_dirs() -> dict:
    """设备侧既有 run 目录快照：名字 → 设备端 mtime epoch。

    stat 失败的目录按 0.0 记（视为「旧目录」——若它真是本轮新建/重建，
    轮询期任何 mtime 读数都会 > 0 触发命中）。
    """
    ...
    for name in names:
        raw = adb_shell(
            f"stat -c %Y /sdcard/results/realresult/{name}", timeout=30,
        ).strip()
        try:
            snap[name] = float(raw.splitlines()[0])
        except (ValueError, IndexError):
            snap[name] = 0.0
    return snap


def _wait_run_dir(timeout_s: int = 60, before: "dict | None" = None) -> str:
    """轮询 /sdcard/results/realresult/ 直到出现**本步骤之后**的运行目录。

    #810：只看「存在目录」会让上一轮残留目录瞬时满足 60s 门（新任务没起来也
    报启动成功）。
    v1.4.1（#1715）：候选判据与**触发启动前的快照**（``before``）对比——不在
    快照中的目录（新建）或快照内但 mtime 相对快照值增大的目录（同名重建）。
    mtime 与快照同取自设备时钟，自比无跨钟——v1.4.0 用宿主 time.time() 比对
    设备 mtime（2s 容差），设备时钟偏差 >2s 时新目录被拒、60s 假超时。
    """
    ...
    while time.time() < deadline:
        ...
        for name in sorted(names, reverse=True):
            ...
            try:
                mtime = float(mtime_raw.splitlines()[0])
            except (ValueError, IndexError):
                continue
            if name not in before or mtime > before.get(name, 0.0):
                return name
        time.sleep(5)
    return ""
