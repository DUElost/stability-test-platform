...
def step_att_clean(serial: str, cfg: dict) -> dict:
    ...
    errors = []
    warnings = []
    # #2777：超时 30→10s；force-stop 失败不进 errors，am kill 兜底，再失败
    # 也只记 warning 继续——monkey 不依赖 AutoTestTool 已死才能跑。
    try:
        adb_shell("am force-stop com.tinno.autotesttool", timeout=10)
    except Exception as exc:
        warnings.append(f"force-stop autotesttool: {exc}")
        try:
            adb_shell("am kill com.tinno.autotesttool", timeout=10)
            warnings.append("force-stop 超时/失败，已走 am kill 兜底")
        except Exception as exc2:
            warnings.append(f"force-stop 与 am kill 兜底均失败（继续 init）: {exc2}")
    try:
        out = adb_shell("rm -rf /data/data/com.tinno.autotesttool/shared_prefs/*.xml", timeout=30)
    except Exception as exc:
        errors.append(f"rm autotesttool prefs: {exc}")
    if errors:
        result = {"success": False, "error": "; ".join(errors)}
        if warnings:
            result["att_warnings"] = warnings
        return result
    result = {"success": True, "att_prefs_cleared": True}
    if warnings:
        result["att_warnings"] = warnings
    return result
