...
`validated_log_dirs` 只接受 `/data/` 或 `/sdcard/` 下的绝对路径，非法项记红该步骤
（不静默跳过——否则「清理没做」会以 success 结束）。

v2.3.10（#2975）：**结构化出口挪进引擎读取面**。v2.3.9 把 `att_stop_issued` /
`att_prefs_cleared` 放进了**步骤字典**的 `metrics`，而 `pipeline_engine` 只读
stdout **顶层** `payload["metrics"]`（`pipeline_engine.py` `_run_script_action`）
——于是 v2.3.9 声称的「降级结论进观测面」实际不可达，`att_warnings` 的静默
命运原样重演。现 `main()` 把各步骤的 `metrics` 聚合为顶层 `metrics` 一并输出
（成功与失败两条出口都带），步骤内嵌位置保持不变（step_trace 复盘用）。

v2.3.9（#2862）：`att_clean` 的 force-stop / am kill / prefs rm 三条命令改按**退出码**
判读（`adb_shell_quiet`），降级结论进 `metrics.att_stop_issued` /
`metrics.att_prefs_cleared`——此前 `adb_shell` 不看 rc，「命令被拒 / 设备 offline」与
「命令成功」同形，`att_prefs_cleared` 恒 True（详见 `step_att_clean` 的 docstring）。

v2.3.7 相对 v2.3.6 的差异（#1027 收口，R08-F07）：
  - `push` 的 bundle 分支此前不检查 `adb_shell_progress`（tar 解包）的退出码
    —— 空间不足 / 权限 / 损坏归档导致解包失败时仍返回 success=True，准备阶段
    带病继续（#812 同族）。
  - 修复：检查 returncode（非零 → 步骤失败，error 带退出码与 stderr 摘要）；
    TimeoutExpired 显式转失败；**后置条件**校验 marker
    `.stp_bundle_sha256` 内容 == manifest 期望值（复合命令的 echo 段没执行
    同样判失败）。退出码语义按 ADR-0033 §D2 方向约定取工具自有域
    （脚本整体退出码不变：成功 0 / 失败 1）。

v2.3.6 相对 v2.3.5 的差异（#894 收口）：
  - `_pump_lines` 捕获上限改为原子预留剩余容量：单行或并发两流都不再越过
    `_MAX_CAPTURED_CHARS`，超限行按剩余容量截断后丢弃

v2.3.4 相对 v2.3.3 的差异（review 非阻塞项收口）：
  - `_pump_lines` 加 8MiB 两流合计捕获上限：超限后丢弃后续行但仍继续
    读取（防管道阻塞），对齐引擎侧 #123 的 _MAX_CAPTURED_CHARS 防护
  - on_line 回调不受上限影响——PROGRESS 打戳不能因内存上限丢失

v2.3.3 相对 v2.3.2 的差异（#140 收口）：
  - `main()` 入口立即打 `phase=init` 戳；每个 step 前后打
    `step:<name>:start` / `step:<name>:end` 戳
  - bundle sha256 全量校验期间按 64MiB 打 `phase=sha256` 戳（慢 NFS 上
    前置校验不再是无戳窗口）
  - 前置阶段不再消耗"无戳"预算，`stall_seconds` 只需覆盖单个步骤内部
    最长一条命令的耗时

v2.3.2 相对 v2.3.1 的差异（#138 / #139 收口）：
  - tar 解包阶段打 PROGRESS 戳：开始/结束时各一枚，期间每
    `tar_progress_interval_s` 秒一枚心跳戳，避免"慢但健康"的解包被停滞钟误杀
  - `_make_progress` 回调同时兼容 `on_progress(pct)` 位置参数与
    `on_progress(written_bytes=...)` 关键字参数
  - `_pump_lines` 不再静默吞回调异常，留 debug 日志便于排查
...
_SHA256_PROGRESS_BYTES = 64 * 1024 * 1024


def _sha256_file(path: str, on_progress: "callable | None" = None) -> str:
    """逐块计算 sha256；每 64MiB 与结束时可选打 PROGRESS 戳。

    #140: 大 bundle 在慢 NFS 上做全量校验可能耗时且全程无戳，启用
    stall_seconds 时会在进入 push 之前就被停滞钟误杀。这里按固定字节数
    打 `phase=sha256` 心跳，结束再打一枚 done 戳。
    """
    digest = hashlib.sha256()
    hashed = 0
    next_stamp = _SHA256_PROGRESS_BYTES
    with open(path, "rb") as handle:
        for chunk in iter(lambda: handle.read(1024 * 1024), b""):
            digest.update(chunk)
            hashed += len(chunk)
            if on_progress is not None and hashed >= next_stamp:
                on_progress(phase="sha256", bytes_hashed=hashed)
                next_stamp += _SHA256_PROGRESS_BYTES
    if on_progress is not None:
        on_progress(phase="sha256", bytes_hashed=hashed, done=True)
...
# tar 解包是单条 adb shell 长命令，无任何输出；默认每 30s 打一枚心跳戳。
# 测试会 monkeypatch 成极小值来加速验证。
_TAR_PROGRESS_INTERVAL_S = 30
...
def step_push(serial: str, cfg: dict) -> dict:
    ...
    if bundle and manifest_path:
        ...
        actual_sha = _sha256_file(bundle, on_progress=progress)
        ...
        for src, dst in (
            ...
            if failure:
                return failure
        # #139: tar 解包最长可到 timeout_seconds（缺省 600s），全程无输出。
        # 开始/结束各打一枚戳，期间按 tar_progress_interval_s 周期性打心跳戳，
        # 否则启用 stall_seconds 时"慢但健康"的解包会被引擎停滞钟误杀。
        progress(phase="tar_start")
        try:
            proc = adb_shell_progress(
                ...
                progress_interval=float(
                    cfg.get("tar_progress_interval_s", _TAR_PROGRESS_INTERVAL_S)
                ),
            )
        except subprocess.TimeoutExpired:
            # #1027：解包超时同样不许报成功（复合命令可能半途而废，marker 不可信）
            return {
                "success": False,
                "error": f"tar extract timed out after {cfg.get('timeout_seconds', 600)}s",
            }
        finally:
            progress(phase="tar_end")
        # #1027：检查退出码 —— tar 空间不足 / 权限 / 损坏归档都会非零退出，
        # 此前直接 return success=True 让准备阶段带病继续（#812 同族）。
        if proc.returncode != 0:
            detail = ((proc.stderr or "") or (proc.stdout or "")).strip()[:300]
            return {
                "success": False,
                "error": f"tar extract failed rc={proc.returncode}: {detail}",
            }
        # #1027：后置条件 —— marker 内容 == 期望 sha。复合命令的 tar 段成功但
        # echo 段失败（如目录被并发清理）时 rc 也可能为 0，只有 marker 可信。
        try:
            marker_value = adb_shell(f"cat {marker} 2>/dev/null", timeout=10)
        except Exception as exc:
            return {"success": False, "error": f"bundle marker check failed: {exc}"}
        if marker_value.strip().splitlines()[0] != expected_sha:
            return {
                "success": False,
                "error": f"bundle marker mismatch: expected {expected_sha}, "
                         f"got {marker_value.strip()[:100]}",
            }
...
#: `log_dirs` 白名单形态（#3107）：这些值被插进设备端 `rm -rf {d}/*`（root 执行），
#: 空值 / `/` / 含空格或元字符的值会把删除面扩张到任意路径（`[""]` ⇒ `rm -rf /*`）。
_LOG_DIR_RE = re.compile(r"^/(?:data|sdcard)/[A-Za-z0-9._/-]{1,120}$")


def validated_log_dirs(raw: object) -> list:
    """校验计划参数 `log_dirs`（#3107）。
...
def step_clean(serial: str, cfg: dict) -> dict:
    ...
    if cfg.get("clear_logs", False):
        try:
            log_dirs = validated_log_dirs(cfg.get("log_dirs"))
        except ValueError as exc:
            # 参数非法 = 配置错误，直接红（#3107）——不把未校验的值插进 root shell。
...
def step_att_clean(serial: str, cfg: dict) -> dict:
    """#894：清 AutoTestTool 残留（防 monkey 期间设备重启触发叠加）。

    AutoTestTool（com.tinno.autotesttool）是 powercycle/sleep 专项的宿主
    app。若前序专项 finish 写 prefs running=false 失败（重启窗口 run-as
    失败静默），残留 running=true 会在设备 boot 后经 boot receiver 拉起
    PowerCycleService/SleepTestService——与 monkey 无序叠加（实测见 #894：
    run 345 monkey 段 1 台 PowerCycleService 存活 + prefs running=true）。

    monkey 不使用 AutoTestTool——此处 force-stop + 删除其全部 prefs
    （root 路径；prefs 缺失静默跳过）。

    v2.3.8（#2777）：AM 卡死时 force-stop 挂满超时会把整个 init 判败
    （.59 六窗复发）——att_clean 的语义是「清残留防叠加」，**清不掉 ≠ 不能
    跑 monkey**：force-stop 降级为 warning，失败后走 ``am kill`` 兜底；
    超时 30→10s 收窄（卡死设备上 30s×设备数会拖垮整窗 init 预算）。
    prefs 清理仍判失败（#894 的叠加风险依赖它，真失败值得红）。

    v2.3.9（#2862）：三条命令全部改**按 rc 判读**（``adb_shell`` 不看 rc，只在
    超时时抛异常 ⇒ 「命令被拒 / 设备 offline」与「命令成功」同形）。三条后果
    随之修正：① ``am kill`` rc≠0 不再被记成「已走兜底」；② force-stop 非超时
    失败不再连 warning 都没有；③ ``rm`` 权限拒绝不再判绿（docstring 自陈
    「prefs 真失败值得红」，此前只在**抛异常**时才红）。
    降级结论的出口是**结构化**的：``metrics.att_stop_issued`` /
    ``metrics.att_prefs_cleared`` 进步骤指标（``pipeline_engine`` 只读顶层
    success/error_message/metrics/skipped，嵌套 ``att_warnings`` 只落 output 文本），
    ``att_prefs_cleared`` 也如实反映结果而不是恒 True。
    （#2975 勘正：v2.3.9 实际只落到步骤字典、顶层无 ``metrics`` 键——「只读顶层」
    这一正确前提当时未兑现；v2.3.10 起 ``main()`` 将步骤 metrics 聚合进顶层。）

    边界（不主张）：``am kill`` 只杀「可安全杀」的后台进程，rc=0 **不等于**
    进程已死——这里核验的是「命令是否被设备接受」，真机进程判据另议（issue 边界）。
    """
    errors = []
    warnings = []
    # #2777：超时 30→10s；#2862：rc 判读（adb_shell_quiet 返回 CompletedProcess）
    stop_issued = False
    try:
        proc = adb_shell_quiet("am force-stop com.tinno.autotesttool", timeout=10)
    except Exception as exc:  # 超时 / adb 不可用
        forced_note = f"force-stop autotesttool: {exc}"
    else:
        if proc.returncode == 0:
            stop_issued = True
            forced_note = ""
            ...
            forced_note = (
                f"force-stop rc={proc.returncode}: "
                f"{(proc.stderr or '').strip()[:120]}"
            )
    if not stop_issued:
        warnings.append(forced_note)
        try:
            proc_kill = adb_shell_quiet("am kill com.tinno.autotesttool", timeout=10)
        except Exception as exc2:
            warnings.append(f"am kill 兜底失败（继续 init）: {exc2}")
        else:
            if proc_kill.returncode == 0:
                stop_issued = True
                warnings.append("已走 am kill 兜底（rc=0；未核验进程是否真死）")
            else:
                warnings.append(
                    f"am kill 兜底 rc={proc_kill.returncode}（继续 init）: "
                    f"{(proc_kill.stderr or '').strip()[:120]}"
                )

    prefs_cleared = False
    try:
        proc_rm = adb_shell_quiet(
            "rm -rf /data/data/com.tinno.autotesttool/shared_prefs/*.xml", timeout=30,
        )
    except Exception as exc:
        errors.append(f"rm autotesttool prefs: {exc}")
    else:
        if proc_rm.returncode == 0:
            prefs_cleared = True
        else:
            errors.append(
                f"rm autotesttool prefs rc={proc_rm.returncode}: "
                f"{(proc_rm.stderr or '').strip()[:120]}"
            )

    metrics = {
        "att_stop_issued": int(stop_issued),
        "att_prefs_cleared": int(prefs_cleared),
    }
    if errors:
        result = {"success": False, "error": "; ".join(errors), "metrics": metrics}
        if warnings:
            result["att_warnings"] = warnings
        return result
    result = {"success": True, "att_prefs_cleared": prefs_cleared, "metrics": metrics}
    if warnings:
        result["att_warnings"] = warnings
    return result
...
def main() -> None:
    ...
    step_names = args.get("steps", ["wifi", "root", "push", "install", "clean", "att_clean"])
    ...
    for name in step_names:
        cfg = args.get(name, {})
        progress(phase=f"step:{name}:start")
        try:
            try:
            ...
            progress(phase=f"step:{name}:end")
    ...
    # #2975：引擎只读**顶层** payload["metrics"]——把各步骤的 metrics 聚合上来到
    # 读取面（v2.3.9 只落进步骤字典，「结构化出口」实际零消费者）。
    top_metrics: dict = {}
    for step_result in results.values():
        step_metrics = step_result.get("metrics") if isinstance(step_result, dict) else None
        if isinstance(step_metrics, dict):
            top_metrics.update(step_metrics)
    if overall_success:
        output_result(True, steps=results, total_duration_s=duration, metrics=top_metrics)
    else:
