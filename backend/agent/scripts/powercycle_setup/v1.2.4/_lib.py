...
def get_app_uid() -> int:
    ...
    raise RuntimeError(
        f"无法解析 {_PKG} 的 uid（attempts={len(history)}/{max_attempts} "
...
def install_apk(apk: Path) -> None:
    ...
    **系统就绪**（``get-state==device`` 且 ``sys.boot_completed==1``，与
    ...
    wait_deadline = time.time() + att_install_wait_budget_seconds()
    history: list[str] = []
    ...
    for attempt in range(1, max_attempts + 1):
        if attempt > 1:
            ...
            if remaining <= 0:
                history.append(f"{attempt}:wait_budget_exhausted")
                break
            ok, observed = wait_system_ready(min(time.time() + ready_seconds, wait_deadline))
            if not ok:
    ...
    raise RuntimeError(
        ...
        f"(attempts={len(history)}/{max_attempts} history=[{','.join(history[:6])}])"
    )
