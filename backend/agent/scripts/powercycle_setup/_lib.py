...
- v1.2.2（#2802，r477 取证）：``install_apk`` 重试前等**系统就绪**
  （``get-state==device`` 且 ``sys.boot_completed==1``，同 ``check_device v1.0.2``
  判定）替代只等 adbd 可见的 ``wait-for-device``；尝试上限默认 3、等待总预算
  默认 90s（``STP_ATT_INSTALL_*`` env 可覆盖）；失败报文追加 ``attempts=/history=``。
"""
...
# v1.2.2：install 重试前「等系统就绪」缺省（env 可覆盖；见 install_apk）
_ATT_INSTALL_DEFAULT_MAX_ATTEMPTS = 3
...
_PCS_RETRY_DEFAULT_READY_SECONDS = 60.0
_PCS_RETRY_DEFAULT_WAIT_BUDGET_SECONDS = 90.0
...
def _parse_uid(info: str) -> int | None:
    """从 ``dumpsys package`` 输出解析 uid（纯函数，便于单测）。"""
...
def get_app_uid() -> int:
    ...
    v1.2.4（#2802，r503 取证：uid 解析失败属**撞峰同族**——设备重启窗内
    ``dumpsys package`` 输出不含 sharedUser/userId/uid 字段）：首试仍走快速路径；
    未解析到字段时等**系统就绪**（同 ``wait_system_ready``）再重试，最多
    ``pcs_retry_max_attempts()`` 次、预算 ``pcs_retry_wait_budget_seconds()`` 封顶；
    失败报文带 ``attempts=`` / ``history=[``（逐次含 rc）与 dumpsys 片段。

    v1.2.5（#3140）：预算改为**累计等待时间**计账（与 ``install_apk`` 同款，
    dumpsys 的执行耗时不再吞掉重试预算）；``attempts=`` 取实际尝试数并补
    ``waited=``。
    """
    max_attempts = pcs_retry_max_attempts()
    wait_budget = pcs_retry_wait_budget_seconds()
    waited = 0.0
    attempts_made = 0
    history: list[str] = []
    snippet = "no output"
    for attempt in range(1, max_attempts + 1):
        if attempt > 1:
            ...
            if remaining <= 1:
                history.append(f"{attempt}:wait_budget_exhausted(waited={waited:.1f}s)")
                break
            wait_start = time.time()
            wait_system_ready(time.time() + min(pcs_retry_ready_seconds(), remaining))
            waited += time.time() - wait_start
        attempts_made = attempt
        rc, out, err = adb("shell", f"dumpsys package {_PKG}", timeout=30)
        info = out or ""
        snippet = _adb_diag(info, err, limit=160)
        uid = _parse_uid(info)
        if uid is not None:
            return uid
        history.append(f"{attempt}:no_uid_fields(rc={rc})")
    raise RuntimeError(
        f"无法解析 {_PKG} 的 uid（attempts={attempts_made}/{max_attempts} "
        f"waited={waited:.1f}s/{wait_budget:.0f}s "
        f"history=[{','.join(history)}] dumpsys={snippet!r}）"
    )
...
def _root_read_prefs() -> tuple[str, str]:
    ...
    为什么必须同源（#2979 对 #2846 判据的收紧）：v1.2.1/v1.2.2 的
    ``get_prefs_xml``（cat）与 ``_prefs_file_state``（test -f）是**两次独立 adb
    调用**——第一次 cat 撞瞬时失败（超时 rc=-1 / 设备下线）被当成「读不到」，
    紧接着第二次 test -f 成功回 present ⇒ 删除健康 prefs，续跑 current_count
    归零、auto_resume 断链。「一败一成」的交错不需要概率假设：两条命令各自
    走一次 adb fork，设备重启窗里这就是常态。
    """
    probe = (
...
def get_prefs_xml() -> str:
    ...
    **空串只表示"此刻读不到"，不表示"文件不存在"**（存在性判定见 ``_root_read_prefs``）。
    """
    if is_root():
...
def _prefs_file_state() -> str:
    ...
    v1.2.3 起 ``repair_prefs_ownership`` 改用 ``_root_read_prefs`` 的单调用同源证据，
    本函数保留供其它读取路径与历史测试锚点使用。
    """
    rc, out, _ = adb(
        "shell",
...
def repair_prefs_ownership() -> None:
    ...
    v1.2.3 判据（对照 v1.2.1/v1.2.2 的「两次独立调用一败一成即误删」）：
    - 非 root 一律不动（沿用 v1.2.1）；
...
def _att_env_float(name: str, default: float, minimum: float = 0.0) -> float:
    raw = os.getenv(name)
    if raw is None:
        return default
    try:
...
def _att_env_int(name: str, default: int, minimum: int = 1) -> int:
    raw = os.getenv(name)
    if raw is None:
        return default
    try:
...
def att_install_max_attempts() -> int:
    """push+pm install 总尝试次数（v1.2.2；``STP_ATT_INSTALL_MAX_ATTEMPTS`` 可覆盖）。"""
    return _att_env_int("STP_ATT_INSTALL_MAX_ATTEMPTS", _ATT_INSTALL_DEFAULT_MAX_ATTEMPTS)


def att_install_ready_seconds() -> float:
    """单次「等系统就绪」上限秒数（v1.2.2；``STP_ATT_INSTALL_READY_SECONDS`` 可覆盖）。"""
    return _att_env_float("STP_ATT_INSTALL_READY_SECONDS", _ATT_INSTALL_DEFAULT_READY_SECONDS)


def att_install_wait_budget_seconds() -> float:
    """等待就绪的总预算秒数（v1.2.2；不含 push/pm 执行时间，``STP_ATT_INSTALL_WAIT_BUDGET_SECONDS`` 可覆盖）。"""
    return _att_env_float(
        "STP_ATT_INSTALL_WAIT_BUDGET_SECONDS", _ATT_INSTALL_DEFAULT_WAIT_BUDGET_SECONDS
    )


def pcs_retry_max_attempts() -> int:
    """服务启动 / uid 解析的总尝试次数（v1.2.4；``STP_PCS_RETRY_MAX_ATTEMPTS`` 可覆盖）。"""
    return _att_env_int("STP_PCS_RETRY_MAX_ATTEMPTS", _PCS_RETRY_DEFAULT_MAX_ATTEMPTS)


def pcs_retry_ready_seconds() -> float:
    """单次「等系统就绪」上限秒数（v1.2.4；``STP_PCS_RETRY_READY_SECONDS`` 可覆盖）。"""
...
def pcs_retry_wait_budget_seconds() -> float:
    ...
    return _att_env_float(
        "STP_PCS_RETRY_WAIT_BUDGET_SECONDS", _PCS_RETRY_DEFAULT_WAIT_BUDGET_SECONDS
    )
...
def system_ready() -> tuple[bool, str]:
    """设备系统就绪：``get-state==device`` 且 ``sys.boot_completed==1``（v1.2.2）。

    与 ``check_device v1.0.2`` / ``powercycle_finish`` 同判定——只等 adbd 可见
    （``wait-for-device``）不够：adbd 在 boot 早期即在线，此时 push/pm 会拿到
    ``device is still booting`` / ``device not found``。
    """
    rc, out, err = adb("get-state", timeout=15)
    state = (out or "").strip()
    if rc != 0 or state != "device":
        return False, f"state={state or _adb_diag(out, err, limit=40)!r}"
    rc, out, err = adb("shell", "getprop sys.boot_completed", timeout=15)
    if rc != 0:
        return False, f"boot_prop rc={rc}: {_adb_diag(out, err, limit=40)}"
    value = (out or "").strip()
    if value != "1":
        return False, f"boot_completed={value!r}"
    return True, "ready"
...
def _classify_install_failure(text: str) -> str:
    """把 push/pm 失败文本归成短标签（进 history=，便于跨窗聚合）。"""
    low = (text or "").lower()
    for kw, name in (
        ("not found", "not_found"),
        ("offline", "offline"),
        ("unauthorized", "unauthorized"),
        ("still booting", "still_booting"),
        ("closed", "closed"),
        ("no space", "no_space"),
        ("timeout", "timeout"),
    ):
...
def install_apk(apk: Path) -> None:
    """force-stop → uninstall → push + pm install（v1.2.2，D1′-同款等待）。
    ...
    v1.2.2（#2802，r477 取证：残差 59 条里 21 条 ``device is still booting`` +
    19 条 ``push: device not found``）：``wait-for-device`` 只等 adbd 可见、
    10s 退避又短于 ~75s 重启周期 ⇒ 重试仍落在重启窗内。改为重试前等
    **系统就绪**（``get-state==device`` 且 ``sys.boot_completed==1``，与
    ``check_device v1.0.2`` 同判定），尝试上限默认 3 次、等待总预算默认 90s
    （``STP_ATT_INSTALL_MAX_ATTEMPTS`` / ``STP_ATT_INSTALL_READY_SECONDS`` /
    ``STP_ATT_INSTALL_WAIT_BUDGET_SECONDS`` 可覆盖）；失败报文保留原文并追加
    ``attempts=`` / ``history=[``（逐次归类）。**首试仍走快速路径**（不等就绪），
    正常设备耗时不变；判定语义不变（仍要求 ``Success``）。

    v1.2.5（#3140，审计取证：v1.2.4 的 ``wait_deadline`` 起在循环**之前**，
    attempt 1 的 push/pm（各 ``timeout=300``）耗时被计入等待预算 ⇒ 慢安装时
    ``max_attempts`` **静默塌缩**、报文写成 ``attempts=1/3`` 把归因指向设备）：
    预算改为**累计等待时间**计账（push/pm 执行耗时不再吞预算），``attempts=``
    取实际执行过的尝试数、并补 ``waited=X.Ys/90s`` 供归类；等待上限与重试次数
    语义不变（仍由 ``STP_ATT_INSTALL_*`` 覆盖）。
    """
    ...
    max_attempts = att_install_max_attempts()
    ready_seconds = att_install_ready_seconds()
    # v1.2.5（#3140）：预算只计等待——不含 push/pm 执行时间。
    wait_budget = att_install_wait_budget_seconds()
    waited = 0.0
    attempts_made = 0
    history: list[str] = []
    ...
    for attempt in range(1, max_attempts + 1):
        if attempt > 1:
            ...
            if remaining <= 0:
                ...
                break
            wait_start = time.time()
            ok, observed = wait_system_ready(time.time() + min(ready_seconds, remaining))
            waited += time.time() - wait_start
            if not ok:
        ...
        if rc != 0:
            ...
            history.append(f"{attempt}:push_{_classify_install_failure(err or out)}")
        else:
            ...
            history.append(f"{attempt}:install_{_classify_install_failure(err or out)}")
        if attempt < max_attempts:
            adb_shell(f"am force-stop {_PKG}", timeout=30)
            time.sleep(backoff)
    raise RuntimeError(
        f"安装失败: {apk.name}: {last_reason} "
        f"(attempts={attempts_made}/{max_attempts} waited={waited:.1f}s/{wait_budget:.0f}s "
...
def service_probe() -> tuple[bool, int, str]:
    """``dumpsys activity services`` 探测（v1.2.4：带 rc 与有界片段，供失败报文取证）。

    返回 (是否可见服务, rc, 片段)。与 ``service_alive`` 同判据（文本含
    ``PowerCycleService``），只是换成 rc 感知的 ``adb`` 调用。
    """
    rc, out, err = adb("shell", f"dumpsys activity services {_PKG}", timeout=30)
    text = out or ""
    return ("PowerCycleService" in text), rc, _adb_diag(text, err, limit=160)
