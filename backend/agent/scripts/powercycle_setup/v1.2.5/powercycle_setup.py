...
**静默塌缩**、报文写成 `attempts=1/3` 把归因指向设备（审计注 2026-09-22 05:01Z，
live main `45c159cf` 复验）。现：
- `install_apk` / `get_app_uid`（_lib.py）：只累计 `wait_system_ready` 的等待
  时长，执行耗时不再吞预算；
- 失败报文 `attempts=` 取**实际执行过的尝试数**，并补 `waited=X.Ys/90s`——
  「预算耗尽」与「设备未就绪」两类可直接区分；
- 等待上限 / 重试次数 / 判定语义均不变（`STP_ATT_INSTALL_*` / `STP_PCS_RETRY_*`
  覆盖口径不变）。
