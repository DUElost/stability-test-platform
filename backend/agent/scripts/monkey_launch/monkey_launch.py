"""Monkey 启动：仅启动 MonkeyTest.sh 看门狗 + aimwd 守护进程。

不做资源推送（由 monkey_resource_push 负责），只启动进程并验证看门狗运行。

v5.0.1（#809）：aimwd 是普通脚本文件（旧资源链按 is_dir 守卫永远推不上），
启动后必须 post-check 到 MonkeyWatchdog 进程；未见即 fail——nohup 的 shell
rc 恒 0，不验证会把"双看门狗只剩单层"静默记成启动成功。

v5.2.0（#2048）：补回 v5.1.0 从 v5.0.1 拷贝时丢失的 #1711——aimwd post-check
使用**独立** max_wait 窗口；v5.0.1/v5.1.0 与 MonkeyTest.sh 共用 deadline，sh
迟到时 aimwd 预算已耗尽，会把「双看门狗」误判成 aimwd 未运行。

v5.1.0（#1690）：两个启动后轮询（MonkeyTest.sh / aimwd）逐次 progress_tick
——停滞钟只认 PROGRESS 戳（#115），max_wait_seconds 拉长后的静默等待会被
误判停滞。

环境变量:
    STP_DEVICE_SERIAL      (required)
    STP_STEP_PARAMS        (optional JSON)

STP_STEP_PARAMS:
{
    "need_nohup": true,
    "watchdog_script": "MonkeyTest.sh"
}

输出 (stdout):
    {"success": true/false, "metrics": {...}}
"""

import subprocess
import sys
import time

from _adb import adb_path, device_serial, output_result, params, progress_tick


def _shell(serial: str, cmd: str, timeout: int = 30) -> tuple[int, str]:
    result = subprocess.run(
        [adb_path(), "-s", serial, "shell", cmd],
        capture_output=True, text=True, timeout=timeout,
    )
    return result.returncode, (result.stdout or "").strip()


def _ps_grep(serial: str, pattern: str, timeout: int = 10) -> bool:
    """Check if a process matching `pattern` is running on device."""
    rc, out = _shell(
        serial,
        f"ps -ef | grep '{pattern}' | grep -v grep | head -1",
        timeout=timeout,
    )
    return rc == 0 and len(out.strip()) > 0


def main():
    serial = device_serial()
    args = params()

    need_nohup = bool(args.get("need_nohup", True))
    watchdog_script = args.get("watchdog_script", "MonkeyTest.sh")
    max_wait = int(args.get("max_wait_seconds", 15))

    t0 = time.time()

    # ── 预检 ──
    rc, _ = _shell(serial, "echo ready", timeout=10)
    if rc != 0:
        output_result(False, error_message=f"Device {serial} not reachable")
        sys.exit(1)

    # ── 检查是否已在运行 ──
    if _ps_grep(serial, watchdog_script):
        output_result(
            True,
            metrics={"serial": serial, "already_running": True, "duration_s": round(time.time() - t0, 1)},
        )
        return

    # ── 启动 MonkeyTest.sh ──
    if need_nohup:
        cmd = (
            "cd /data/local/tmp; "
            "nohup sh /data/local/tmp/MonkeyTest.sh >/dev/null 2>&1 &"
        )
    else:
        cmd = "nohup sh /data/local/tmp/MonkeyTest.sh >/dev/null 2>&1 &"

    rc, _ = _shell(serial, cmd, timeout=30)
    if rc != 0:
        output_result(False, error_message=f"MonkeyTest.sh start failed (rc={rc})")
        sys.exit(1)

    # ── 启动 aimwd 守护 ──
    _shell(serial, "nohup /data/local/tmp/aimwd >/dev/null 2>&1 &", timeout=15)

    # ── Post-check: 等待 MonkeyTest.sh 出现 ──
    sh_deadline = time.time() + max_wait
    sh_running = False
    while time.time() < sh_deadline:
        if _ps_grep(serial, watchdog_script):
            sh_running = True
            break
        progress_tick("watchdog_wait", elapsed_s=round(time.time() - t0, 1))
        time.sleep(2)

    # ── Post-check: aimwd 守护进程出现（#809；#1711 独立窗口）──
    aimwd_deadline = time.time() + max_wait
    aimwd_running = False
    while time.time() < aimwd_deadline:
        if _ps_grep(serial, "MonkeyWatchdog"):
            aimwd_running = True
            break
        progress_tick("aimwd_wait", elapsed_s=round(time.time() - t0, 1))
        time.sleep(2)

    elapsed = round(time.time() - t0, 1)

    if not sh_running:
        output_result(
            False,
            error_message=f"MonkeyTest.sh not running after {max_wait}s",
            metrics={"duration_s": elapsed},
        )
        sys.exit(1)

    if not aimwd_running:
        output_result(
            False,
            error_message=f"aimwd (MonkeyWatchdog) not running after {max_wait}s",
            metrics={"duration_s": elapsed},
        )
        sys.exit(1)

    output_result(
        True,
        metrics={
            "serial": serial,
            "watchdog_started": True,
            "aimwd_started": True,
            "duration_s": elapsed,
        },
    )


if __name__ == "__main__":
    main()
