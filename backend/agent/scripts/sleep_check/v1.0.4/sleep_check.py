...
v1.0.4（#1693）：#814 离线豁免分支不写键——新 Job 首拍（v1.0.2 的 job_id
重置后 state 仅含 job_id）遇设备离线时走 ``pass``，随后对
``state["dead_streak"]`` 的无条件读取抛 KeyError，被 main() 捕获报
failure——离线反而必判失败，豁免分支自爆。修复：离线分支同样把
dead_streak 归一入 state（保持现值、不累计），保证判死比较前键必然在场。

v1.0.3（#814，R08）：判死前置设备在线判定——设备离线/adb 拥塞时服务探测
必然为空，不再累计 dead_streak（一次意外重启/USB 节能断连不再导致假死）。

v1.0.2（#1028，R08-F10）：与 mtbf_check v1.4.0 同型——巡检状态写入``job_id``（STP_JOB_ID），新 Job 不继承上一 Job 的 dead_streak/seq。
