...
def instrument_alive() -> bool:
    """设备端循环脚本或 instrument 进程在跑（shell 侧 ps -A 过滤）。"""
    out = adb_shell("ps -A", timeout=30)
    return any(token in out for token in ("gpu_stress_loop", "TestStressGpu", "AndroidJUnitRunner"))
...
def stop_stress() -> None:
    ...
    adb_shell("pkill -f gpu_stress_loop", timeout=30)
    adb_shell("pkill -f 'am instrument|TestStressGpu|AndroidJUnitRunner'", timeout=30)
