...
def _cleanup_device_script() -> None:
    """删除设备端循环脚本并回读验证（v1.0.4/#894 清理完整化）。

    只删平台自产循环脚本；test_log.txt 保留（原始日志未进中心存储，
    删除即丢证——链式衔接的残留风险来自脚本，不是日志文件）。
    回读仍存在 → raise（teardown 报错而非静默残留）。
    """
    adb_shell(f"rm -f {_DEVICE_SCRIPT}", timeout=30)
    out = adb_shell(f"[ -e {_DEVICE_SCRIPT} ] && echo REMAINS || echo CLEAN", timeout=30)
    if "REMAINS" in out:
