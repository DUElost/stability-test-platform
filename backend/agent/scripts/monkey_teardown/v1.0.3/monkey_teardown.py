...
v1.0.3（#2285）：`cleanup_paths` 逐项 shell 引号化——该参数由计划作者提供并直接
拼进设备侧 shell（`rm -rf` 与回读探针的 `for` 列表），含空格的路径会拆坏列表、
含 `;`/`$( )`/反引号的路径会成为设备侧命令执行。改 `shlex.quote` 后含空格的路径
仍能正确删除与回读，且不再有注入面。
...
def _cleanup_device_resources(paths: list[str], verify: bool) -> dict:
    """删除设备端 monkey 资源，可选回读验证（#894，v1.0.2；引号化 v1.0.3）。
    ...
    errors: list[str] = []
    remaining: list[str] = []
    # v1.0.3（#2285）：逐项 shlex.quote——路径会拼进设备侧 shell，不引号化时
    # 空格会拆坏列表、`;`/`$( )`/反引号则是设备侧命令执行面。
