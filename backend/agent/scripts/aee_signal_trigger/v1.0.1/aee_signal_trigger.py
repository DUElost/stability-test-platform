...
v1.0.1（#816）：kill 前 pid 存活证据 + kill rc 判定（失败立即暴露，不再
盲等整个轮询窗口）；新 db_history 行做 pkg_name 归属核对后才认领——并发
来源的 AEE 事件不再被当成本次 kill 产物。
...
def main() -> None:
    ...
    try:
        ...
        # #816：kill 前 pid 存活证据 + kill rc 判定——盲 kill 会让失败被
        # 静默吞掉然后空等整个轮询窗口；先证存活再发信号，失败立即暴露。
        rc_alive, _, _ = _run_adb("shell", f"kill -0 {pid}", timeout=10)
        if rc_alive != 0:
            output_result(
                False,
                error_message=f"pid {pid} 在 kill 前不存在（monkey 拉起后快速退出？）",
                metrics={"killed_pid": pid},
            )
            return

        # kill -<signal> <pid>：默认 SIGSEGV(11) 触发 native crash → AEE 落盘
        rc_kill, _, err_kill = _run_adb("shell", f"kill -{signal} {pid}", timeout=10)
        if rc_kill != 0:
            output_result(
                False,
                error_message=f"kill -{signal} {pid} failed: rc={rc_kill} err={err_kill[:200]}",
                metrics={"killed_pid": pid},
            )
            return
        ...
        new_line = None
        unmatched_new_lines: list[str] = []
        waited = 0.0
        while time.monotonic() < deadline:
            time.sleep(poll_interval)
            waited += poll_interval
            after_lines = _db_history_lines()
            # 取按出现顺序第一个 before 不存在、且 pkg_name 归属本次目标的新行
            # （#816：并发来源的 AEE 事件不得被当成本次 kill 产物——“新行即
            # 本次产物”的旧判据会在多来源并发时误吸上报）。
            for ln in after_lines:
                if ln in before_line_set:
                    continue
                candidate = parse_db_history_line(ln)
                if candidate and candidate.get("pkg_name") == package:
                    new_line = ln
                    break
                if ln not in unmatched_new_lines:
                    unmatched_new_lines.append(ln)
            if new_line:
                break

        if not new_line:
            if unmatched_new_lines:
                output_result(
                    False,
                    error_message=(
                        f"kill -{signal} {pid} 后出现新 db_history 行但不属于 "
                        f"{package}（归属不匹配，拒绝上报）："
                        f"{unmatched_new_lines[0][:160]!r}"
                    ),
                    metrics={
                        "killed_pid": pid,
                        "wait_seconds": round(waited, 2),
                        "unmatched_new_lines": len(unmatched_new_lines),
                    },
                )
                return
            output_result(
