...
v1.0.10（#1695）：protobuf 字段值判定从**全等**改**前缀**——真机形态
``test_result\x12\x05true\x0f``（2026-09-01 实证，length 第 5 字节为下一
字段的 tag 字节 ``\\x0f``，非空白）经 ``bytes.strip()`` 仍是 ``b"true\\x0f"``，
v1.0.8/v1.0.9 的全等比较返回 None → verdict ``no-tests`` → 正常完成被判
「空跑」失败（PASS→FAIL 回归）。改为 ``startswith``：长度前缀仍在，
#746 的「最后一条为准 + 不被后文 true 子串干扰」语义不变。

v1.0.9（#1693）：同 sleep_check v1.0.4——#814 离线豁免分支不写键，新 Job
首拍（v1.0.6 的 job_id 重置后 state 仅含 job_id）遇设备离线时走 ``pass``，
随后对 ``state["dead_streak"]`` 的无条件读取抛 KeyError，被 main() 捕获报
failure——离线反而必判失败，豁免分支自爆。修复：离线分支同样把
dead_streak 归一入 state（保持现值、不累计），保证判死比较前键必然在场。

v1.0.8（#746 / #814，R08）：
  - protobuf `test_result` 判定改为按长度前缀取值（字段级），不再用 64B
    窗口子串扫描——最后一条 false 后续窗口内出现 "true" 文本不再误判成功；
  - 判死前置设备在线判定：离线/adb 拥塞不累计 dead_streak（意外重启/慢 boot
    不再假死）。
...
def _last_protobuf_test_result(log: bytes) -> "bool | None":
    ...
    文本（2026-09-01 全量实证）。protobuf 字段：``test_result`` 后接
    tag ``\\x12``（field 2, wire type 2 = length-delimited）→ 长度字节 →
    值（"true"/"false"）。

    v1.0.8（#746）：改为**按长度前缀取值**（字段级），不再用 64B 窗口里的
    子串扫描——后者在「最后一条 test_result=false 但后续 64B 内出现 true
    子串（如 stacktrace）时会误判成功」。找不到合法字段返回 None。

    v1.0.10（#1695）：值判定从全等改前缀——真机形态 length 多含一个下一字段
    的 tag 字节（``\\x12\\x05true\\x0f``，``\\x0f`` 非空白、strip 不去除），
    全等比较对真机日志恒返回 None（PASS→FAIL 回归）。
    """
    idx = log.rfind(b"test_result")
    if idx < 0:
        return None
    rest = log[idx + len(b"test_result"):]
    # 字段名后可能有 NUL/空白分隔（文本与二进制混排容错）——定位 wire tag。
    pos = 0
    while pos < len(rest) and rest[pos:pos + 1] in (b"\x00", b"\x20", b"\t", b"\n"):
        pos += 1
    if rest[pos:pos + 1] != b"\x12":
        return None
    length = rest[pos + 1] if pos + 1 < len(rest) else 0
    value = rest[pos + 2:pos + 2 + length].strip().lower()
    # 前缀判定而非全等：length 可能多吞一个尾随 tag 字节（真机 \x0f 形态），
    # 长度前缀仍限定值域，#746 的「最后一条为准」语义不变。
...
def _run(cfg: dict) -> dict:
    ...
    if alive:
        ...
        # #814：设备离线/adb 拥塞属正常重启窗口——不累计 dead_streak，也不判死。
        # #1693：仍须把键归一入 state——新 Job 首拍 state 无此键，跳过写入
        # 会让下方判死比较的无条件读抛 KeyError，离线豁免反而必判失败。
        state["dead_streak"] = int(state.get("dead_streak", 0))
    else:
