...
def _install_apk_stable(apk: Path) -> "tuple[int, str]":
    """v1.0.4：378MB Lite APK 的 adb install 流式安装不稳定（2026-08-31
    实证 2/3 失败）——改 push 到设备本地 + pm install（设备侧安装，
    不经 adb 流式通道）。失败自动重试一次（594 首次失败重试成功先例）。
    """
    tmp = "/data/local/tmp/stp_install.apk"
    rc, out = 1, ""   # v1.0.7（#755）：push 全失败时 out 未定义（NameError）
    for attempt in (1, 2):
        ...
        with progress_heartbeat(f"apk_push:{apk.name}"):
            push_rc, _, _ = adb("push", str(apk), tmp, timeout=600)
        if push_rc != 0:
            rc, out = push_rc, f"push failed (rc={push_rc})"
            continue
        with progress_heartbeat(f"apk_install:{apk.name}"):
            rc, out, _ = adb("shell", f"pm install -r -g -t -d {tmp}", timeout=300)
