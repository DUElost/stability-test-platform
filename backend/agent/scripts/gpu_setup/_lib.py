...
def decode_device_output(raw: bytes | None) -> str:
    """设备 / adb 输出解码：**宽容 UTF-8**（v1.2.3，#3069）。

    设备侧输出（instrument 日志、UI dump、getprop）可能是任意字节。``text=True``
    按 locale 严格解码，遇到一个非 UTF-8 字节就抛 ``UnicodeDecodeError``——它不是
    ``OSError``，调用点无从兜住，会把整个 setup 炸成 init 失败（v1.2.2 实测
    462/487 台倒在 ``0xf9`` 上）。坏字节替换成 U+FFFD 不影响签名匹配与后续判定。
    """
...
def adb(*args: str, timeout: int = 60) -> tuple[int, str, str]:
    ...
    try:
        # v1.2.3（#3069）：显式收字节再解宽容码，不用 ``text=True``——后者严格解码，
...
def gpu_config(cfg: dict) -> dict:
    ...
    return {
        "project": project,
        "gpu_resources_dir": cfg.get("gpu_resources_dir") or env("STP_GPU_RESOURCES_DIR", ""),
        ...
        "pre_reboot": str(pick("pre_reboot", "STP_GPU_PRE_REBOOT", None, "true")).lower() == "true",
        # #774：长循环前跑一轮 instrument 冒烟；撞上 UiAutomation/BaseTestCase 边界即 init 失败，
        # 避免 273 台把整窗烧完才发现 APK 不兼容。
        "compat_probe": str(pick("compat_probe", "STP_GPU_COMPAT_PROBE", None, "true")).lower() == "true",
    }
...
def _retry_uninstall_pkgs_for_apk(apk: Path) -> "tuple[str, ...]":
    """#755：重试前只卸「当前失败 APK」对应包，勿清同批已装成功的包。

    v1.0.7 已拆单包调用，但仍无时卸 FULL+LITE——Lite 变体装完 Full 再装
    Lite 失败时会把 Full 清掉；宿主 APK 失败时也会误卸 Antutu。
    v1.2.0（#2048）：本修复在 v1.1.0 从 v1.0.9 拷贝时丢失，此处补回并加
    版本回归守卫（tests: 最新版本目录必须保留本函数）。
    """
    name = apk.name
    if name == "antutu_benchmark_v10_3d.apk":
        return (_ANTUTU_FULL_PKG,)
    if name == "Antutu_3D_Lite_10.2.9.apk":
        return (_ANTUTU_LITE_PKG,)
    if name == "scripts-debug.apk":
        return (_HOST_PKG,)
    if name == "scripts-debug-androidTest.apk":
        return (_HOST_TEST_PKG,)
    return ()


def _gpu_install_backoff_seconds() -> float:
    """重试退避秒数（env 可覆盖；非法值回落默认）。"""
    raw = os.getenv("STP_GPU_INSTALL_RETRY_BACKOFF_SECONDS", "10")
    try:
...
def _install_apk_stable(apk: Path) -> "tuple[int, str]":
    """v1.0.4：流式安装不稳 → push + pm install（设备侧安装）。

    v1.0.7（#755）/v1.0.10/v1.2.0（#2048）：重试卸载收敛到失败 APK 对应包。
    v1.2.1（#2756，run 431 取证）：push 失败输出保留（连接层错误可见）；
    重试前 wait-for-device + 短退避（``STP_GPU_INSTALL_RETRY_BACKOFF_SECONDS``，
    默认 10s）——吸收链交接瞬时 adb 风暴。
    """
    tmp = "/data/local/tmp/stp_install.apk"
    rc, out = 1, "no attempt made"   # v1.0.7（#755）：push 全失败时 out 未定义
    backoff = _gpu_install_backoff_seconds()
    for attempt in (1, 2):
        # #1690：378MB 级 APK 的 push/pm install 可达数分钟——全程打 PROGRESS
        # 心跳，否则启用 stall_seconds 的 Plan 会把它当停滞误杀。
        with progress_heartbeat(f"apk_push:{apk.name}"):
            push_rc, push_out, push_err = adb("push", str(apk), tmp, timeout=600)
        if push_rc != 0:
            # v1.2.1（#2756）：push 的 stderr/stdout 不再丢弃（连接层错误
            # 可见：device offline / no space 等）。
            rc, out = push_rc, (
                f"push failed (rc={push_rc}): " + _adb_diag(push_out, push_err)
            )
        else:
            ...
            if rc == 0 and "Success" in out:
                return rc, out
            out = f"{out.strip() or 'rc=%d' % rc}: {_adb_diag('', err)}"
        if attempt == 1:
            # v1.0.7：pm uninstall 只接受单个包名——逐包调用
            # v1.0.10（#755）/ v1.2.0（#2048）：只卸当前失败 APK 对应包
            for pkg in _retry_uninstall_pkgs_for_apk(apk):
                adb("shell", f"pm uninstall {pkg}", timeout=60)
            # v1.2.1（#2756）：跨出链交接瞬时 adb 风暴窗口再重试。
...
def install_apks(apk_dir: Path, apks: tuple[str, ...]) -> dict:
    ...
    for name in apks:
        ...
        rc, out = _install_apk_stable(apk)
        if rc != 0 or "Success" not in out:
            raise RuntimeError(f"安装失败: {name}: {out.strip() or 'rc=%d' % rc}")
    return shas


def dismiss_antutu_dialogs(pkg: str) -> None:
    """#774 run 356/357 根因修复：Antutu 首启弹窗阻塞测试。

    Antutu 3D Bench 启动时弹「Security Warning: Please enable development
    settings first!」（即使 development_settings_enabled=1）——弹窗阻塞在
    TransferActivity，测试的 UiAutomator 未能点击 → 测试检测
    「antutu app start」超时 81s → tearDown AssertionError → 每轮失败。

    此处在 prepare_device 后预热启动 Antutu → 检测弹窗（uiautomator dump
    按文本找 OK）→ 点击 → 关闭。一次性确认标记写入后测试即可正常执行
    （手动验证：点掉弹窗后 instrument OK (1 test)）。
    """
    adb_shell(f"monkey -p {pkg} -c android.intent.category.LAUNCHER 1", timeout=30)
    time.sleep(8)
    adb_shell("uiautomator dump /sdcard/stp_ui.xml", timeout=30)
    _, xml, _ = adb("shell", "cat /sdcard/stp_ui.xml", timeout=30)
    m = re.search(
        r'text="(?:OK|确定|知道了|允许)"[^>]*bounds="\[(\d+),(\d+)\]\[(\d+),(\d+)\]"',
        xml or "",
    )
    if m:
        x = (int(m.group(1)) + int(m.group(3))) // 2
        y = (int(m.group(2)) + int(m.group(4))) // 2
        adb_shell(f"input tap {x} {y}", timeout=30)
        time.sleep(3)
    adb_shell(f"am force-stop {pkg}", timeout=30)
    adb_shell("rm -f /sdcard/stp_ui.xml", timeout=30)


#: #774 全量实证签名——命中即判定为 transsion 测试框架兼容边界（平台侧不可根治）。
_COMPAT_SIGNATURES: tuple[tuple[str, str], ...] = (
    ("uiautomation_already_registered", r"UiAutomationService.*already registered"),
    ("basedtestcase_uidevice_npe", r"NullPointerException:.*UiDevice\.(?:isScreenOn|wakeUp)"),
    ("basedtestcase_teardown_npe", r"BaseTestCase\.tearDown"),
)


def classify_compat_failure(blob: str) -> str | None:
    """从 instrument 输出里识别 #774 兼容边界签名；未命中返回 None。"""
    if not blob:
        return None
    for name, pattern in _COMPAT_SIGNATURES:
        if re.search(pattern, blob, re.IGNORECASE | re.DOTALL):
            return name
    return None


def run_compat_probe(test_id: str, *, timeout_s: int = 180) -> None:
    """长循环前跑一轮 instrument 冒烟（#774）。

    命中 UiAutomation / BaseTestCase NPE 等已知签名 → raise，init 快速失败，
    避免全量机队把整窗 GPU 轮次烧在 APK 框架崩溃上。探测前 force-stop 测试包，
    降低「上一窗残留」假阳性；成功则继续正常 launch_stress。
    """
    for pkg in (_HOST_PKG, _HOST_TEST_PKG):
        adb_shell(f"am force-stop {pkg}", timeout=30)
    probe_log = "/sdcard/Auto/stp_gpu_compat_probe.txt"
    adb_shell(f"rm -f {probe_log}", timeout=30)
    class_name = (
        "com.transsion.testcaserepository.stressgpu."
        f"TestStressGpuExecute#test_StressSpecial_GPUTest_{test_id}"
    )
    cmd = (
        f"am instrument -w -m "
        f"-e listener com.transsion.common.TestCaseRunListener "
        f"-e debug false -e loop 1 "
        f"-e class {class_name} "
        f"{_HOST_TEST_PKG}/androidx.test.runner.AndroidJUnitRunner "
        f"> {probe_log} 2>&1; echo __STP_PROBE_RC=$? >> {probe_log}"
    )
    # 整条 shell 超时：instrument 卡住时也不拖死 setup（默认 180s）。
    adb_shell(cmd, timeout=timeout_s)
    blob = adb_shell(f"cat {probe_log}", timeout=60)
    sig = classify_compat_failure(blob)
    if sig:
        raise RuntimeError(
            f"GPU 兼容边界探测失败（#774 signature={sig}）："
            f"transsion 测试框架在本机不可用，跳过长循环以免整窗空转。"
            f" 证据片段：{blob[-400:].replace(chr(10), ' | ')}"
        )
