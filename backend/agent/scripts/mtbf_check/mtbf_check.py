...
v1.5.0（#810，R08）：run_dir 以 setup 绑定为准（按项目匹配；无绑定回退
「最新目录」并打戳留痕），不再无条件取最新目录——避免残留目录导致
testpoints_done/log_bytes 数到上一轮数据。

v1.4.0（#1028，R08-F10）：巡检状态按 **Job** 隔离——状态文件键此前只有设备序列号，``dead_streak``/``seq`` 跨 Job 继承：上一 Job 留下的死亡计数会在新 Job 首个周期就背上旧账、提前耗尽宽限。现在状态写入 ``job_id``（Agent 注入的 STP_JOB_ID），身份变化即重置为全新状态；同 Job 崩溃恢复仍延续计数。
...
v1.3.0（ADR-0030 P1b env 退役，#404 PR-D）：``expected_testpoint_count``
改为**只读 STP_STEP_PARAMS**——托管绑定由控制面按套件自动注入
（P1 设计 §3.4），fleet 单值 env 预置 ``STP_MTBF_EXPECTED_TESTPOINT_COUNT``
已从 `_FLEET_ENV_KEYS` 摘除且本版本不再读取（旧版 v1.2.0 的读取按 ADR-0020
版本不可变保留；无绑定 Plan 无注入 → 只报绝对数，安全降级）。

STP_STEP_PARAMS:
{
    "project": "legacy",
    "expected_testpoint_count": 130,   // 托管绑定自动注入；0/缺失=只报绝对数
...
def _bound_run_dir(project: str) -> str:
    """#810：优先读 setup 绑定的 run_dir；无绑定才回退「最新」并留痕。

    回退仅用于兼容旧 setup（尚未写绑定）或手动执行——生产新链路下 setup 必然
    先写绑定，避免 check 在残留目录下数到上一轮数据。
    """
    ...
    if latest:
        progress_stamp({"event": "mtbf_check_run_dir_unbound_fallback",
                        "project": project, "run_dir": latest})
    return latest
...
def _run(cfg: dict) -> dict:
    # v1.3.0：只认 STP_STEP_PARAMS 注入，env 预置退役（ADR-0030 P1 设计 §3.4）。
    expected = int(str(cfg.get("expected_testpoint_count") or 0) or 0)
    grace = int(str(param_or_env(cfg, "dead_grace_cycles", "STP_MTBF_DEAD_GRACE_CYCLES", "2")) or 2)
