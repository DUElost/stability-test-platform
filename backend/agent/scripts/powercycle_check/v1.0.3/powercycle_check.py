...
v1.0.3（综合验收方案 A）：**定时收取窗口**——``collect_window_start``
（"HH:MM"，**东八区业务时间**，v1.0.3 修正：主机时区各异（实测 PDT），
不能按主机本地时间判定）+ ``collect_window_minutes``（10/30/60）Per-Plan
...
def _run(cfg: dict) -> dict:
    ...
    # ⑥ 修复（验收发现）：设备刚上线（offline→online 转换）属 boot 窗口，
    # 服务未起不累计 dead_streak——重启周期 ~75s > patrol 60s，boot 窗口
    # 可跨 2 个 check（此前误判「连续 N 周期未存活」）。
    if alive or not was_online:
        state["dead_streak"] = 0
    else:
