...
v1.0.4（综合验收方案 A）：**定时收取窗口**——``collect_window_start``
（"HH:MM"，**东八区业务时间**，主机时区各异（实测 PDT），不能按主机本地
时间判定）+ ``collect_window_minutes``（10/30/60）Per-Plan。⑥ boot 窗口
判死修复补强：cycles_done==0（首个 boot 窗口）不累计 dead_streak。
...
def _run(cfg: dict) -> dict:
    ...
    if alive or cycles_done == 0 or not was_online:
        state["dead_streak"] = 0
    else:
