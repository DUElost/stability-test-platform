...
v1.0.1（#809）：aimwd 是普通脚本文件（旧 is_dir 守卫让它永远推不上设备，
双看门狗只剩单层）——按 is_file 推送并纳入 required_files，由 post-check
统一验证 test -f + 非零大小；bundle 中缺失 aimwd 视为资源错误 fail。

v1.1.0（#1690）：所有 adb push（含 timeout=300 的媒体资源）接入 PROGRESS
...
from _adb import adb_path, device_serial, output_result, params, progress_heartbeat
...
def _push(serial: str, local: str, remote: str, timeout: int = 60) -> bool:
    if not os.path.exists(local):
        return False
    with progress_heartbeat(f"push:{os.path.basename(local)}"):
...
def main():
    ...
    # ── aimwd 守护进程（#809：普通脚本文件——旧 is_dir 守卫让它永远推不上；
    #     纳入 required_files 由 Post-check 统一回验）──
    local_aimwd = aimonkey_dir / "aimwd"
    if local_aimwd.is_file():
        if not _push(serial, str(local_aimwd), "/data/local/tmp/aimwd"):
            errors.append("push failed: aimwd")
        else:
            pushed += 1
        required_files.append(("aimwd", "/data/local/tmp/aimwd"))
        _shell(serial, "chmod 777 /data/local/tmp/aimwd", timeout=10)
    else:
        errors.append("missing resources: aimwd")
