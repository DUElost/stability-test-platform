"""aee_prepare v1.0.1 — 荣耀项目测试前 AEE/日志配置准备。

v1.0.1（#816）：开发者设置恢复移入 finally（失败/异常/早退路径不再遗留
development_settings_enabled=1）；adb root 后轮询 get-state==device 再
setprop（上限 15s，超时由 rc+回读核验兜底）。
...
def _wait_adbd_ready(serial: str, deadline_seconds: float = 15.0) -> bool:
    """adb root 后轮询 adbd 恢复（#816）：get-state == device 才算就绪。"""
    deadline = time.monotonic() + deadline_seconds
    while time.monotonic() < deadline:
        rc, out = _shell(serial, "get-state", timeout=5)
...
def main() -> None:
    ...
    try:
        ...
        # #816：adbd 重启窗口——轮询 get-state==device 再走 setprop（上限 15s；
        # 超时继续，后续 setprop 的 rc+回读核验兜底）
        _wait_adbd_ready(serial)
        ...
        # #816：恢复开发者设置移到 finally——失败/异常/早退路径不再遗留
        # development_settings_enabled=1 的设备态（原实现只在成功路径恢复）。
        _emit_progress("dev-settings-off")
        try:
            _shell(serial, "settings put global development_settings_enabled 0")
        except Exception:  # noqa: BLE001 — 恢复失败不覆盖主结果
            pass
