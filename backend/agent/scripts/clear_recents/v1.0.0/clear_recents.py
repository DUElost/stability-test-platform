...
STP_STEP_PARAMS schema:
    ...
    after_tap_seconds   : float  (default 1.2; 点击清除后等待)
...
def _count_tasks(xml: str) -> int:
    # ZTE MiFavor task cards; also accept AOSP-ish task_view.
...
def _already_clear(xml: str) -> bool:
    """Overview open but no tasks / no clear button → treat as done."""
    if _count_tasks(xml) == 0 and _find_clear_target(xml) is None:
        # Still on launcher overview or home: package hint optional.
        return True
    return False
...
def main() -> None:
    ...
    after_tap = _as_float(cfg.get("after_tap_seconds"), 1.2)
    ...
    with progress_heartbeat("clear_recents"):
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            tasks_before = _count_tasks(last_xml)
            if metrics["tasks_before"] is None:
                metrics["tasks_before"] = tasks_before

            target = _find_clear_target(last_xml)
            if target is None:
                if tasks_before == 0 or _already_clear(last_xml):
            ...
            # Re-open overview to verify task cards are gone.
            _open_overview()
            time.sleep(open_settle)
            verify_xml = _dump_ui(dump_path)
            tasks_after = _count_tasks(verify_xml)
            metrics["tasks_after"] = tasks_after
            adb_shell(f"input keyevent {_KEYCODE_HOME}", timeout=15)
            adb_shell_quiet(f"rm -f {dump_path}", timeout=10)

            if tasks_after == 0 or _find_clear_target(verify_xml) is None:
                output_result(True, metrics=metrics)
                return
        ...
        output_result(
            ...
            error_message=(
                ...
                f"tasks_before={metrics['tasks_before']} "
                f"tasks_after={metrics['tasks_after']} "
