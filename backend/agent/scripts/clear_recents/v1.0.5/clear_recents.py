...
v1.0.5（#3107）：**`dump_path` 校验**。该值来自计划参数（`STP_STEP_PARAMS`）并被
插进设备端 shell（`rm -f {dump_path}` / `cat` / uiautomator 重定向），且流程以 root
执行：此前 `"/"` 会展开成设备端 `rm -f /`，含空格 / `;` / `$()` 的值可扩张成任意
...
STP_STEP_PARAMS schema:
    ...
    dump_path           : str    (default /data/local/tmp/stp_clear_recents.xml;
                                 v1.0.5 起必须匹配 ^/data/local/tmp/[A-Za-z0-9._-]+$)
...
#: `/data/local/tmp/` 下的普通文件名——空值/含元字符/含空格/绝对穿越一律拒绝。
...
def validated_dump_path(raw: object) -> str:
    ...
    if not _DUMP_PATH_RE.match(text) or text.rsplit("/", 1)[-1] in (".", ".."):
        raise ValueError(
            "dump_path 必须匹配 ^/data/local/tmp/[A-Za-z0-9._-]+$（且非 `.`/`..`）"
            f"（收到 {text!r}）"
        )
    return text
