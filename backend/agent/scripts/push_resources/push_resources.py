...
v1.0.1（#812）：bundle 解包链（tar/rm/marker 写入）改为 rc 判定，并在解包后
回读 `.stp_bundle_sha256` 核验 marker——半途失败不再报 success。

v1.1.0（#1690）：bundle push / tar 解包 / 逐文件 push 长阻塞段接入 PROGRESS
心跳——停滞钟只认 PROGRESS 戳（#115），长静默段会被误判停滞。
...
def _push_files(serial: str, files: list) -> dict:
    ...
    for f in files:
        ...
        if not local or not remote:
            continue
        try:
            with progress_heartbeat(f"push:{os.path.basename(local) or remote}"):
...
def _push_bundle(serial: str, bundle: str, manifest_path: str, remote_dir: str, skip_if_match: bool) -> dict:
    ...
    try:
        ...
        with progress_heartbeat(f"bundle_push:{bundle_name}"):
            adb_push(bundle, remote_bundle)
        adb_push(manifest_path, f"{remote_dir}/manifest.json")
        with progress_heartbeat(f"bundle_unpack:{bundle_name}"):
            unpack = adb_shell_quiet(
        ...
        if unpack.returncode != 0:
            return {
                "success": False,
                "error": (
                    f"bundle unpack failed: rc={unpack.returncode} "
                    f"err={(unpack.stderr or '').strip()[:200]}"
                ),
            }
        verify = adb_shell_quiet(f"cat {marker} 2>/dev/null", timeout=10)
        if (verify.stdout or "").strip() != expected_sha:
            return {"success": False, "error": "bundle sha marker verification failed"}
        return {
            "success": True,
