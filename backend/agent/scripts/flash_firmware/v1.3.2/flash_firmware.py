...
def main() -> None:
    ...
    try:
        for attempt in range(1, max_attempts + 1):
            ...
            # Best-effort: hand the device into flash mode via ADB before invoking
            # flash_tool. flash_tool itself USB-polls, so a failed reboot doesn't
            # break the flow. 重试轮里即使 params 关了 reboot 也照发（设备多半
            # 已在 BROM/DA 态，adb 调用会 no-op）。
            if bool(args.get("reboot_to_flash", True)) or attempt > 1:
                pre_reboot = _reboot_into_flash_mode(
                    serial=serial,
                    target=reboot_target,
                    adb_path=adb_path,
                    wait_seconds=pre_reboot_wait,
                )

            # reboot 进入 flash 模式本身是一个阶段（#134）
            if pre_reboot.get("attempted"):
                _emit_progress(seq, stage="reboot",
                               target=reboot_target)
        ...
        # 门控恢复必须先于 verify 阶段，且覆盖所有异常路径——不能把同 host
        # 其它手机留在 authorized=0 的隐藏态。
