...
v1.3.17（#2353）：ttyACM 写入路径判据改按**规则形态 × 本进程组集合**——
`_udev_has_mtk_0666_rule()`（只认 0666 文本）换成 `_udev_rule_form()`
（`0660+dialout` / `0666` / 无）。此前在已升 0660 的宿主上会误报
"no udev MODE=0666 rule"（只影响 WARNING 级），且若把"规则在位"当可写，
"0660 + 进程不在 dialout"这一**写不了**的状态会被放行；现该状态明确判否
并给出「补组 + 重启 agent」指引。

v1.3.16（#2133 / ADR-0037 D5）：门控回落面收敛为 wrapper 窄面——authorized
直写失败后改调 `stp-agent-priv usb-authorized --port <p> --value <v>`
（不再有 `sudo -n sh -c` 任意命令面；port 正则 + idVendor=0e8d 校验在
wrapper 内强制）。能力探针带子命令本体（#2011 教训）；wrapper 缺失/旧版本
时门控降级为 no-priv-face 并如实记录（非致命，单设备 host 无感）。

v1.3.15（#1591）：门控口持久化 + 启动孤儿恢复——abort/SIGKILL 若打断
``_settle_lock`` 的 authorized=1 恢复，邻机口会永久滞留隐藏态；下次进入
刷机前先读 ``/tmp/stp-flash-firmware.gated.json`` 补恢复。
...
STP_STEP_PARAMS schema:
    ...
    boot_stabilize_seconds  : int  (optional, default 20; 刷后 boot_completed=1 且
                          MTK USB 拓扑稳定的持续窗口——首刷二次重启窗口在锁内
                          消化；拓扑只取 MTK 口（见 _usb_topology_fingerprint）)
...
v1.3.11 相对 v1.3.10（取消/超时后门控口不再滞留 authorized=0，#1025）：
  - 门控恢复此前只挂在 _settle_lock（正常完成 / 异常 / 失败三处）。Agent
    取消与超时是向进程组发 SIGTERM→SIGKILL，脚本没有任何信号处理器 ——
    进程直接死在结算之外，被门控的邻机 MTK 口永久滞留 authorized=0
    （authorized 只对当前 USB 实例生效，重枚举也不会自己回来）→ 邻机在人工
    干预前一直不可用。
  - 新增 SIGTERM/SIGINT/SIGHUP 处理器：先 _settle_lock()（恢复门控 + 释放
    host lock），再把该信号还原为默认动作并 self-signal —— 进程仍以
    WIFSIGNALED 收场，退出码命名空间不变（ADR-0033 §D2：信号死亡语义不改）。
  - 处理器只在 gating / host lock 就位后注册；注册窗口内被 SIGTERM 命中时
    flock 由内核在进程死亡时自动释放，无额外泄漏风险。

v1.3.10 相对 v1.3.9（boot 稳定指纹收窄——非 MTK USB 抖动不再重置窗口）：
  - v1.3.7 的 boot 稳定等待用整棵 USB 树做拓扑指纹——hub 重枚举、串口
    适配器、邻机非 MTK 设备插拔都会重置稳定窗口。多机 host 串行首刷时
    守卫退化成每台耗满 boot_stabilize_max_wait 才按超时放行（.68 串行
    14 台约多 21min）。
  - 收窄：指纹只取 MTK 口（vid=0e8d，复用 _list_mtk_ports）——判据与
    风险面同构，下一任持锁者的 flash_tool 只可能撞上 MTK 可捕获态。
  - _wait_boot_stable：首轮即记基线（不空烧 poll 周期）；空指纹不误判。
  - PROGRESS `done` 戳挪到 boot-stabilize **之后**——此前设备还可能再
    重启一次，按 stage 解析终态的下游会误判。

v1.3.9 相对 v1.3.8（门控保持收窄——201c 风暴干扰目标捕获）：
  - v1.3.8 的周期 regate 隐藏所有刷机态口（含 201c 正常态设备），每 10s
    toggle 14 个口制造 udev 风暴——run 261 实证：工具 120s 扫描被事件
    淹没，S_TIMEOUT(1042) 连目标 3 秒 BROM 窗口都抓不到（attempt 1
    连接后设备已离开、attempt 2 彻底错过）。
  - 收窄：保持阶段只压制 BROM/preloader 态口（0003/2000/2001/3000）——
    幽灵（serial 空 pid 2000）持续被压制，201c 不再周期 toggle。工具的
    BROM 扫描目标只有 2000，201c 本就不干扰捕获。
  - _gate_other_mtk 新增 hold_pids 参数（覆盖被隐藏的 pid 集合；初始
    门控仍用全量 _FLASH_STAGE_PIDS）。
...
v1.3.7 相对 v1.3.6（锁内 boot 稳定等待，2026-08-30 run 258 实证）：
  - **verify 通过 ≠ 设备完成首次开机**。run 258（.68 串行 14 台）11 台
    ...
    boot_completed=1 且 MTK USB 拓扑指纹稳定 boot_stabilize_seconds（设备
    不再重启）才释放锁。失败/卡死路径保持 v1.3.4 兜底语义（确认卡死
    即放行——卡死设备不会重启,无窗口）。
  - 拓扑指纹只取 MTK 口（v1.3.10 收窄；v1.3.7–v1.3.9 曾扫整棵 USB 树）。
  - 新增 PROGRESS 阶段 `boot-stabilize`（打在 `done` 之前）；metrics 新增
    顶层键 `boot_stable`（boot_completed / stable_seconds_elapsed / ok / reason）。
...
v1.3.0 相对 v1.2.0（多设备消歧 · 门控 / 重试环 / 环境预检）：
  - **环境预检**：启动 flash_tool 前一次性检查——可执行位、动态库完整性
    （ldd "not found" 扫描）、adb 可用性、ttyACM 写入路径（进程组集合覆盖
    dialout；否则看规则形态：`0666` 对任何人放行，`0660+dialout` 只对成员放行，
    v1.3.17 起后者在非成员进程下判否——两者皆无 → WARNING，
    strict_env_check 时升级为失败）。
  ...
  - **门控**：以 STP_DEVICE_SERIAL 在 sysfs 反查目标口（必须在 adb reboot
    **之前**做——BROM 态手机 serial 为空），然后把其它所有处于刷机态
    ...
    无提权窄面（wrapper 缺失/旧版本）/ 目标 serial 缺失 / 非 Linux 时跳过
    并记录原因（单设备 host 无感）。
...
# #1591：门控隐藏口清单——SIGKILL 打断 settle 时下一任刷机启动前补恢复
_GATE_STATE_PATH = "/tmp/stp-flash-firmware.gated.json"
...
# v1.3.9：门控保持只压制 BROM/preloader 态。201c（HONOR DA/adb 态）不在
# 工具的 BROM 扫描目标内，周期 toggle 只会制造 udev 风暴干扰目标捕获
# （run 261 实证：v1.3.8 每 10s toggle 14 个 201c 口 → 工具 120s S_TIMEOUT）。
_BROM_STAGE_PIDS = {"0003", "2000", "2001", "3000"}

_PRIV_WRAPPER = "/usr/local/sbin/stp-agent-priv"
_priv_wrapper_cache: "list[bool | None]" = [None]
...
def _priv_wrapper_usable() -> bool:
    """stp-agent-priv 是否具备 usb-authorized 窄面（ADR-0037 D5 / #2133）。

    探针带子命令本身（#2011 教训：纯 `--help` 探针抓不到「探针过、真调用挂」）；
    旧 wrapper / 无 wrapper / 无免密 sudo → False，门控降级并记录原因。
    """
    if _priv_wrapper_cache[0] is None:
        try:
            proc = subprocess.run(
                ["sudo", "-n", _PRIV_WRAPPER, "usb-authorized", "--help"],
                capture_output=True, timeout=10,
            )
            _priv_wrapper_cache[0] = proc.returncode == 0
        except (OSError, subprocess.TimeoutExpired):
            _priv_wrapper_cache[0] = False
    return _priv_wrapper_cache[0]


def _set_authorized(port: str, value: str, base: str = _SYSFS_USB_BASE) -> "tuple[bool, str]":
    """写 authorized；直写（root/0666）失败回落 wrapper 窄面。返回 (ok, how)。

    v1.3.16（#2133）：回落面从 `sudo -n sh -c` 收敛为
    `stp-agent-priv usb-authorized --port <p> --value <v>`（port 正则 +
    idVendor 校验在 wrapper 内强制）；不再存在任意命令面。
    """
    ...
    try:
        ...
        pass
    if not _priv_wrapper_usable():
        return False, "no-priv-face"
    try:
        proc = subprocess.run(
            ["sudo", "-n", _PRIV_WRAPPER, "usb-authorized",
             "--port", port, "--value", value],
            capture_output=True, timeout=10,
        )
        return proc.returncode == 0, "wrapper"
    except (OSError, subprocess.TimeoutExpired):
        return False, "wrapper-failed"


def _gate_other_mtk(target_port: "str | None", base: str = _SYSFS_USB_BASE,
                    hold_pids: "set[str] | None" = None) -> dict:
    """隐藏除 target_port 外所有刷机态 MTK 口。返回报告（永不 raise）。

    hold_pids: 覆盖被隐藏的 pid 集合。v1.3.9 门控保持传 _BROM_STAGE_PIDS
    （只压制 BROM/preloader 幽灵）；初始门控用全量 _FLASH_STAGE_PIDS。
    """
    ...
    stage_pids = set(hold_pids) if hold_pids else _FLASH_STAGE_PIDS
    ports = _list_mtk_ports(base)
    others = {
        name: info for name, info in ports.items()
        if name != target_port and info["pid"] in stage_pids
    }
    if not others:
    ...
    if not report["hidden"]:
        report["skipped_reason"] = (
            "cannot write authorized (need root/dialout+udev or stp-agent-priv usb-authorized)"
        )
    return report
...
def _persist_gated_ports(
    hidden: list, path: str = _GATE_STATE_PATH,
) -> None:
    """#1591：把当前隐藏口落到磁盘，供 SIGKILL 后孤儿恢复。"""
    try:
        payload = json.dumps(
            {"hidden": list(hidden), "pid": os.getpid()},
            ensure_ascii=False,
        )
...
def _clear_gated_state(path: str = _GATE_STATE_PATH) -> None:
    try:
        os.unlink(path)
    except OSError:
        pass
...
def _restore_orphaned_gates(
    base: str = _SYSFS_USB_BASE, path: str = _GATE_STATE_PATH,
) -> dict:
    """#1591：启动时恢复上次未 settle 的门控口（abort→SIGKILL 常见）。"""
    try:
        with open(path, encoding="utf-8") as handle:
            data = json.load(handle)
    except (OSError, json.JSONDecodeError, TypeError):
        return {"restored": [], "errors": {}, "skipped_reason": "no orphan state"}
    hidden = data.get("hidden") if isinstance(data, dict) else None
    if not isinstance(hidden, list) or not hidden:
        _clear_gated_state(path)
        return {"restored": [], "errors": {}, "skipped_reason": "empty orphan state"}
    report = _restore_gated({"hidden": [str(x) for x in hidden]}, base)
    _clear_gated_state(path)
    report["orphan_cleared"] = True
    return report
...
def _udev_rule_form(rules_dir: str = "/etc/udev/rules.d") -> "str | None":
    """rules_dir 里 ttyACM(0e8d) 规则的形态：`"0660"` / `"0666"` / `None`（#2353）。

    两种形态**放行面不同**：`0666` 对任何本地用户放行；`0660 + GROUP="dialout"` 只对
    dialout 成员放行。故"规则在位"不能当作"当前进程可写"的证据（v1.3.16 只认 0666 文本，
    对 0660 返回 False → 命中 dialout 成员的主机会被误报"无规则"；反过来若拿"在位"当可写，
    非成员主机就会被漏报）。rules_dir 可注入（单测），与 sysfs base 同一约定。
    """
    try:
        files = sorted(
    ...
    for path in files:
        ...
        for line in content.splitlines():
            ...
            if 'idvendor}=="0e8d"' not in low and 'kernel=="ttyacm' not in low:
                continue
...
def _precheck_environment(
    ...
    if writable is True:
        ...
        # #2353：节点还没出现（BROM 前）时按**规则形态 × 本进程组集合**判可写性。
        form = _udev_rule_form()
        if form == "0666":
            tty_detail, tty_ok = "udev 0666 rule installed", True
        elif form == "0660":
            tty_detail = (
                "0660+dialout rule present but current process is NOT in dialout "
                "— 0660 grants write only to the group; BROM handshake may fail "
                "with EACCES; fix: usermod -aG dialout <user> then restart the agent")
            tty_ok = False
        else:
            tty_detail = (
                "no /dev/ttyACM* yet to probe; user NOT in dialout and no udev "
                "0e8d rule — BROM handshake may fail with EACCES; fix: "
                "usermod -aG dialout <user> (provisioning) or install udev rule")
            tty_ok = False
...
def _install_termination_cleanup(settle: "callable") -> None:
    """#1025：注册 SIGTERM/SIGINT/SIGHUP —— 被取消也要先恢复门控再死。

    Why: 门控恢复此前只挂在 _settle_lock（正常完成 / 异常 / 失败三处），而
         Agent 取消与超时走的是向进程组发 SIGTERM→SIGKILL：没有处理器时进程
         直接死在结算之外，被门控的邻机 MTK 口永久滞留 authorized=0
         （authorized 只对当前 USB 实例生效，重枚举也不回来）。
    How to apply: settle 必须幂等（_settle_lock 有 lock_settled 闸门）；清理后
                 还原默认动作并 self-signal —— 仍以 WIFSIGNALED 收场，退出码
                 命名空间不变（ADR-0033 §D2：信号死亡视为工具缺陷，语义不改）。
    """
    def _on_signal(signum, _frame) -> None:
        try:
            ...
            pass
        # 清理完必须继续死：还原默认动作后 self-signal，保持信号死亡语义
        try:
            signal.signal(signum, signal.SIG_DFL)
            os.kill(os.getpid(), signum)
        except Exception:
            # self-signal 都没能终止进程时兜底退出 —— 绝不能吞掉取消
    ...
    for sig in (signal.SIGTERM, signal.SIGINT, signal.SIGHUP):
        try:
            ...
            # 非主线程注册 / Windows 无 SIGHUP —— 保持原有行为，不因注册失败
            # 影响刷机主流程
            pass
...
def _usb_topology_fingerprint(base: str = _SYSFS_USB_BASE) -> str:
    """MTK 设备拓扑快照：`port:pid` 排序拼接（复用 _list_mtk_ports）。

    只取 MTK（vid=0e8d）设备——判据与风险面必须同构：下一任持锁者的
    flash_tool 扫描窗只可能撞上 **MTK** 可捕获态。若把整棵 USB 树纳入
    指纹，非 MTK 的 USB 事件（hub 重枚举、串口适配器、键鼠插拔）与本次
    风险无关，却会无限重置稳定窗口：host 上只要有邻机周期性重启，指纹
    几乎不可能收敛，每台都会耗满 boot_stabilize_max_wait 才按「超时
    放行」结算——窗口守卫退化成每轮白等（.68 串行 14 台约多 21 分钟）。

    跳过规则（接口目录含 ':'、根 hub 无 '-'）由 _list_mtk_ports 单点维护；
    门控写 authorized=0 不会把设备目录移出 sysfs，故门控不影响指纹。
    """
    return "|".join(
        f"{name}:{info['pid']}"
        for name, info in sorted(_list_mtk_ports(base).items())
    )
...
def _wait_boot_stable(
    ...
    while time.monotonic() < deadline:
        ...
        if not fp:
            # 指纹为空 = 瞬时读失败：无从判定稳定，也不留旧基线（避免把
            # 失败前后的读数误当成"未变化"）。
            stable_since = None
            last_fp = None
        elif last_fp is None:
            # 首轮只记基线：没有上一次指纹可比，但不必为此空烧一个 poll
            # 周期——此刻若已 boot_completed，稳定窗口就从当前读数起算。
            last_fp = fp
            stable_since = time.monotonic() if booted == "1" else None
        elif fp != last_fp:
            last_fp = fp
            stable_since = None
        elif booted != "1":
...
def main() -> None:
    ...
    # #1591：上次 abort/SIGKILL 可能打断 settle——先恢复孤儿门控再抢锁
    orphan_gates = _restore_orphaned_gates()
    if orphan_gates.get("restored") or orphan_gates.get("errors"):
        _emit_progress(
            seq, stage="orphan-gate-restore",
            result=json.dumps(orphan_gates, ensure_ascii=False),
        )
    ...
    if hidden_ports:
        _persist_gated_ports(hidden_ports)
    ...
    def _settle_on_signal(signum: int) -> None:
        # #1025：取消/超时路径唯一会经过的清理点——先结算再死
        _settle_lock()
        _emit_progress(seq, stage="signal-cleanup", signal=signum)

    _install_termination_cleanup(_settle_on_signal)
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            def _regate() -> None:
                ...
                if now - last_regate_at < 10:
                    return
                last_regate_at = now
                # v1.3.9：保持阶段只压制 BROM/preloader 态口——201c 正常
                # 态设备周期 toggle 会制造 udev 风暴,干扰工具捕获目标的
                # 3 秒 BROM 窗口（run 261 实证:120s S_TIMEOUT）。
                regate = _gate_other_mtk(target_port,
                                         hold_pids=_BROM_STAGE_PIDS)
                for name in regate.get("hidden", []):
                    if name not in hidden_ports:
                        hidden_ports.append(name)
                if hidden_ports:
    ...
    # done 戳必须落在 boot 稳定等待**之后**：在此之前设备还可能再重启一次，
    # 称 done 名不副实，会让按 stage 解析终态的下游误判。
