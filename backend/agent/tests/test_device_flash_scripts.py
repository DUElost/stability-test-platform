...
def test_flash_v1314_model_ready_wait():
    ...
    d = Path(__file__).resolve().parents[2] / "agent/scripts/flash_firmware"
...
def test_oobe_v112_verify_retry():
    ...
    d = Path(__file__).resolve().parents[2] / "agent/scripts/oobe_skip"
...
def test_pipeline_flash_terminate_grace():
    ...
    assert _script_terminate_grace_seconds("/opt/agent/scripts/flash_firmware/flash_firmware.py") == 2.0
