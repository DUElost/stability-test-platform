"""seed flash_firmware v1.3.3 params and deactivate v1.3.1

Revision ID: c0d1e2f3a4b5
Revises: b8c9d0e1f2a3
Create Date: 2026-08-26

Data migration (reboot 时序真机修正):

1. Ensure flash_firmware v1.3.3 exists in the script table with populated
   param_schema and default_params — self-contained and deployment-order
   independent w.r.t. scan_script_root (c0d1e2f3a4b5 precedent).
2. Deactivate v1.3.1 (two generations back). v1.3.2 stays active as the
   rollback path.

Behavioral delta vs v1.3.2: flash_tool now starts BEFORE adb reboot fires
(on_running callback) so the tool is already scanning when the BROM window
opens. Real-device evidence 2026-08-25/26 on 172.21.15.66: the old order
(reboot -> sleep 5s -> start tool) only ever saw post-window normal-mode
enumerations (udev add@ events in tool stdout) -> S_TIMEOUT(1042) on every
attempt; tool-first ordering attached immediately and completed Download
Succeeded at ~36MB/s. pre_reboot_wait_seconds semantics shift to the
tool-start lead time before the reboot fires (default 5s unchanged).
"""
...
# ── flash_firmware v1.3.3 metadata ──────────────────────────────────────────
...
# sha256 of flash_firmware.py v1.3.3
_CONTENT_SHA256 = "d76ba68c1c39459ca6075a2464122fc450e8ff53ea7824936dbebe5b8cd4b184"
...
def upgrade() -> None:
    ...
    # ── 1. Ensure v1.3.3 row exists with metadata ────────────────────────
    row = conn.execute(
        text("SELECT id FROM script WHERE name = :name AND version = :ver"),
        {"name": "flash_firmware", "ver": "1.3.3"},
    ).fetchone()
    ...
    if row is None:
        prior = conn.execute(
            text(
                ...
                "WHERE name = 'flash_firmware' AND version = '1.3.2'"
            ),
        ).fetchone()
        nfs_path = (
            prior.nfs_path.replace("/v1.3.2/", "/v1.3.3/") if prior
            else "flash_firmware/v1.3.3/flash_firmware.py"
        )
        ...
        conn.execute(
            ...
            ),
            {
                ...
                "desc": (
                    "MTK SP Flash Tool firmware flash — v1.3.2 + tool-before-reboot "
                    "ordering (BROM window observed; real-device verified)"
                ),
                "now": now,
            },
        )
...
def downgrade() -> None:
    ...
    # Undo: clear v1.3.3 metadata and reactivate v1.3.1.
    conn.execute(
        text(
