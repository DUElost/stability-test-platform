"""seed flash_firmware v1.3.4 params and deactivate v1.3.2

Revision ID: c0d1e2f3a4b5
Revises: b8c9d0e1f2a3
Create Date: 2026-08-26

Data migration (锁语义真机修正):

1. Ensure flash_firmware v1.3.4 exists in the script table with populated
   param_schema and default_params — self-contained and deployment-order
   independent w.r.t. scan_script_root (c9d0e1f2a3b4 precedent).
2. Deactivate v1.3.2 (two generations back). v1.3.3 stays active as the
   rollback path.

Behavioral delta vs v1.3.3: the host lock is now held through
re-enumeration and post-flash verification — released only after the
flashed phone has stabilized (adb back or confirmed stuck). Scenario-2
evidence (2026-08-26, .66 two devices concurrent): releasing right after
tool exit let the NEXT lock holder's tool scan window hit this phone's
post-download watchdog reboot, flashing new firmware into the WRONG
device (caught by verify; one device double-flashed, one left stale).
Param schema unchanged; new PROGRESS stage lock-released.
"""
...
# ── flash_firmware v1.3.4 metadata ──────────────────────────────────────────
...
# sha256 of flash_firmware.py v1.3.4
_CONTENT_SHA256 = "aec5dd613eabb4e49011d22d608b33f74de923dea65ff91f486b18a50d5cc0bf"
...
def upgrade() -> None:
    ...
    # ── 1. Ensure v1.3.4 row exists with metadata ────────────────────────
    row = conn.execute(
        text("SELECT id FROM script WHERE name = :name AND version = :ver"),
        {"name": "flash_firmware", "ver": "1.3.4"},
    ).fetchone()
    ...
    if row is None:
        ...
        nfs_path = (
            prior.nfs_path.replace("/v1.3.3/", "/v1.3.4/") if prior
            else "flash_firmware/v1.3.4/flash_firmware.py"
        )
        ...
        conn.execute(
            ...
            ),
            {
                ...
                "desc": (
                    "MTK SP Flash Tool firmware flash — v1.3.3 + lock held through "
                    "verify (no cross-device BROM capture; field verified)"
                ),
                "now": now,
            },
        )
...
def downgrade() -> None:
    ...
    # Undo: clear v1.3.4 metadata and reactivate v1.3.2.
    conn.execute(
        text(
