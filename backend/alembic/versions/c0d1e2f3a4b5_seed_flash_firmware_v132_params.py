"""seed flash_firmware v1.3.2 params and deactivate v1.3.0

Revision ID: c0d1e2f3a4b5
Revises: b8c9d0e1f2a3
Create Date: 2026-08-26

Data migration (reboot 语义真机修正):

1. Ensure flash_firmware v1.3.2 exists in the script table with populated
   param_schema and default_params — self-contained and deployment-order
   independent w.r.t. scan_script_root (b8c9d0e1f2a3 precedent).
2. Deactivate v1.3.0 (now two generations back). v1.3.1 stays active as the
   rollback path.

Behavioral delta vs v1.3.1: reboot_target default moves "bootloader" →
"normal" (plain `adb reboot`, passes through the BROM window so a waiting
flash_tool can attach). Real-device evidence 2026-08-25 on 172.21.15.66:
`adb reboot bootloader` lands directly in HONOR fastboot/download mode
(pid 201c), skipping BROM entirely → S_TIMEOUT(1042) on both retry
attempts; plain `adb reboot` → tool catches BROM immediately and completes
Download Succeeded at ~36MB/s. "bootloader"/"fastboot" remain explicit
options in the enum.
"""
...
# ── flash_firmware v1.3.2 metadata ──────────────────────────────────────────
...
# sha256 of flash_firmware.py v1.3.2
_CONTENT_SHA256 = "565022e6d22386cfc2fccca4d4cec4926a0bad5e75156febbc348369700ee837"
...
def upgrade() -> None:
    ...
    # ── 1. Ensure v1.3.2 row exists with metadata ────────────────────────
    row = conn.execute(
        text("SELECT id FROM script WHERE name = :name AND version = :ver"),
        {"name": "flash_firmware", "ver": "1.3.2"},
    ).fetchone()
    ...
    if row is None:
        ...
        nfs_path = (
            prior.nfs_path.replace("/v1.3.1/", "/v1.3.2/") if prior
            else "flash_firmware/v1.3.2/flash_firmware.py"
        )
        ...
        conn.execute(
            ...
            ),
            {
                ...
                "desc": (
                    "MTK SP Flash Tool firmware flash — v1.3.1 + plain adb "
                    "reboot default (passes BROM window; real-device verified)"
                ),
                "now": now,
            },
        )
...
def downgrade() -> None:
    ...
    # Undo: clear v1.3.2 metadata and reactivate v1.3.0.
    conn.execute(
        text(
