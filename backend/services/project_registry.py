...
覆盖 create / update / rename / archive / unarchive 五条登记簿生命周期，以及被多个
路由复用的「取项目 + 来源/归档门禁」。路由退化为「解析 → 调服务 → 序列化」；
异常沿用 ``HTTPException``（与 #1519 / #1520 首个切片一致的服务层既有先例），
``request`` 仅用于审计客户端 IP 提取（可空）。
