...
def sync_host_via_hot_update(host_id: str, db: Session) -> tuple[bool, Optional[str]]:
    ...
    # 故 code 层恒全量（force），plan 只用来取 desired digest——digest 仍随部署写入
    # 远端，使 UI/API 与 --direct 通道进入 no-op 稳态。ADR-0040 D8 R1 起无 resources 层。
    plan = plan_convergence(host, force=True)
