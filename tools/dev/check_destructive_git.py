...
命中破坏性模式即 exit 2（Claude 阻断该次工具调用，stderr 反馈给 agent）。
...
# 阻断模式（作用于「段首为 git」的段内剩余词）
BLOCK_PATTERNS = [
    re.compile(r"\bgit\s+reset\s+--hard\b"),
    # stash 创建/销毁；读侧与恢复侧（list/show/pop/apply/branch）放行
    re.compile(r"\bgit\s+stash\b(?!\s+(?:list|show|pop|apply|branch)\b)"),
]
...
def find_blocked(command: str) -> str | None:
    ...
    for segment in _SEGMENT_SPLIT.split(command):
        ...
        while i < len(words) and _ENV_ASSIGN.fullmatch(words[i]):
            i += 1
        if i < len(words) and words[i] == "git":
            rest = " ".join(words[i:])
