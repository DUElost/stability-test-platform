...
MONITORING_RULES = (
    # #2643 方向 1：站点只装**站点可见面**规则（`site-alerts.yml`），平台全量规则不进
    # 站点安装清单——后者引用的都是控制面进程指标，而站点那个唯一的 node-exporter job
    # 结构上抽不到 → 装了也恒不触发，却让「监控就绪」的假象成立。两侧**条数不在此处固化**
    # （#2663 之后的口径：抄进注释的计数会随规则增删说谎），需要规模时用两份文件的 `alert`
    # 名做集合差当场算。dest 保持原名与原路径：升级时覆盖同名文件，不会在 rules/ 里留下旧副本。
    (
