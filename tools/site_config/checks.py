...
MESSAGES = {
    ...
    # #2277：阶段里未映射的异常（不含已映射的 124/126/127）不再穿成 traceback。
    "stage_crashed": (
        "The stage stopped with an unexpected error; the install report carries this entry "
        "instead of a traceback. See the tool log for the exception, fix the cause and re-run "
    ...
    "release_components_missing": "Include at least the agent-code component digest.",
