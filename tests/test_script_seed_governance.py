...
def deactivation_sites(source: str, *, filename: str = "<migration>") -> list[str]:
    """返回该迁移源码里**逐处**真实的「停用既有版本」位置（#2527）。

    只认两种形态：

    1. SQL：``UPDATE … SET … is_active = false …``（字符串常量，按序且有界窗口）；
...
#:
#: #2527：判据由「整文件子串 `is_active = false`」改为 AST 识别真实停用形态后，本表
#: **新增一条 `i9j0k1l2m3n4`（2026-08-31，legacy 窗口内）**——它写的是
