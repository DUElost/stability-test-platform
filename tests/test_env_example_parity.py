"""环境变量示例对齐门禁（#737）：示例里的每个键名都要在仓库里出现过。

口径（#2062 明示）：本门禁是**拼写/悬挂键的粗筛**，不是「存在真实读取点」的证明
——注释、文档与示例自身都能让键通过（见下方「判据说明」）。要收窄到真实读取点
（`os.getenv` / `os.environ[` / `import.meta.env` / compose `${}` / systemd
`Environment=`），需先处置存量差异：2026-09-15 实测 25 个示例键里 9 个未命中读取点
模式（`AUTH_COOKIE_SECURE` / `CORS_ORIGINS` / `SESSION_WATCHDOG_INTERVAL_SECONDS` 等），
直接收紧会立刻变红而非暴露新问题——列入 #2062 的 Revisit。
