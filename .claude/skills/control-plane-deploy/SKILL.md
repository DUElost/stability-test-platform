...
| **CLI 跑 `run_sweep` 全量 = 静默打脏**（✅2026-09-25 实跑踩坑，#3315/#3333） | verify RPC（`call_agent_rpc`）走 **backend 进程内 socketio 长连接**，CLI 进程里 48 台全 `AgentNotConnectedError`，但 CLI 返回形似成功（`hosts_verified=48/rows=2496`）——副作用：`host.script_packages_mode` 全被 None 打脏、账本按 `agent_offline`（state=unknown）落库并推进 `checked_at`——**新鲜度告警同样被喂假**：`stability_script_presence_sweep_timestamp` 是 `/metrics` 拉取期从库里 `min(checked_at)` 现算的（`backend/api/routes/metrics.py`），垃圾轮会让 `StabilityScriptPresenceSweepStale` 误判新鲜（真 sweep 已死两天时 CLI 一跑即把告警静音）；gap 告警只看 missing/mismatch，unknown 不触发。全量 sweep 合法触发点**只有每日 cron**（`script_presence_sweep_cron`，默认 09:30）；部署后要立即重采走 `POST /refresh?host_id=` 逐台循环（实测 48 台 fail=0，列恢复 `{package:48}`；包模式 gauge 拉取期从列现算，刷完下一次抓取即可见）。入口/防护缺口见 #3333 |
