...
- **渲染 env 面 / 退役键变更 ⇒ 本批 hot-update 必须 `--force`**（#3356 候选 3）：diff 触及 `backend/services/agent_env_sync.py`（渲染键与 `RETIRED_ENV_KEYS` 全在其中）即适用；`check-deploy-readiness.py --diff-base <部署基线>` 会给 WARN 提示。退役键经 wrapper `sync-env --retired-keys-b64` 删除主机 `.env` 残留行（同一次 sync-env 调用，不需要单独跑），删除清单看响应 `env_keys_retired`。**首次携带该参数前，目标机必须先经 `update_agent.yml` 装上新 wrapper**——旧 wrapper 缺 `sync-env/retired-keys` 能力时，远端脚本在任何写动作之前 fail-closed 并提示装 wrapper（#2319 同款能力判据）。
...
| **CLI 跑 `run_sweep` 全量 = 被拒（fail-closed）**（✅2026-09-26 修复；原形态为静默打脏，#3315/#3333） | `run_sweep` 在**任何写动作之前**检查本进程 SocketIO server / Agent 命名空间（`agent_rpc_ready()`）：CLI/独立进程直接 `SweepNotReadyError`，**一行都不写**。原形态：verify RPC（`call_agent_rpc`）走 backend 进程内 socketio 长连接，CLI 里 48 台全 `AgentNotConnectedError`，但 CLI 返回形似成功（`hosts_verified=48/rows=2496`）——`host.script_packages_mode` 全被 None 打脏、账本按 `agent_offline`（state=unknown）落库并推进 `checked_at`，且 `stability_script_presence_sweep_timestamp` 是 `/metrics` 拉取期从库里 `min(checked_at)` 现算的 ⇒ 垃圾轮会把 `StabilityScriptPresenceSweepStale` 喂成新鲜（真 sweep 已死两天时 CLI 一跑即静音告警）。**立即全量重采**走 `POST /api/v1/script-presence/refresh-all`（admin + 60s 节流，backend 进程内跑）；**单机重核**仍走 `POST /refresh?host_id=`（实测 48 台 fail=0，列恢复 `{package:48}`；包模式 gauge 拉取期从列现算，刷完下一次抓取即可见）。全量 sweep 的常设触发点仍只有每日 cron（`script_presence_sweep_cron`，默认 09:30）。claude-skill 校准记录见 §7 |
...
| 2026-09-26 | **更正 09-25「CLI 跑 run_sweep」行的告警结论**：原写「alert 不受影响（吃进程内 gauge）」是**错的**——`stability_script_presence_sweep_timestamp` 在 `/metrics` 拉取期从库现算（`routes/metrics.py:_refresh_script_presence_gauges`），CLI 垃圾轮推进 `checked_at` 即把 `StabilityScriptPresenceSweepStale` 喂成新鲜；§6 行已改。同批：包模式 gauge 改为拉取期从 `host.script_packages_mode` 列现算（原在 sweep 内按切片 set，重启后缺席到下次 cron），部署后无需等 cron 即可核验 | 代码核对（错误断言未先读 gauge 写点）+ `docs/notes/bug-fix/2026-09-26-3315-packages-gauge-scrape-time.md` |
| 2026-09-26 | **#3333 落地**：§6 行改为「CLI 全量 = 被拒」——`run_sweep` 加 `agent_rpc_ready()` fail-closed（不写任何行）+ 新增 `POST /script-presence/refresh-all`（admin + 60s 进程内节流）作为立即全量重采入口；单机 `POST /refresh` 语义不变 | #3333 实施 PR |
